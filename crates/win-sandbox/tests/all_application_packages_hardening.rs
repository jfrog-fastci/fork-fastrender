...
use windows_sys::Win32::System::Threading::{
  ...
  OpenProcessToken, UpdateProcThreadAttribute, LPPROC_THREAD_ATTRIBUTE_LIST,
};
...
const CHILD_ENV: &str = "FASTR_TEST_WIN_AAP_HARDENING_CHILD";
...
fn appcontainer_token_omits_all_application_packages_group_when_hardened() {
  ...
  if is_child {
    // -------------------------------------------------------------------------
    // Child path: validate AppContainer token is present and does not contain AAP.
    // -------------------------------------------------------------------------
    unsafe {
      ...
      assert_ne!(
        is_appcontainer, 0,
        "child token is not an AppContainer token (unexpected sandbox downgrade?)"
      );
      ...
      if has_aap {
        ...
        if !supported {
          eprintln!(
            "skipping: host rejected PROC_THREAD_ATTRIBUTE_ALL_APPLICATION_PACKAGES_POLICY; \
...
when hardening is enabled; token groups={groups:?}"
  ...
  // ---------------------------------------------------------------------------
  // Parent path: spawn this test under win-sandbox CreateProcessW + AppContainer.
  // ---------------------------------------------------------------------------
  if !common::require_appcontainer_profile("win-sandbox ALL APPLICATION PACKAGES hardening test") {
    return;
  }
  ...
  let args = vec![
    OsString::from("--exact"),
    OsString::from("appcontainer_token_omits_all_application_packages_group_when_hardened"),
  ...
  let child = spawn_sandboxed(&cfg).expect("spawn sandboxed child");
  let exit_code = child
    .wait_timeout(Duration::from_secs(30))
    .expect("wait for sandboxed child")
    .expect("sandboxed child exit code");
  assert_eq!(exit_code, 0, "sandboxed child exited with code {exit_code}");
}
