...
unsafe fn is_appcontainer_supported_impl() -> bool {
  ...
  //
  // Even if `userenv.dll` is already loaded, `LoadLibraryExW` returns the existing module handle
  // (refcount bump). We always `FreeLibrary` at the end so this probe doesn't change the process's
  // steady-state DLL set.
  let module: HMODULE = LoadLibraryExW(userenv.as_ptr(), std::ptr::null_mut(), LOAD_LIBRARY_SEARCH_SYSTEM32);
  if module.is_null() {
    return false;
  }
  ...
  // Drop our reference now so this lightweight probe doesn't permanently increase process DLL load
  // surface.
