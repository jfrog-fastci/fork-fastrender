...
pub fn convert_arguments<R: WebIdlJsRuntime>(
  ...
  let required_len = params
    .iter()
    .take_while(|p| !p.optional && !p.variadic && p.default.is_none())
    .count();
  ...
  for (idx, param) in params.iter().enumerate() {
    if param.variadic {
      ...
      for &item in args.iter().skip(idx) {
      ...
      out.push(ConvertedValue::Sequence {
        elem_ty: Box::new(param.ty.clone()),
        values,
      });
      break;
    }
...
fn convert_to_named_type<R: WebIdlJsRuntime>(
  ...
  match named.kind {
    NamedTypeKind::Interface => {
      ...
      // embedding. The conversion itself only needs to validate the brand; bindings can then pass
      // the JS object handle through to the host.
      let obj = to_interface(rt, v, name)?;
      Ok(ConvertedValue::Object(obj))
    }
...
fn convert_to_int(
  ...
  webidl::convert_to_int(n, bit_length, signed, ext).map_err(|e| match e.kind() {
    webidl::NumericConversionErrorKind::TypeError => WebIdlException::type_error(e.message()),
    webidl::NumericConversionErrorKind::RangeError => WebIdlException::range_error(e.message()),
  })
}
...
fn convert_to_dictionary<R: WebIdlJsRuntime>(
  ...
  } in members
  {
    let maybe_converted = rt.with_stack_roots(&roots, |rt| {
      ...
      if required {
        return Err(rt.throw_type_error(&format!(
          "Missing required dictionary member {dict_name}.{name}"
        )));
      }
