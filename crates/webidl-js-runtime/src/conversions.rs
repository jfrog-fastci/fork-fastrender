...
pub fn to_float<R: WebIdlJsRuntime>(rt: &mut R, value: R::JsValue) -> Result<f32, R::Error> {
  ...
  webidl::convert_to_float(x).map_err(|e| match e.kind() {
    webidl::NumericConversionErrorKind::TypeError => rt.throw_type_error(e.message()),
    webidl::NumericConversionErrorKind::RangeError => rt.throw_range_error(e.message()),
  })
}
...
pub fn to_double<R: WebIdlJsRuntime>(rt: &mut R, value: R::JsValue) -> Result<f64, R::Error> {
  let x = rt.to_number(value)?;
  webidl::convert_to_double(x).map_err(|e| match e.kind() {
    webidl::NumericConversionErrorKind::TypeError => rt.throw_type_error(e.message()),
    webidl::NumericConversionErrorKind::RangeError => rt.throw_range_error(e.message()),
  })
}
