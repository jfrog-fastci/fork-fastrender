...
#[test]
fn imports_bundle_with_html_entity_quote_wrapped_css_urls() {
  let repo_root = repo_root();
  let bundle_path = repo_root.join("tests/fixtures/bundle_page/css_url_entity_quotes");

  let temp = tempdir().expect("temp dir");
  let output_root = temp.path().join("fixtures");
  let fixture_name = "css_url_entity_quotes_fixture";
  ...
  let img_bytes = fs::read(bundle_path.join("resources/00000_bg.png")).expect("read bundled image");
  ...
  assert!(
    html.contains(&format!("url(assets/{hash}.png)")),
    "importer should rewrite the decoded URL to a local asset"
  );
  assert!(
    !html.contains("cdn.example.test/bg.png"),
