...
pub(crate) fn is_valid_custom_element_name(name: &str) -> bool {
  ...
  let Some((&first, bytes)) = name.as_bytes().split_first() else {
    return false;
  };
...
mod tests {
  ...
  fn compatibility_mode_skips_placeholder_data_attr_candidates() {
    fn assert_img_src(html: &str, expected: &str) {
      let dom = parse_html_with_options(html, DomParseOptions::compatibility()).expect("parse html");
      let img = find_node_by_id(&dom, "img").expect("img node");
      assert_eq!(img.get_attribute_ref("src"), Some(expected));
    }

    assert_img_src(
      r##"<html><body><img id="img" src="about:blank" data-src="#" data-original="real.jpg"></body></html>"##,
      "real.jpg",
    );
    assert_img_src(
      r##"<html><body><img id="img" src="about:blank" data-delayed-url="#" data-actualsrc="real.jpg"></body></html>"##,
      "real.jpg",
    );
    assert_img_src(
      r##"<html><body><img id="img" src="about:blank" data-img-url="#" data-default-src="real.jpg"></body></html>"##,
      "real.jpg",
    );
  }
  ...
  fn compatibility_mode_lifts_data_default_src_images() {
    ...
    let html = std::fs::read_to_string(&html_path).expect("read fixture HTML");
  ...
  fn compatibility_mode_overwrites_1x1_png_placeholder_img_src_from_data_src() {
    ...
    let html = format!(
      r#"<html><body><img id="img" src="{placeholder}" data-src="real.jpg"></body></html>"#
    );

    let standard_dom = parse_html(&html).expect("parse standard DOM");
    let standard_img = find_by_id(&standard_dom, "img").expect("standard img element");
    assert_eq!(
    ...
    let compat_dom =
      parse_html_with_options(&html, DomParseOptions::compatibility()).expect("parse compat DOM");
    let compat_img = find_by_id(&compat_dom, "img").expect("compat img element");
    assert_eq!(
  ...
  fn compatibility_mode_overwrites_base64_image_header_without_payload_img_src_from_data_src() {
    let html = r#"<html><body><img id="img" src="data:image/png;base64" data-src="real.jpg"></body></html>"#;
...
<div id="a">
  ...
  fn any_link_matches_href_anchors() {
    ...
    assert!(
      !matches(&stylesheet_link, &[], &PseudoClass::AnyLink),
      "<link> elements are not interactive hyperlinks and must not match :any-link"
    );
  }
  ...
  fn slot_uses_fallback_when_unassigned() {
    ...
    assert!(slot.children.iter().any(|c| matches!(
      c.node_type,
      DomNodeType::Text { ref content } if content.contains("fallback")
    )));
  }
  ...
  #[test]
  fn declarative_shadow_dom_is_not_attached_for_invalid_shadow_host() {
    let html = concat!(
      "<select id=host>",
  ...
  fn slot_assignment_times_out_under_expired_deadline() {
    ...
    match err {
      Error::Render(crate::error::RenderError::Timeout { stage, .. }) => {
        assert_eq!(stage, RenderStage::Cascade)
      }
  ...
  "#;
    ...
    let dom2_doc = crate::dom2::parse_html(html).expect("dom2 parse_html");
    ...
    {
      use std::collections::HashSet;
      let mut seen: HashSet<crate::dom2::NodeId> = HashSet::new();
    ...
    assert_eq!(
      crate::debug::snapshot::snapshot_dom(&legacy),
      crate::debug::snapshot::snapshot_dom(&dom2_snapshot.dom),
    ...
    assert!(
      host.children.first().is_some_and(|n| {
        matches!(n.node_type, DomNodeType::ShadowRoot { .. })
      }),
