...
fn fixed_inside_fixed_containing_block_is_supported() {
  ...
  ancestor_style
    .transform
    .push(Transform::Translate(Length::px(1.0), Length::px(1.0)));
  ...
  let fixed =
    FragmentNode::new_block_styled(Rect::from_xywh(0.0, 0.0, 1.0, 1.0), vec![], fixed_style);
  ...
  let intermediate = FragmentNode::new_block(Rect::from_xywh(0.0, 0.0, 10.0, 10.0), vec![fixed]);
...
#[test]
fn extra_animation_timeline_entries_do_not_disable_scroll_blit_when_animation_name_list_is_shorter()
{
  // The engine defines the animation entry count by `animation-name`; extra entries in
  // `animation-timeline` are ignored.
  let style = Arc::new(ComputedStyle {
    animation_names: vec![Some("a".into())],
  ...
  assert!(
    crate::scroll::scroll_blit_supported(&tree),
    "expected extra animation-timeline values beyond animation-name list to not disable scroll blit"
  );
}
