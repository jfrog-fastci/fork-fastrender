...
fn resolve_light_dark_in_color_stops(stops: &mut [crate::css::types::ColorStop], is_dark: bool) {
  for stop in stops {
    stop.color = stop.color.resolve_light_dark(is_dark);
  }
}

fn resolve_light_dark_in_background_image(image: &mut BackgroundImage, is_dark: bool) {
  match image {
    BackgroundImage::LinearGradient { stops, .. }
    | BackgroundImage::RepeatingLinearGradient { stops, .. }
    | BackgroundImage::ConicGradient { stops, .. }
    | BackgroundImage::RepeatingConicGradient { stops, .. } => {
      resolve_light_dark_in_color_stops(stops, is_dark);
    }
...
fn resolve_light_dark_in_border_image_source(source: &mut BorderImageSource, is_dark: bool) {
  if let BorderImageSource::Image(image) = source {
    resolve_light_dark_in_background_image(image, is_dark);
  }
}
  ...
  match property {
    ...
    "mask" => {
      ...
      for parsed in parsed_layers {
        ...
        if let Some(img) = image.as_mut() {
          resolve_light_dark_in_background_image(img, is_dark_color_scheme);
        }
        layer.image = image;
    ...
    "background" => {
      ...
      for (idx, layer_tokens) in layers.into_iter().enumerate() {
        ...
        if let Some(parsed) = parse_background_shorthand(
          &layer_tokens,
          styles.color,
          is_dark_color_scheme,
          allow_color,
    ...
    "border-image" => {
      if let Some(img) = parse_border_image_shorthand(resolved_value) {
        let mut img = img;
        resolve_light_dark_in_border_image_source(&mut img.source, is_dark_color_scheme);
...
fn parse_object_position(value: &PropertyValue) -> Option<ObjectPosition> {
  ...
  fn classify(value: &PropertyValue) -> Option<Part> {
    match value {
      PropertyValue::Keyword(kw) => {
        let lower = kw.to_ascii_lowercase();
        match lower.as_str() {
          "left" => Some(Part::Keyword(AxisKind::Horizontal, PK::Start)),
          "right" => Some(Part::Keyword(AxisKind::Horizontal, PK::End)),
          "top" => Some(Part::Keyword(AxisKind::Vertical, PK::Start)),
          "bottom" => Some(Part::Keyword(AxisKind::Vertical, PK::End)),
          "center" => Some(Part::Keyword(AxisKind::Either, PK::Center)),
          _ => None,
        }
      }
      PropertyValue::Length(len) => Some(Part::Offset(*len)),
  ...
  fn offset_to_component(offset: Length) -> PC {
    if offset.calc.is_some() {
      return PC::Length(offset);
    }
    if offset.unit.is_percentage() {
      return PC::Percentage(offset.value / 100.0);
    }
    PC::Length(offset)
  }

  fn axis_pos_to_component(pos: AxisPos) -> Option<PC> {
    match pos.offset {
      None => Some(PC::Keyword(pos.align)),
      Some(offset) => match pos.align {
  ...
  fn single_axis(part: Part, axis: AxisKind) -> Option<AxisPos> {
    match (part, axis) {
      (Part::Keyword(kind, kw), AxisKind::Horizontal)
        if kind == AxisKind::Horizontal || kind == AxisKind::Either =>
      {
        Some(AxisPos {
          align: kw,
          offset: None,
      ...
      (Part::Keyword(kind, kw), AxisKind::Vertical)
        if kind == AxisKind::Vertical || kind == AxisKind::Either =>
      {
        Some(AxisPos {
          align: kw,
          offset: None,
      ...
      (Part::Offset(len), AxisKind::Horizontal | AxisKind::Vertical) => Some(AxisPos {
        align: PK::Start,
        offset: Some(len),
      }),
      _ => None,
  ...
  let mut x: Option<AxisPos> = None;
  let mut y: Option<AxisPos> = None;

  match parts.len() {
    1 => match parts[0] {
      Part::Keyword(AxisKind::Horizontal, kw) => {
        x = Some(AxisPos {
          align: kw,
          offset: None,
        });
      }
      Part::Keyword(AxisKind::Vertical, kw) => {
        y = Some(AxisPos {
          align: kw,
          offset: None,
        });
      }
      Part::Keyword(AxisKind::Either, kw) => {
        x = Some(AxisPos {
          align: kw,
          offset: None,
        });
...
fn parse_text_decoration_line(value: &PropertyValue) -> Option<TextDecorationLine> {
  ...
  // - Legacy keywords such as `blink` are accepted but have no effect in FastRender.
  if tokens.iter().any(|t| t == "none") {
    return (tokens.len() == 1).then_some(TextDecorationLine::NONE);
  }
...
mod tests {
  ...
  fn display_webkit_box_sets_flag_and_box_orient_parses() {
    let mut styles = ComputedStyle::default();
    let parent = ComputedStyle::default();
    assert_eq!(styles.webkit_box_orient, WebkitBoxOrient::Horizontal);
  ...
  fn appearance_parses_keywords() {
    ...
    apply_declaration(
      &mut style,
      &Declaration {
        property: "-webkit-appearance".into(),
        value: PropertyValue::Keyword("none".into()),
        contains_var: false,
        raw_value: String::new(),
        important: false,
  ...
  #[test]
  fn parses_object_position_edge_offsets_into_calc() {
    let mut style = ComputedStyle::default();
    let decl = Declaration {
      property: "object-position".into(),
      value: parse_property_value("object-position", "right 10px bottom 20px")
    ...
    let expected_x = CalcLength::single(LengthUnit::Percent, 100.0)
      .add_scaled(&CalcLength::single(LengthUnit::Px, 10.0), -1.0)
      .expect("calc terms");
    let expected_y = CalcLength::single(LengthUnit::Percent, 100.0)
      .add_scaled(&CalcLength::single(LengthUnit::Px, 20.0), -1.0)
      .expect("calc terms");

    assert_eq!(
      style.object_position.x,
      PositionComponent::Length(Length::calc(expected_x))
    );
    assert_eq!(
      style.object_position.y,
      PositionComponent::Length(Length::calc(expected_y))
    );
  }

  #[test]
  fn parses_object_position_center_offset_with_other_axis_keyword() {
    let mut style = ComputedStyle::default();
    let decl = Declaration {
      property: "object-position".into(),
      value: parse_property_value("object-position", "center 10px top")
        .expect("object-position parsed"),
      contains_var: false,
      raw_value: String::new(),
      important: false,
    ...
    let expected_x = CalcLength::single(LengthUnit::Percent, 50.0)
      .add_scaled(&CalcLength::single(LengthUnit::Px, 10.0), 1.0)
      .expect("calc terms");
    assert_eq!(
      style.object_position.x,
  ...
  #[test]
  fn text_decoration_line_splits_whitespace_separated_keywords() {
    let mut style = ComputedStyle::default();

    let decl = Declaration {
      property: "text-decoration-line".into(),
      value: PropertyValue::Keyword("underline overline".to_string()),
      contains_var: false,
      raw_value: String::new(),
      important: false,
    };
  ...
  #[test]
  fn text_decoration_line_rejects_invalid_combinations() {
    let mut style = ComputedStyle::default();

    // `none` cannot be combined with other keywords.
    let decl = Declaration {
      property: "text-decoration-line".into(),
      value: PropertyValue::Keyword("none underline".to_string()),
      contains_var: false,
      raw_value: String::new(),
      important: false,
    };
    apply_declaration(&mut style, &decl, &ComputedStyle::default(), 16.0, 16.0);
    assert!(style.text_decoration.lines.is_empty());
    assert!(
      !style.text_decoration_line_specified,
      "invalid declarations should be ignored without mutating the specified flag"
    );

    // Reject mixing non-keyword tokens into the value list.
    let decl = Declaration {
      property: "text-decoration-line".into(),
      value: PropertyValue::Multiple(vec![
        PropertyValue::Keyword("underline".to_string()),
  ...
  #[test]
  fn text_decoration_line_accepts_legacy_keywords_and_clears_lines() {
    let mut style = ComputedStyle::default();
    style.text_decoration.lines = TextDecorationLine::UNDERLINE;

    let decl = Declaration {
      property: "text-decoration-line".into(),
      value: PropertyValue::Keyword("blink".to_string()),
      contains_var: false,
      raw_value: String::new(),
      important: false,
    };
    ...
    assert!(
      style.text_decoration.lines.is_empty(),
      "legacy keywords are accepted but have no representable line flags"
    );
    assert!(style.text_decoration_line_specified);
  }
  ...
  #[test]
  fn text_decoration_shorthand_unions_line_keywords() {
    let mut style = ComputedStyle::default();
  ...
  #[test]
  fn text_decoration_shorthand_rejects_unknown_keywords() {
    let mut style = ComputedStyle::default();
    style.text_decoration.lines = TextDecorationLine::LINE_THROUGH;
    style.text_decoration.style = TextDecorationStyle::Double;
    style.text_decoration.color = Some(Rgba::BLUE);
    ...
    let decl = Declaration {
      ...
      value: PropertyValue::Multiple(vec![
        PropertyValue::Keyword("underline".to_string()),
        PropertyValue::Keyword("bogus".to_string()),
      ]),
      contains_var: false,
      raw_value: String::new(),
      important: false,
    ...
    assert_eq!(
      style.text_decoration, before_decoration,
      "invalid shorthand should be ignored without resetting the computed value"
    );
    assert_eq!(style.text_decoration_line_specified, before_specified);
  }
...
fn parse_background_shorthand(
  ...
  while idx < tokens.len() {
    ...
    if shorthand.image.is_none() {
      if let Some(img) = parse_background_image_value(token) {
        let mut img = img;
        resolve_light_dark_in_background_image(&mut img, is_dark_color_scheme);
