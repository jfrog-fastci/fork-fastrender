...
fn background_position_shorthand_resets_positions() {
  let dom =
    dom::parse_html(r#"<div style="background-position: 20% 30%; background: red;"></div>"#)
      .unwrap();
  let stylesheet = parse_stylesheet("").unwrap();
  let styled = apply_styles_with_media(&dom, &stylesheet, &MediaContext::screen(800.0, 600.0));
...
fn background_size_logical_maps_inline_and_block_horizontal_tb() {
  ...
  let dom = dom::parse_html(
    r#"<div style="background-size-inline: 10px; background-size-block: 20px"></div>"#,
  )
  .unwrap();
  let stylesheet = parse_stylesheet("").unwrap();
  let styled = apply_styles_with_media(&dom, &stylesheet, &MediaContext::screen(800.0, 600.0));
