...
fn create_event(scope: &mut Scope<'_>, event_type: &str) -> Result<Value, VmError> {
...
fn create_progress_event(scope: &mut Scope<'_>, loaded: usize, total: usize) -> Result<Value, VmError> {
  let event = scope.alloc_object()?;
  scope.push_root(Value::Object(event))?;

  let type_s = scope.alloc_string("progress")?;
  scope.push_root(Value::String(type_s))?;
  let type_key = alloc_key(scope, "type")?;
  scope.define_property(event, type_key, data_desc(Value::String(type_s), /* writable */ false))?;
  ...
  let queue_result = event_loop.queue_task(TaskSource::DOMManipulation, move |host, event_loop| {
    ...
    let call_result: Result<(), VmError> = match tick_result {
      Ok(()) => {
        ...
        let ev = create_event(&mut scope, "loadstart")?;
        ...
        let progress_ev = create_progress_event(&mut scope, bytes.len(), bytes.len())?;
        ...
        let outcome_ev = create_event(&mut scope, outcome_event)?;
        dispatch_event_and_handler(
          &mut vm,
          &mut scope,
          vm_host,
        ...
        // loadend.
        let end_ev = create_event(&mut scope, "loadend")?;
  ...
  let queue_result = event_loop.queue_task(TaskSource::DOMManipulation, move |host, event_loop| {
    ...
    let call_result: Result<(), VmError> = match tick_result {
      Ok(()) => {
        ...
        let abort_ev = create_event(&mut scope, "abort")?;
...
mod tests {
  use super::*;
  use crate::dom2;
  use crate::js::window::{WindowHost, WindowHostState};
  ...
  fn install(host: &mut WindowHost) {
    let window_realm = host.host_mut().window_realm();
    let (vm, realm, heap) = window_realm.vm_realm_and_heap_mut();
    install_window_file_reader_bindings::<WindowHostState>(vm, realm, heap).unwrap();
  }
  ...
  fn read_as_array_buffer_matches_bytes() -> crate::error::Result<()> {
    ...
    host.exec_script(
      "globalThis.buf = null;\n\
       ...
       reader.readAsArrayBuffer(new Blob([new Uint8Array([1,2,3])], { type: 'application/octet-stream' }));",
    )?;
