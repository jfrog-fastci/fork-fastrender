...
// `ipc::websocket` exposes its limits as `u32` because the values are serialized over IPC. The JS
// bindings mostly work with Rust containers indexed by `usize`, so keep `usize` projections here.
