...
///
/// This is a conservative, deterministic bound for FastRender's MVP cookie surface; it is not
/// intended to match any particular browser limit.
...
impl CookieJar {
  ...
  pub(crate) fn cookie_string(&self) -> String {
    ...
    for (idx, (name, value)) in self.cookies.iter().enumerate() {
      if idx > 0 {
        out.push_str("; ");
      }
      out.push_str(name);
