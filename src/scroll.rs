...
fn find_first_visible_box_in_viewport(
  node: &FragmentNode,
  parent_origin: Point,
  viewport_rect: Rect,
) -> Option<(usize, Rect)> {
  let abs_bounds = node.bounds.translate(parent_origin);
  if abs_bounds.intersection(viewport_rect).is_some() {
    if let Some(box_id) = fragment_box_id(node) {
      return Some((box_id, abs_bounds));
    }
  }

  let self_origin = abs_bounds.origin;
  for child in node.children.iter() {
    if let Some(found) = find_first_visible_box_in_viewport(child, self_origin, viewport_rect) {
      return Some(found);
    }
  }
...
fn find_first_box_bounds_by_id(
  node: &FragmentNode,
  parent_origin: Point,
  box_id: usize,
) -> Option<Rect> {
  let abs_bounds = node.bounds.translate(parent_origin);
  if fragment_box_id(node) == Some(box_id) {
    return Some(abs_bounds);
  }

  let self_origin = abs_bounds.origin;
  for child in node.children.iter() {
    if let Some(found) = find_first_box_bounds_by_id(child, self_origin, box_id) {
      return Some(found);
    }
  }
...
/// Applies scroll anchoring to the viewport scroll offset by comparing the previous and next layout.
///
/// Scroll anchoring (CSS Scroll Anchoring 1) adjusts the scroll position to compensate for layout
/// shifts above the user's viewport, keeping the "anchor" element visually stable. This helper
/// implements a minimal heuristic:
/// - Choose the first fragment with a box id that intersects the viewport in the previous layout.
/// - Find the corresponding fragment in the next layout.
/// - Adjust the viewport scroll offset by the delta of that fragment's **block-start edge**.
///
/// The adjustment is writing-mode aware: for vertical writing modes (`writing-mode: vertical-*`),
/// the block axis is horizontal and the scroll offset is updated in `scroll_state.viewport.x`.
pub fn apply_scroll_anchoring(
  prev: &FragmentTree,
  next: &FragmentTree,
  scroll: &ScrollState,
) -> ScrollState {
  let viewport = prev.viewport_size();

  let (writing_mode, direction) = prev
    .root
    .style
    .as_deref()
  ...
  let viewport_rect =
    viewport_rect_for_scroll_state(scroll.viewport, viewport, writing_mode, direction);

  let Some((anchor_box_id, prev_bounds)) =
    find_first_visible_box_in_viewport(&prev.root, Point::ZERO, viewport_rect)
  else {
    return scroll.clone();
  };

  let Some(next_bounds) = find_first_box_bounds_by_id(&next.root, Point::ZERO, anchor_box_id)
  else {
    return scroll.clone();
  };

  let block_positive = block_axis_positive(writing_mode);
  let block_start_prev = if block_axis_is_horizontal(writing_mode) {
    if block_positive {
      prev_bounds.min_x()
    } else {
      prev_bounds.max_x()
  ...
  let block_start_next = if block_axis_is_horizontal(writing_mode) {
    if block_positive {
      next_bounds.min_x()
    } else {
      next_bounds.max_x()
  ...
  let delta = block_start_next - block_start_prev;
  if !delta.is_finite() {
    return scroll.clone();
  }

  let sign = if block_positive { 1.0 } else { -1.0 };
  let mut out = scroll.clone();
  if block_axis_is_horizontal(writing_mode) {
    out.viewport.x = out.viewport.x + sign * delta;
    if !out.viewport.x.is_finite() {
      out.viewport.x = 0.0;
    } else if out.viewport.x < 0.0 {
      out.viewport.x = 0.0;
    }
  } else {
    out.viewport.y = out.viewport.y + sign * delta;
    if !out.viewport.y.is_finite() {
      out.viewport.y = 0.0;
    } else if out.viewport.y < 0.0 {
      out.viewport.y = 0.0;
    }
  }
  out.viewport_delta = Point::new(
    out.viewport.x - scroll.viewport.x,
    out.viewport.y - scroll.viewport.y,
  );
