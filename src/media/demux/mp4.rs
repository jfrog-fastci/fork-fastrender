...
fn parse_avc_sample_entry(
  ...
  // For H.264, expose the raw AVCDecoderConfigurationRecord (avcC) bytes as codec_private.
...
mod tests {
  ...
  fn demuxer_h264_codec_private_is_decoder_format() {
    ...
    // `Mp4Demuxer` should expose the raw avcC bytes (`AVCDecoderConfigurationRecord`), which starts
    // with configurationVersion = 1 and contains at least one SPS and PPS.
    let data = &h264.codec_private;
    assert!(!data.is_empty(), "expected non-empty H264 codec_private");
    assert!(
      data.len() >= 7,
      "expected at least 7 bytes for AVCDecoderConfigurationRecord"
    );
    assert_eq!(data[0], 1, "expected avcC configurationVersion == 1");
    assert_eq!(data[4] & 0b1111_1100, 0b1111_1100, "reserved bits");
    assert_eq!(data[5] & 0b1110_0000, 0b1110_0000, "reserved bits");

    let nal_length_size = (data[4] & 0b11) + 1;
    assert!((1..=4).contains(&nal_length_size));
    ...
    for _ in 0..sps_count {
      let len = u16::from_be_bytes([
        *data.get(i).expect("sps len hi"),
        *data.get(i + 1).expect("sps len lo"),
      ]) as usize;
      i += 2;
      let end = i + len;
    ...
    for _ in 0..pps_count {
      let len = u16::from_be_bytes([
        *data.get(i).expect("pps len hi"),
        *data.get(i + 1).expect("pps len lo"),
      ]) as usize;
      i += 2;
      let end = i + len;
