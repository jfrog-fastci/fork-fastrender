...
impl Painter {
  ...
  fn paint_inside_border_rect(&mut self, rect: Rect, color: Rgba, clip_mask: Option<&Mask>) {
    ...
    // Chrome's 1px border sits inside the image box. Using a stroke here would center the stroke
    ...
    self.paint_solid_rect_simple(Rect::from_xywh(x, y, w, th), color, clip_mask);
    if bottom_y > y {
      self.paint_solid_rect_simple(Rect::from_xywh(x, bottom_y, w, th), color, clip_mask);
    }

    self.paint_solid_rect_simple(Rect::from_xywh(x, y, tw, h), color, clip_mask);
    if right_x > x {
      self.paint_solid_rect_simple(Rect::from_xywh(right_x, y, tw, h), color, clip_mask);
    }
  }
  ...
  fn paint_broken_image_icon(&mut self, icon_rect: Rect, clip_mask: Option<&Mask>) {
    ...
    if inner_rect.width() > 0.0 && inner_rect.height() > 0.0 {
      self.paint_solid_rect_simple(inner_rect, Rgba::WHITE, clip_mask);
  ...
  fn paint_inside_border_rect(&mut self, rect: Rect, color: Rgba, clip_mask: Option<&Mask>) {
    ...
    // Chrome's 1px border sits inside the image box. Using a stroke would center the line on the
    // edge, which gets clipped/anti-aliased and ends up darker than browsers.
    let thickness: f32 = 1.0;
    let th = thickness.min(h);
    let tw = thickness.min(w);
    let x = rect.x();
    ...
    self.paint_solid_rect_crisp(Rect::from_xywh(x, y, w, th), color, clip_mask);
    if bottom_y > y {
      self.paint_solid_rect_crisp(Rect::from_xywh(x, bottom_y, w, th), color, clip_mask);
    }

    self.paint_solid_rect_crisp(Rect::from_xywh(x, y, tw, h), color, clip_mask);
    if right_x > x {
      self.paint_solid_rect_crisp(Rect::from_xywh(right_x, y, tw, h), color, clip_mask);
    }
  }
  ...
  fn paint_broken_image_icon(&mut self, icon_rect: Rect, clip_mask: Option<&Mask>) {
    ...
    if inner_rect.width() > 0.0 && inner_rect.height() > 0.0 {
      self.paint_solid_rect_crisp(inner_rect, Rgba::WHITE, clip_mask);
