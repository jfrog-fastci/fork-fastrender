...
#[test]
fn display_list_bidi_split_caret_uses_affinity() {
  ...
  // Caret should be a thin vertical line (snapped to device pixels by the display list builder).
  assert!(
    up_max_x.saturating_sub(up_min_x) <= 2,
    "expected upstream caret to be thin (min_x={up_min_x}, max_x={up_max_x}, count={up_count})"
  );
  assert!(
    down_max_x.saturating_sub(down_min_x) <= 2,
    "expected downstream caret to be thin (min_x={down_min_x}, max_x={down_max_x}, count={down_count})"
  );

  let delta = (down_min_x as i32 - up_min_x as i32).abs();
  assert!(
    delta >= 5,
    "expected split caret affinities to paint at different x positions (up={up_min_x}, down={down_min_x}, delta={delta})"
  );
}
