...
use crate::{DiagnosticsLevel, FastRender, FastRenderConfig, RenderOptions};
...
fn foreign_object_renders_nested_html_children() {
  std::thread::Builder::new()
    ...
    .spawn(|| {
      ...
      let toggles = crate::debug::runtime::RuntimeToggles::from_map(
...
fn foreign_object_overflow_hidden_clips_filter_effects() {
  std::thread::Builder::new()
    ...
    .spawn(|| {
      ...
      let toggles = crate::debug::runtime::RuntimeToggles::from_map(std::collections::HashMap::from([(
        "FASTR_PAINT_BACKEND".to_string(),
        "display_list".to_string(),
      )]));
      let options = RenderOptions::new()
