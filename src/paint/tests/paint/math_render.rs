...
use crate::Rgba;
use crate::{FastRender, FragmentContent, FragmentNode};
...
fn stix_math_style(font_size: f32) -> crate::ComputedStyle {
...
fn scripts_on_italic_identifiers_offset_and_raise() {
  ...
  let (sup_origin, sup_advance) = combined
    ...
    .get(base_fragments)
    .and_then(|f| match f {
      crate::math::MathFragment::Glyph { origin, run, .. } => Some((*origin, run.advance)),
      _ => None,
    })
