...
use crate::paint::text_rasterize::{TextRasterizer, TextRenderState};
use crate::paint::text_shadow::{resolve_text_shadows, PathBounds};
use crate::style::color::{Color, Rgba};
use crate::style::font_palette::FontPaletteRegistry;
...
fn render_legacy_text_shadows(
  ...
  origin_x: f32,
  baseline_y: f32,
  shadows: &[crate::paint::display_list::TextShadowItem],
  pixmap: &mut Pixmap,
) {
  ...
  for shadow in shadows {
    ...
    if shadow.blur_radius > 0.0 {
      crate::paint::blur::apply_gaussian_blur_anisotropic(
        &mut shadow_pixmap,
        shadow.blur_radius,
        shadow.blur_radius,
      )
...
fn display_list_preserves_palette_overrides_for_text_shadows() {
  ...
  for (run, style, origin_x) in [
    ...
    let shadows: Vec<crate::paint::display_list::TextShadowItem> = resolve_text_shadows(style)
      .into_iter()
      .map(|shadow| crate::paint::display_list::TextShadowItem {
        offset: crate::geometry::Point::new(shadow.offset_x, shadow.offset_y),
