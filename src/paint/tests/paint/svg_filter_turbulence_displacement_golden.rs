...
const HTML_PATH: &str = "tests/fixtures/html/svg_filter_turbulence_displacement.html";
const GOLDEN_PATH: &str = "tests/fixtures/golden/svg_filter_turbulence_displacement.png";
const DIFF_DIR: &str = "target/svg_filter_turbulence_displacement_diffs";
const VIEWPORT: (u32, u32) = (192, 96);
...
fn render_fixture() -> Vec<u8> {
  ...
  let pixmap = renderer
    .render_html_with_options(
      &fs::read_to_string(HTML_PATH).expect("read turbulence+displacement fixture"),
      options,
    )
    .expect("render turbulence+displacement html");
  encode_image(&pixmap, OutputFormat::Png).expect("encode png")
}

fn should_update_golden() -> bool {
  std::env::var("UPDATE_SVG_FILTER_TURBULENCE_DISPLACEMENT_GOLDEN").is_ok()
}

#[test]
fn svg_filter_turbulence_displacement_matches_golden() {
  let compare_config =
