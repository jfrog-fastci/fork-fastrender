...
use crate::paint::display_list::{DisplayItem, DisplayList, GlyphInstance, TextItem};
...
fn block_pos(item: &TextItem, axis: InlineAxis) -> f32 {
  match axis {
    ...
    InlineAxis::Vertical => {
      crate::paint::display_list::text_bounds(item)
        .center()
        .x
    }
  }
...
fn line_metrics_in_rect(list: &DisplayList, rect: Rect, axis: InlineAxis) -> Vec<LineMetrics> {
  ...
  let mut items: Vec<(usize, &TextItem)> = list
    ...
    .filter(|(_idx, text)| {
      ...
      let bounds = crate::paint::display_list::text_bounds(text);
