...
use crate::{BorderRadii, FastRender, Length, LengthUnit, Point, Rect, Rgba};
...
fn cpu_budget_allows_parallel_paint() -> bool {
  // Paint parallelism is clamped by `crate::system::cpu_budget()` (e.g. cgroup quotas).
...
#[test]
fn manual_blend_mode_triggers_parallel_paint_in_auto_mode() {
  // `PaintParallelismMode::Auto` relies on heuristics to decide whether tiling is worth the
  // overhead. Manual blend modes (HSL/HSV/OKLCH conversions + plus-darker) are per-pixel and can
  // dominate runtime; ensure they are treated as expensive enough to enable tiling.
  let mut list = DisplayList::new();
...
#[test]
fn non_manual_blend_mode_triggers_parallel_paint_in_auto_mode() {
  // Non-manual blend modes (tiny-skia) still require compositing an intermediate surface back into
  // the destination. Treat them as expensive enough in auto mode to enable tiling for large
  // blend-mode layers.
  let mut list = DisplayList::new();
  let viewport = Rect::from_xywh(0.0, 0.0, 1024.0, 1024.0);
  list.push(DisplayItem::FillRect(FillRectItem {
    rect: viewport,
    color: Rgba::from_rgba8(200, 0, 0, 255),
  }));
  ...
  let font_ctx = FontContext::new();
  let serial = DisplayListRenderer::new(1024, 1024, Rgba::WHITE, font_ctx.clone())
    .unwrap()
    .with_parallelism(PaintParallelism::disabled())
    .render(&list)
    .expect("serial paint");
  ...
  let pool = ThreadPoolBuilder::new().num_threads(4).build().unwrap();
  let report = pool.install(|| {
    DisplayListRenderer::new(1024, 1024, Rgba::WHITE, font_ctx)
      .unwrap()
      .with_parallelism(parallelism)
      .render_with_report(&list)
      .expect("auto paint")
...
#[test]
fn hue_blend_mode_allows_parallel_tiling() {
  // `hue` is implemented via the manual blend-mode compositor (HSL conversion path). This is used
  // by `background-blend-mode` and SVG/CSS blend mode layers; ensure it remains tile-friendly and
  // produces byte-identical results when tiled.
  let mut list = DisplayList::new();
  list.push(DisplayItem::FillRect(FillRectItem {
    rect: Rect::from_xywh(0.0, 0.0, 96.0, 96.0),
    color: Rgba::from_rgba8(200, 200, 40, 255),
...
#[test]
fn color_oklch_blend_mode_allows_parallel_tiling() {
  // `color-oklch` is implemented via the manual blend-mode compositor (OKLCH conversion path).
  // Ensure it remains tile-friendly and produces byte-identical results when tiled.
  let mut list = DisplayList::new();
  list.push(DisplayItem::FillRect(FillRectItem {
    rect: Rect::from_xywh(0.0, 0.0, 96.0, 96.0),
...
#[test]
fn plus_darker_blend_mode_allows_parallel_tiling() {
  // `plus-darker` uses a bespoke manual compositor path (not expressible via tiny-skia). Ensure it
  // remains tile-friendly and produces byte-identical results when tiled.
  let mut list = DisplayList::new();
  list.push(DisplayItem::FillRect(FillRectItem {
...
#[test]
fn html_mix_blend_mode_hue_hsb_maps_to_hue_hsv_and_allows_parallel_paint() {
  // Exercise the end-to-end CSS → computed style → display list mapping for the `hue-hsb` alias.
  // This path uses the manual HSV compositor and should remain tile-parallel friendly.
  let html = r#"
    <!doctype html>
    <style>
      body { margin: 0; }
      .base { position: relative; width: 96px; height: 96px; }
      .left { position: absolute; left: 0; top: 0; width: 48px; height: 96px; background: rgb(255 0 0); }
      .right { position: absolute; left: 48px; top: 0; width: 48px; height: 96px; background: rgb(128 128 128); }
      .blend {
        position: absolute;
        left: 8px;
        top: 8px;
  ...
  let (serial, report) = pool.install(|| {
    ...
    assert!(
      display_list.items().iter().any(|item| matches!(
        item,
        DisplayItem::PushStackingContext(sc) if sc.mix_blend_mode == BlendMode::HueHsv
      )),
      "expected display list to include mix-blend-mode: hue-hsv stacking context"
    );
...
#[test]
fn html_background_blend_mode_color_oklch_maps_and_allows_parallel_paint() {
  // Ensure extended blend modes wired through `background-blend-mode` (PushBlendMode) stay tile
  // parallel and map to the manual OKLCH compositor.
  let html = r#"
    <!doctype html>
    <style>
      body { margin: 0; }
      .box {
        ...
        height: 96px;
        background-image:
          linear-gradient(180deg, rgb(180 0 255), rgb(180 0 255)),
          linear-gradient(180deg, rgb(30 30 30), rgb(220 220 220));
        background-blend-mode: color-oklch, normal;
      }
    </style>
    <div class="box"></div>
  "#;
...
fn auto_parallelizes_expensive_gradients() {
  use crate::paint::display_list::{GradientSpread, GradientStop, LinearGradientItem};
