...
use crate::text::font_db::{FontConfig, FontDatabase};
use crate::{ComputedStyle, FontContext, Rgba, ShapingPipeline, TextRasterizer};
...
fn render_text_with_translucent_backdrop() -> Pixmap {
  let mut raw = HashMap::new();
  ...
  let runs = pipeline
    .shape("A", &style, &font_ctx)
    .expect("shape glyph for subpixel AA translucent backdrop test");
  let run = runs.first().expect("expected at least one shaped run");

  let mut pixmap = Pixmap::new(96, 96).expect("pixmap");
  pixmap.fill(tiny_skia::Color::from_rgba8(255, 255, 255, 128));

  // Use a fractional x origin so the glyph edges land between device pixels.
  let mut rasterizer = TextRasterizer::new();
  rasterizer
    .render_shaped_run(run, 10.3, 64.0, Rgba::BLACK, &mut pixmap)
    .expect("render shaped run");
...
#[test]
fn text_subpixel_aa_translucent_backdrop_is_grayscale() {
  let pixmap = render_text_with_translucent_backdrop();
  assert_eq!(
    count_colored_pixels(&pixmap),
    0,
    "expected subpixel AA to fall back to grayscale edges on non-opaque backdrops"
  );
}
