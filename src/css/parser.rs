...
#[derive(Debug, Clone)]
struct ParsedMediaQueryList {
  queries: Arc<[MediaQuery]>,
  matches: Option<bool>,
}

type ParsedMediaQueryListCache<'i> = FxHashMap<&'i str, ParsedMediaQueryList>;
...
/// Parse a list of CSS rules within a nested context.
///
...
/// Parse a group rule's `{ ... }` contents.
///
/// At the top level, group rules like `@media`/`@supports` contain a nested rule list.
/// When nested within a style rule's block (i.e. `parent_selectors` is present), CSS Nesting
/// allows the group rule's block to contain both:
/// - declarations that apply to the parent style rule under the group rule condition, and
/// - nested rules that inherit the parent selector context.
///
/// Spec ref: CSS Nesting Module Level 1, "Nested Group Rules".
fn parse_group_rule_contents<'i, 't>(
  parser: &mut Parser<'i, 't>,
  errors: &mut CssErrorCollector,
  parent_selectors: Option<&SelectorList<FastRenderSelectorImpl>>,
  css_source: &str,
  media_ctx: Option<&MediaContext>,
  media_query_cache: &mut Option<&mut MediaQueryCache>,
  parsed_media_query_list_cache: &mut ParsedMediaQueryListCache<'i>,
  namespace_state: &mut NamespaceParseState,
) -> std::result::Result<Vec<CssRule>, ParseError<'i, SelectorParseErrorKind<'i>>> {
  if let Some(parent_selectors) = parent_selectors {
    let (declarations, nested_rules) = parse_style_block(
      parser,
      errors,
      parent_selectors,
      css_source,
    ...
    if declarations.is_empty() {
      ...
      Ok(vec![CssRule::Style(StyleRule {
    ...
    Ok(parse_rule_list_with_context(
      parser,
      errors,
      None,
      css_source,
...
fn parse_rule<'i, 't>(
  ...
  if at_rule {
    ...
    if kw.eq_ignore_ascii_case("import") {
      // `@import` rules are only valid in the stylesheet prelude (after optional `@charset` and
      // before any qualified rules/other at-rules). Per spec, `@import` inside nested rule lists
      // (e.g. inside `@media`) is invalid and must be ignored.
      //
      // We also disallow `@import` inside nested selectors (`.foo { @import ... }`): there is no
      // sensible interpretation for placing a full stylesheet inside a style rule's nesting
      // context.
      if !is_top_level || parent_selectors.is_some() {
        skip_at_rule(parser);
        return Ok(None);
      }
      if !namespace_state.imports_allowed() {
        skip_at_rule(parser);
        return Ok(None);
      }
...
fn parse_media_rule<'i, 't>(
  ...
  let (queries, matches) = {
    let entry = parsed_media_query_list_cache
      .entry(prelude)
      .or_insert_with(|| {
        let parsed = MediaQuery::parse_list(prelude).unwrap_or_default();
    ...
    let queries = Arc::clone(&entry.queries);
    let matches = media_ctx.map(|media_ctx| {
      if let Some(value) = entry.matches {
        return value;
      }

      let value =
        media_ctx.evaluate_list_with_cache(&entry.queries, media_query_cache.as_deref_mut());
      entry.matches = Some(value);
      value
    });
    (queries, matches)
  };
...
fn parse_container_condition<'i, 't>(
  ...
  parse_container_disjunction(parser)
}

fn parse_container_disjunction<'i, 't>(
  parser: &mut Parser<'i, 't>,
) -> std::result::Result<ContainerQuery, ParseError<'i, ()>> {
  let mut conditions = vec![parse_container_conjunction(parser)?];
  loop {
    parser.skip_whitespace();
    if parser.try_parse(|p| p.expect_ident_matching("or")).is_ok() {
      parser.skip_whitespace();
      conditions.push(parse_container_conjunction(parser)?);
    } else {
      break;
    }
  }
...
fn parse_container_conjunction<'i, 't>(
  parser: &mut Parser<'i, 't>,
) -> std::result::Result<ContainerQuery, ParseError<'i, ()>> {
  let mut conditions = vec![parse_container_negation(parser)?];
  loop {
    parser.skip_whitespace();
    if parser.try_parse(|p| p.expect_ident_matching("and")).is_ok() {
      parser.skip_whitespace();
      conditions.push(parse_container_negation(parser)?);
    } else {
      break;
    }
  }
...
fn parse_container_negation<'i, 't>(
  parser: &mut Parser<'i, 't>,
) -> std::result::Result<ContainerQuery, ParseError<'i, ()>> {
  parser.skip_whitespace();
...
fn parse_font_variation_setting<'i, 't>(
  ...
  let value = match parser.next()? {
    ...
    Token::Function(name) if name.as_ref().eq_ignore_ascii_case("calc") => {
      ...
      ) -> std::result::Result<f32, cssparser::ParseError<'i, ()>> {
...
mod tests {
  ...
  fn nested_rule_errors_are_reported() {
    ...
    let nested_error = result
      .errors
      .iter()
      .find(|err| err.line >= 3)
      .expect("expected nested error with location");
    assert!(
      nested_error
        .snippet
        .as_deref()
        .map(|line| line.contains("color: ;"))
        .unwrap_or(false),
      "snippet should point inside nested block, got {:?}",
      nested_error
    );
...
"#;
  ...
  #[test]
  fn ignores_import_rule_with_empty_href() {
    let css = r#"
        @import url("");
        body { color: red; }
      "#;
  ...
  #[test]
  fn font_face_src_skips_empty_url() {
    let css = r#"@font-face {
            font-family: "TestFamily";
            src: url("") format("woff2"), local(TestLocal);
        }"#;
    let stylesheet = parse_stylesheet(css).unwrap();
    assert_eq!(stylesheet.rules.len(), 1);
    match &stylesheet.rules[0] {
  ...
  #[test]
  fn media_query_preludes_share_parsed_query_lists() {
    let css = r#"
    ...
    let stylesheet = parse_stylesheet(css).unwrap();
    assert_eq!(stylesheet.rules.len(), 2);
    let (first, second) = match (&stylesheet.rules[0], &stylesheet.rules[1]) {
      (CssRule::Media(first), CssRule::Media(second)) => (first, second),
      other => panic!("expected two media rules, got {:?}", other),
    };
    assert!(
      Arc::ptr_eq(&first.queries, &second.queries),
      "expected media query list to be shared across identical preludes"
    );
  }
