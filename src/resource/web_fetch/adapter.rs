...
use crate::resource::{
  origin_from_url, validate_cors_allow_origin, DocumentOrigin, FetchCredentialsMode, FetchDestination,
  FetchRequest, FetchedResource, HttpRequest, ReferrerPolicy, ResourceFetcher,
};
...
struct CorsPreflightCacheEntry {
  ...
  allow_headers_wildcard: bool,
  allow_methods: HashSet<Method>,
  allow_headers: HashSet<HeaderName>,
}

fn cors_preflight_cache(
) -> &'static Mutex<HashMap<CorsPreflightCacheKey, CorsPreflightCacheEntry>> {
  static CACHE: OnceLock<Mutex<HashMap<CorsPreflightCacheKey, CorsPreflightCacheEntry>>> =
    OnceLock::new();
  CACHE.get_or_init(|| Mutex::new(HashMap::new()))
}

fn cors_unsafe_request_header_names(headers: &[(String, String)]) -> HashSet<HeaderName> {
  // https://fetch.spec.whatwg.org/#cors-unsafe-request-header-names
  let mut unsafe_names: Vec<HeaderName> = Vec::new();
  let mut potentially_unsafe_names: Vec<HeaderName> = Vec::new();
  ...
  for (name, value) in headers {
    ...
    if super::headers::is_cors_safelisted_request_header(&name, value) {
  ...
  unsafe_names.into_iter().collect()
}

fn parse_cors_preflight_list_values<T>(
  resource: &FetchedResource,
  header_name: &str,
  mut parse_token: impl FnMut(&str) -> Option<T>,
) -> std::result::Result<(bool, HashSet<T>), ()>
where
  T: Eq + std::hash::Hash,
{
  // Fetch "extract header list values" splits list headers on commas and trims HTTP OWS.
  // https://fetch.spec.whatwg.org/#concept-header-list-extract
  let mut wildcard = false;
  let mut out: HashSet<T> = HashSet::new();

  for raw_value in resource.header_values(header_name) {
    for raw_token in raw_value.split(',') {
      let token = trim_http_whitespace(raw_token);
      if token.is_empty() {
        continue;
      }
      if token == "*" {
...
fn parse_cors_preflight_max_age(resource: &FetchedResource) -> Option<u64> {
  // https://fetch.spec.whatwg.org/#http-access-control-max-age
  // https://fetch.spec.whatwg.org/#concept-header-list-extract
  let values = resource.header_values("access-control-max-age");
  match values.as_slice() {
    [] => None,
    [value] => trim_http_whitespace(value).parse::<u64>().ok(),
    // `Access-Control-Max-Age` is not a list header; treat multiple headers as invalid.
    _ => None,
  }
}

fn should_cors_preflight(
  method_is_safelisted: bool,
  unsafe_header_names: &HashSet<HeaderName>,
) -> bool {
  !method_is_safelisted || !unsafe_header_names.is_empty()
}

fn cors_preflight_allows_method(
  method: &Method,
  method_is_safelisted: bool,
  allow_methods: &HashSet<Method>,
  allow_methods_wildcard: bool,
  ...
  method_is_safelisted || allow_methods.contains(method) || (!credentialed && allow_methods_wildcard)
}

fn cors_preflight_allows_header(
  header: &HeaderName,
  allow_headers: &HashSet<HeaderName>,
  allow_headers_wildcard: bool,
  credentialed: bool,
) -> bool {
  allow_headers.contains(header) || (!credentialed && allow_headers_wildcard)
}

fn maybe_run_cors_preflight(
  fetcher: &dyn ResourceFetcher,
  fetch_request: FetchRequest<'_>,
  request_method: &str,
  method_is_safelisted: bool,
  request_headers: &[(String, String)],
) -> Result<()> {
  let Some(client_origin) = fetch_request.client_origin else {
    return Ok(());
  };
  let Some(target_origin) = origin_from_url(fetch_request.url) else {
    return Ok(());
  };
  if !target_origin.is_http_like() {
    return Ok(());
  }
  ...
  let Ok(method) = Method::from_bytes(request_method.as_bytes()) else {
    // `execute_web_fetch` validates the method token; treat this as unreachable.
    return Ok(());
  };

  let unsafe_header_names = cors_unsafe_request_header_names(request_headers);
  if !should_cors_preflight(method_is_safelisted, &unsafe_header_names) {
    return Ok(());
  }

  let credentialed = fetch_request.credentials_mode == FetchCredentialsMode::Include;
  let origin_key = if client_origin.is_http_like() {
    client_origin.to_string()
  } else {
    "null".to_string()
  };
  let cache_key = CorsPreflightCacheKey {
    origin: origin_key,
    url: fetch_request.url.to_string(),
    credentialed,
  };

  {
    ...
    let mut cache = cors_preflight_cache()
      .lock()
      .map_err(|_| Error::Other("web fetch preflight cache lock was poisoned".to_string()))?;
    if let Some(entry) = cache.get(&cache_key) {
      if now < entry.expires_at {
        ...
        if !has_authorization || entry.allow_headers.contains(&authorization) {
          if cors_preflight_allows_method(
            &method,
            method_is_safelisted,
            &entry.allow_methods,
            entry.allow_methods_wildcard,
            credentialed,
          ) && unsafe_header_names.iter().all(|name| {
            cors_preflight_allows_header(
              name,
              &entry.allow_headers,
              entry.allow_headers_wildcard,
              credentialed,
            )
          }) {
            return Ok(());
  ...
  // Use `same-origin` credentials for the preflight request itself (per Fetch, which does not
  // explicitly copy the request's credentials mode to the `preflight` request), but validate the
  // response using the original request credentials mode.
  ...
  if !unsafe_header_names.is_empty() {
    let mut names: Vec<&str> = unsafe_header_names.iter().map(|name| name.as_str()).collect();
    names.sort_unstable();
    preflight_headers.push((
  ...
  let response = fetcher.fetch_http_request(preflight_req)?;
  let status = response.status.unwrap_or(0);
  if !(200..300).contains(&status) {
    return Err(Error::Resource(ResourceError::new(
      fetch_request.url,
      format!("CORS preflight failed with HTTP status {status}"),
    )));
  }
  ...
  if let Err(message) = validate_cors_allow_origin(
    &response,
    fetch_request.url,
    fetch_request.client_origin,
    fetch_request.credentials_mode,
  ) {
    return Err(Error::Resource(ResourceError::new(fetch_request.url, message)));
  }

  let (allow_methods_wildcard, allow_methods) =
    parse_cors_preflight_list_values(&response, "access-control-allow-methods", |token| {
      Method::from_bytes(token.as_bytes()).ok()
    })
    .map_err(|_| {
      Error::Resource(ResourceError::new(
        fetch_request.url,
        "CORS preflight failed: invalid Access-Control-Allow-Methods".to_string(),
      ))
    })?;

  let (allow_headers_wildcard, allow_headers) =
    parse_cors_preflight_list_values(&response, "access-control-allow-headers", |token| {
      HeaderName::from_bytes(token.as_bytes()).ok()
    })
    .map_err(|_| {
      Error::Resource(ResourceError::new(
        fetch_request.url,
        "CORS preflight failed: invalid Access-Control-Allow-Headers".to_string(),
      ))
    })?;
  ...
  let has_authorization = request_headers
    .iter()
    .any(|(name, _)| name.eq_ignore_ascii_case(authorization.as_str()));
  if has_authorization && !allow_headers.contains(&authorization) {
    return Err(Error::Resource(ResourceError::new(
      fetch_request.url,
      "CORS preflight blocked request header: authorization".to_string(),
    )));
  }

  for unsafe_name in unsafe_header_names.iter() {
    if !cors_preflight_allows_header(
      unsafe_name,
      &allow_headers,
      allow_headers_wildcard,
      credentialed,
    ) {
      return Err(Error::Resource(ResourceError::new(
        fetch_request.url,
        format!("CORS preflight blocked request header: {}", unsafe_name.as_str()),
      )));
    }
  }

  let max_age = parse_cors_preflight_max_age(&response).unwrap_or(5);
  let expires_at = Instant::now()
    .checked_add(Duration::from_secs(max_age))
    .unwrap_or_else(Instant::now);
  let entry = CorsPreflightCacheEntry {
    expires_at,
    allow_methods_wildcard,
    allow_headers_wildcard,
    allow_methods,
...
pub fn execute_web_fetch<'a>(
  ...
  if request.mode == RequestMode::Cors {
    maybe_run_cors_preflight(
      fetcher,
      fetch_request,
      method,
      method_is_get || method_is_head || method_is_post,
      &user_header_pairs,
    )?;
  }
...
mod tests {
  ...
  fn cors_preflight_cache_skips_second_options() {
    ...
    let handle = thread::spawn(move || {
      for idx in 0..3 {
        let mut stream =
          accept_http_stream(&listener, "cors_preflight_cache_skips_second_options");
        stream
          .set_read_timeout(Some(Duration::from_millis(500)))
          .unwrap();
        let (headers, _body) = read_http_request(&mut stream);
    ...
    assert!(first.starts_with("options /cache"), "first request: {first}");
    assert!(second.starts_with("put /cache"), "second request: {second}");
    assert!(third.starts_with("put /cache"), "third request: {third}");
  }

  #[test]
  fn cors_preflight_cache_does_not_cache_without_max_age() {
    if skip_if_curl_backend_missing("cors_preflight_cache_does_not_cache_without_max_age") {
      return;
    }
    let Some(listener) = try_bind_localhost("cors_preflight_cache_does_not_cache_without_max_age")
    else {
      return;
    };
    let addr = listener.local_addr().unwrap();
    ...
    let handle = thread::spawn(move || {
      for idx in 0..4 {
        let mut stream = accept_http_stream(&listener, "cors_preflight_cache_does_not_cache_without_max_age");
        stream
          .set_read_timeout(Some(Duration::from_millis(500)))
          .unwrap();
        let (headers, _body) = read_http_request(&mut stream);
        ...
        match idx {
          0 | 2 => {
            ...
            // No Access-Control-Max-Age header: should not be cached.
            let response = "HTTP/1.1 204 No Content\r\nAccess-Control-Allow-Origin: *\r\nAccess-Control-Allow-Methods: PUT\r\nAccess-Control-Allow-Headers: X-Test\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
            stream.write_all(response.as_bytes()).unwrap();
          }
          1 | 3 => {
    ...
    assert!(lines[2].starts_with("options /cache"), "request[2]: {}", lines[2]);
    assert!(lines[3].starts_with("put /cache"), "request[3]: {}", lines[3]);
  }
  ...
  fn cors_preflight_cache_partitions_by_credentials() {
    ...
    let Some(listener) = try_bind_localhost("cors_preflight_cache_partitions_by_credentials") else {
      return;
    };
    let addr = listener.local_addr().unwrap();
    let captured = Arc::new(Mutex::new(Vec::<String>::new()));
    ...
    let handle = thread::spawn(move || {
      for idx in 0..5 {
        let mut stream = accept_http_stream(&listener, "cors_preflight_cache_partitions_by_credentials");
        stream
          .set_read_timeout(Some(Duration::from_millis(500)))
          .unwrap();
        let (headers, _body) = read_http_request(&mut stream);
    ...
    assert!(lines[3].starts_with("options /cache"), "request[3]: {}", lines[3]);
    assert!(lines[4].starts_with("put /cache"), "request[4]: {}", lines[4]);
  }
