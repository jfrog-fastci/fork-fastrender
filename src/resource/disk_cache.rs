...
pub struct DiskCacheConfig {
  ...
  ///
  /// Disk cache keys are derived from the tuple `(FetchContextKind, url)` (and the optional
  /// namespace) to avoid serving cached bytes fetched under one header profile to a different
  /// profile.
...
impl<F: ResourceFetcher> DiskCachingFetcher<F> {
  ...
  fn cache_key(&self, kind: FetchContextKind, url: &str) -> String {
    self.cache_key_with_partition(kind, url, None, super::CacheCredentialsPartition::Include)
  }

  fn cache_key_with_partition(
    &self,
    ...
    hasher.update(&[b'0' + kind.cache_id()]);
    hasher.update(b"\n");
    ...
    hasher.update(&[b'0' + credentials_partition.cache_id()]);
    hasher.update(b"\n");
  ...
  fn artifact_key(&self, kind: FetchContextKind, url: &str, artifact: CacheArtifactKind) -> String {
    self.artifact_key_with_partition(
      ...
      artifact,
      None,
      super::CacheCredentialsPartition::Include,
    )
  }

  fn artifact_key_with_partition(
    &self,
    kind: FetchContextKind,
    url: &str,
    ...
    let base = self.cache_key_with_partition(kind, url, origin_key, credentials_partition);
    match artifact {
  ...
  fn stored_at_for_cached_entry(
    ...
    let base_key = self.cache_key_with_partition(kind, url, origin_key, credentials_partition);
  ...
  fn alias_path(&self, kind: FetchContextKind, url: &str) -> PathBuf {
    self.alias_path_for_key(&self.cache_key(kind, url))
  }

  fn alias_path_with_partition(
    &self,
    kind: FetchContextKind,
    url: &str,
    origin_key: Option<&str>,
    ...
    self.alias_path_for_key(&self.cache_key_with_partition(
      kind,
      url,
      origin_key,
  ...
  fn read_alias_target(
    ...
    let alias_path = self.alias_path_with_partition(kind, url, origin_key, credentials_partition);
  ...
  fn persist_alias(
    ...
    let alias_path = self.alias_path_with_partition(kind, alias, origin_key, credentials_partition);
  ...
  fn persist_resource_with_partition(
    &self,
    kind: FetchContextKind,
    url: &str,
    request: FetchRequest<'_>,
    ...
    if let Some(vary) = resource.vary.as_deref() {
      ...
      {
        self.remove_alias_for(kind, url, origin_key, credentials_partition);
        let key = self.cache_key_with_partition(kind, url, origin_key, credentials_partition);
        let data_path = self.data_path_for_key(&key);
        let meta_path = self.meta_path_for_data(&data_path);
        self.remove_entry_files_best_effort_if_unlocked(&key, &data_path, &meta_path);
        return;
...
impl<F: ResourceFetcher> ResourceFetcher for DiskCachingFetcher<F> {
  ...
  fn fetch_with_request(&self, req: FetchRequest<'_>) -> Result<FetchedResource> {
    ...
    self.fetch_cached(kind, req.url, Some(req))
  }
  ...
  fn fetch_http_request(&self, req: HttpRequest<'_>) -> Result<FetchedResource> {
    ...
    // Delegate to the shared in-memory caching layer. It will bypass caches for non-GET methods and
    // for requests with user-specified headers/bodies.
    self.memory.fetch_http_request(req)
  }
...
mod tests {
  ...
  const TEST_KIND: FetchContextKind = FetchContextKind::Other;
  ...
  fn disk_caching_fetcher_fetch_http_request_caches_get_and_bypasses_post() {
    ...
    let first = fetcher.fetch_http_request(get_req).expect("GET 1");
    assert_eq!(first.bytes, b"get:1");
    let second = fetcher.fetch_http_request(get_req).expect("GET 2");
    assert_eq!(
    ...
    let first_post = fetcher.fetch_http_request(post_req).expect("POST 1");
    assert_eq!(first_post.bytes, b"http:1:POST");
    let second_post = fetcher.fetch_http_request(post_req).expect("POST 2");
  ...
  #[test]
  fn disk_cache_rejects_existing_unhandled_vary_entries() {
    if matches!(
      super::super::http_backend_mode(),
      super::super::HttpBackendMode::Curl
    ) && !super::super::curl_backend::curl_available()
    {
      eprintln!(
        "skipping disk_cache_rejects_existing_unhandled_vary_entries: curl backend selected but curl is unavailable"
      );
      return;
    }

    let Some(listener) = try_bind_localhost("disk_cache_rejects_existing_unhandled_vary_entries")
    else {
      return;
    };
    let addr = listener.local_addr().unwrap();
    ...
    let handle = thread::spawn(move || {
      ...
      while start.elapsed() < Duration::from_secs(2) {
        match listener.accept() {
          ...
          Err(err) => panic!("accept disk_cache_rejects_existing_unhandled_vary_entries: {err}"),
        }
      }
    });
    ...
    let allow_config = DiskCacheConfig {
      allow_unhandled_vary: true,
    ...
    let allow_fetcher = DiskCachingFetcher::with_configs(
      HttpFetcher::new().with_timeout(Duration::from_secs(2)),
      cache_dir,
      memory_config,
      allow_config,
    ...
    let strict_fetcher = DiskCachingFetcher::new(
      HttpFetcher::new().with_timeout(Duration::from_secs(2)),
      cache_dir,
    );
    ...
    assert_eq!(
      request_count.load(Ordering::SeqCst),
      2,
      "expected unhandled Vary entry persisted under allow_unhandled_vary to be ignored once the strict policy is applied"
    );
    ...
    let handle = thread::spawn(move || {
      ...
      while start.elapsed() < Duration::from_secs(5) {
        match listener.accept() {
          ...
          Err(err) if err.kind() == io::ErrorKind::WouldBlock => {
            if captured_for_thread.lock().unwrap().len() >= expected_requests
      ...
      let received = captured_for_thread.lock().unwrap().len();
      assert_eq!(
        received, expected_requests,
        "expected {expected_requests} requests, got {received}"
      );
    });
  ...
  #[test]
  fn disk_cache_partitions_cors_font_entries_by_origin_when_enforced() {
    let Some((url, captured, server)) = spawn_cors_origin_echo_font_server(
      "disk_cache_partitions_cors_font_entries_by_origin_when_enforced",
      2,
    ) else {
      return;
    };
  ...
  #[test]
  fn disk_cache_shares_same_origin_entries_between_same_origin_and_include_credentials() {
    let toggles = Arc::new(runtime::RuntimeToggles::from_map(HashMap::from([(
      "FASTR_FETCH_ENFORCE_CORS".to_string(),
      "0".to_string(),
    )])));
    runtime::with_thread_runtime_toggles(toggles, || {
      ...
      let tmp = tempfile::tempdir().unwrap();
      let calls = Arc::new(AtomicUsize::new(0));
      let origin = super::super::origin_from_url("https://a.test/page").expect("origin");
      ...
      assert_eq!(
        calls.load(Ordering::SeqCst),
        1,
        "expected only one underlying fetch"
      );
      ...
      assert_eq!(
        ...
        "expected include request to reuse same disk cache entry as same-origin credentials"
      );
    });
  }

  #[test]
  fn disk_cache_shares_cross_origin_entries_between_same_origin_and_omit_credentials() {
    let toggles = Arc::new(runtime::RuntimeToggles::from_map(HashMap::from([(
      "FASTR_FETCH_ENFORCE_CORS".to_string(),
      "0".to_string(),
    )])));
  ...
  fn partitions_cache_by_kind() {
    ...
    let url = "https://example.com/resource";
    let doc_key = disk.cache_key(FetchContextKind::Document, url);
    let font_key = disk.cache_key(FetchContextKind::Font, url);
    assert_ne!(
      doc_key, font_key,
      "cache key should incorporate FetchContextKind"
    );
  }
  ...
  #[test]
  fn disk_cache_does_not_update_aliases_on_http_error_refresh() {
    #[derive(Clone, Debug)]
    ...
    let error_final_url = "https://example.com/error";
    ...
    let first = disk.fetch(start_url).expect("seed fetch");
    assert_eq!(first.bytes, b"cached");
  ...
  fn removes_corrupted_entries_on_read() {
    ...
    let request = FetchRequest::new(url, TEST_KIND.into());
    let credentials_partition = super::super::CacheCredentialsPartition::for_request(&request);
    let snapshot = disk
      .read_disk_entry(TEST_KIND, url, request, None, credentials_partition)
  ...
  fn disk_cache_error_entries_expire_and_are_removed() {
    crate::debug::runtime::with_thread_runtime_toggles(
      ...
      || {
        ...
        assert!(
          disk
            .read_disk_entry(kind, url, request, None, credentials_partition)
            .expect("read disk")
            .is_none(),
