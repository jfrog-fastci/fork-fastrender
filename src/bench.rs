//! Benchmark safety helpers.
//!
//! FastRender's Criterion benchmarks are designed to run on large fixture corpora. To avoid
//! accidental OOMs or unbounded thread usage when running benches locally/CI, the bench suite reads
//! a small set of environment variables and applies conservative defaults.
//!
//! This module lives in `src/` so:
//! - it can be shared by both `benches/*` and unit tests (without `#[path = ...]` shims), and
//! - its parsing/validation logic is exercised by `cargo test --lib`.
//!
//! It is not used by the renderer at runtime.

use std::fs::File;
use std::io::{self, Read};
use std::sync::{Once, OnceLock};

/// Bench limits derived from environment variables.
