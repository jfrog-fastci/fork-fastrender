...
impl Document {
  ...
  /// Returns the "tree parent" for `node` when traversing the DOM tree.
  ///
  /// This differs from [`Document::parent_node`] because dom2 stores internal edges that are not
  /// part of the light DOM tree:
  /// - A `ShadowRoot` is stored as a child of its host element, but it must not be exposed via DOM
  ///   tree traversal starting from the host/document.
  /// - `<template>` contents are represented as children of the `<template>` element with
  ///   `inert_subtree=true` on that element; inert subtree roots behave as leaves for DOM tree
  ///   traversal.
  #[inline]
  pub(crate) fn tree_parent_node(&self, node: NodeId) -> Option<NodeId> {
    ...
    // Unless we're *inside* a shadow root, don't treat a ShadowRoot as a tree child of its host.
    if !matches!(parent_node.kind, NodeKind::ShadowRoot { .. }) {
  ...
  /// Returns true if `ancestor` is a tree inclusive ancestor of `node`.
  ///
  /// The "tree" here matches DOM tree traversal semantics used by NodeIterator/TreeWalker and
  /// `Node.childNodes`-style APIs: ShadowRoot host edges are skipped and inert subtrees behave as
  /// disconnected.
  pub(crate) fn is_tree_inclusive_ancestor(&self, ancestor: NodeId, node: NodeId) -> bool {
    ...
    while let Some(id) = current {
      ...
      current = self.tree_parent_node(id);
    }
    false
  }
  ...
  /// Returns the first child node in DOM tree traversal order.
  ///
  /// Tree traversal differs from dom2's raw `Node::children` representation:
  /// - Inert subtree roots (currently: `<template>`) behave as leaves.
  /// - A `ShadowRoot` node stored under its host element is not exposed as a tree child.
  pub fn first_tree_child(&self, node: NodeId) -> Option<NodeId> {
    let node_ref = self.get_node(node)?;
    node_ref
      .children
      .iter()
      .copied()
      .find(|&child| self.tree_parent_node(child) == Some(node))
  }
  ...
  /// Returns the last child node in DOM tree traversal order.
  ///
  /// Tree traversal differs from dom2's raw `Node::children` representation:
  /// - Inert subtree roots (currently: `<template>`) behave as leaves.
  /// - A `ShadowRoot` node stored under its host element is not exposed as a tree child.
  pub fn last_tree_child(&self, node: NodeId) -> Option<NodeId> {
    let node_ref = self.get_node(node)?;
    node_ref
      .children
      .iter()
  ...
  pub fn tree_previous_sibling(&self, node: NodeId) -> Option<NodeId> {
    let parent = self.tree_parent_node(node)?;
    let parent_node = self.get_node(parent)?;
    let pos = parent_node.children.iter().position(|&c| c == node)?;
    parent_node
      .children
      ...
      .rev()
      .copied()
      .find(|&sib| self.tree_parent_node(sib) == Some(parent))
  }
  ...
  pub fn tree_next_sibling(&self, node: NodeId) -> Option<NodeId> {
    let parent = self.tree_parent_node(node)?;
    let parent_node = self.get_node(parent)?;
    let pos = parent_node.children.iter().position(|&c| c == node)?;
    parent_node
  ...
  pub fn tree_last_inclusive_descendant(&self, node: NodeId) -> NodeId {
    if !self.contains_node(node) {
      return node;
    }
    ...
    while remaining > 0 {
      remaining -= 1;
      let Some(last_child) = self.last_tree_child(current) else {
        break;
      };
      current = last_child;
    }
  ...
  pub fn tree_following_in_subtree(&self, root: NodeId, node: NodeId) -> Option<NodeId> {
    if !self.contains_node(root) || !self.contains_node(node) {
      return None;
    }

    if !self.is_tree_inclusive_ancestor(root, node) {
      return None;
    }

    if let Some(first_child) = self.first_tree_child(node) {
      return Some(first_child);
    }
    ...
    while remaining > 0 {
      ...
      if let Some(next_sibling) = self.tree_next_sibling(current) {
        return Some(next_sibling);
      }
      current = self.tree_parent_node(current)?;
    }
  ...
  pub fn tree_preceding_in_subtree(&self, root: NodeId, node: NodeId) -> Option<NodeId> {
    if !self.contains_node(root) || !self.contains_node(node) {
      return None;
    }
    ...
    if let Some(previous_sibling) = self.tree_previous_sibling(node) {
      return Some(self.tree_last_inclusive_descendant(previous_sibling));
    }

    self.tree_parent_node(node)
  }
