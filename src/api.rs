...
fn aspect_ratio_is_usable(value: f32) -> bool {
  value.is_finite() && value > 0.0
}

fn intrinsic_size_is_complete(size: Size) -> bool {
  intrinsic_axis_is_known(size.width) && intrinsic_axis_is_known(size.height)
}

fn complete_intrinsic_size_with_ratio(size: Size, ratio: f32) -> Option<Size> {
  if !aspect_ratio_is_usable(ratio) {
    return None;
  }
  let width_known = intrinsic_axis_is_known(size.width);
  let height_known = intrinsic_axis_is_known(size.height);
  match (width_known, height_known) {
    (true, false) => Some(Size::new(size.width, size.width / ratio)),
    (false, true) => Some(Size::new(size.height * ratio, size.height)),
    _ => None,
  }
}
...
pub struct RenderCounts {
  ...
  #[serde(default)]
  pub fallback_cache_glyph_evictions: Option<usize>,
  #[serde(default)]
...
pub struct CascadeDiagnostics {
  ...
  pub selector_attempts_after_bloom: Option<u64>,
  /// Selector attempts rejected by the ancestor bloom filter fast path.
  pub selector_bloom_fast_rejects: Option<u64>,
...
pub struct ResourceDiagnostics {
  ...
  #[serde(skip_serializing_if = "Option::is_none")]
  pub fetch_inflight_waits: Option<usize>,
  #[serde(skip_serializing_if = "Option::is_none")]
  pub fetch_inflight_wait_ms: Option<f64>,
  #[serde(skip_serializing_if = "Option::is_none")]
...
fn merge_resource_cache_diagnostics(stats: &mut RenderStats) {
  if let Some(resource_stats) = crate::resource::take_resource_cache_diagnostics() {
    ...
    stats.resources.resource_cache_stale_hits = Some(resource_stats.stale_hits);
    ...
    stats.resources.resource_cache_bytes = Some(resource_stats.resource_cache_bytes);
    ...
    stats.resources.fetch_inflight_waits = Some(resource_stats.fetch_inflight_waits);
    stats.resources.fetch_inflight_wait_ms = Some(resource_stats.fetch_inflight_wait_ms);
...
impl Default for FastRenderPoolConfig {
  fn default() -> Self {
    let pool_size = crate::system::cpu_budget();
    Self {
...
fn overflow_establishes_sticky_scrollport(overflow: crate::style::types::Overflow) -> bool {
  matches!(
    overflow,
...
impl FastRender {
  ...
  fn update_base_url_from_dom(&mut self, dom: &DomNode) {
    ...
    fn file_url_prefers_local_base(doc_url: &str) -> bool {
      let Ok(url) = Url::parse(doc_url) else {
        return false;
      };
      if url.scheme() != "file" {
    ...
    let file_http_base = match document_url {
      Some(doc_url) if doc_url.starts_with("file://") => {
        // Directory-style bundles (e.g. `.../fixture/index.html`) should resolve relative URLs
        // against the local filesystem path so bundled subresources under `assets/` load offline.
  ...
  pub fn layout_document_for_media_with_options(
    ...
    // This is a public entry point (used by CLI/tools) that can consult runtime toggles for media
    // preference overrides, profiling gates, etc. Install the renderer's configured toggles for the
  ...
  fn layout_document_for_media_with_artifacts_owned_single_pass(
    ...
    if has_container_queries {
      ...
      let mut include_visibility = false;
      ...
      for name in container_style_query_properties.iter() {
        match name.to_ascii_lowercase().as_str() {
          ...
          "visibility" => include_visibility = true,
  ...
  fn resolve_intrinsic_for_image_for_media_with_probes(
    ...
    // If a usable aspect ratio is already present, we can derive the missing intrinsic axis
    ...
    let srcset_ratio_hint = || -> Option<f32> {
      ...
      match selected.descriptor {
        Some(SrcsetDescriptor::WidthHeight { width, height }) => {
          let ratio = width as f32 / height as f32;
          aspect_ratio_is_usable(ratio).then_some(ratio)
        }
        _ => None,
      }
    };
...
fn visibility_fingerprint(visibility: crate::style::computed::Visibility) -> u8 {
  match visibility {
    crate::style::computed::Visibility::Visible => 0,
    crate::style::computed::Visibility::Hidden => 1,
    crate::style::computed::Visibility::Collapse => 2,
  }
}
  ...
  for id in keys {
    ...
    if let Some(config) = config {
      ...
      if config.include_visibility {
        visibility_fingerprint(info.styles.visibility).hash(&mut hasher);
      }
...
<html>
  ...
  #[test]
  fn diagnostics_populates_style_and_layout_stage_timings_with_external_stylesheet_imports() {
...
<html>
  ...
  fn sticky_ignores_overflow_clip_when_selecting_scroll_container() {
    ...
    clip_style.border_bottom_width = Length::px(0.0);
    clip_style.border_left_width = Length::px(0.0);
    ...
    let sticky = FragmentNode::new_with_style(
      Rect::from_xywh(0.0, 40.0, 200.0, 10.0),
      FragmentContent::Block { box_id: Some(2) },
      vec![],
      sticky_style,
    );
    let filler = FragmentNode::new(
      Rect::from_xywh(0.0, 50.0, 200.0, 200.0),
      FragmentContent::Block { box_id: None },
      vec![],
    );
    let clip_container = FragmentNode::new_with_style(
      Rect::from_xywh(0.0, 50.0, 200.0, 100.0),
      FragmentContent::Block { box_id: Some(1) },
      vec![sticky, filler],
      clip_style,
    ...
    let scroll_state = ScrollState::with_viewport(Point::new(0.0, 100.0));
    renderer.apply_sticky_offsets_to_tree_with_scroll_state(&mut tree, &scroll_state);
  ...
  #[test]
  fn container_query_fingerprint_tracks_visibility_used_by_style_queries() {
    let stylesheet = crate::css::parser::parse_stylesheet(
      "@container style(visibility: hidden) { .a { color: red; } }",
    )
    .expect("stylesheet parses");
    let media_ctx = MediaContext::screen(800.0, 600.0);
    let metadata = stylesheet.collect_css_metadata_with_cache(&media_ctx, None);
    ...
    fn ctx_with_visibility(
      media_ctx: &MediaContext,
      visibility: crate::style::computed::Visibility,
    ) -> ContainerQueryContext {
      let mut style = ComputedStyle::default();
    ...
    let ctx_a = ctx_with_visibility(&media_ctx, crate::style::computed::Visibility::Visible);
    let ctx_b = ctx_with_visibility(&media_ctx, crate::style::computed::Visibility::Hidden);
    let fp_a = container_query_context_fingerprint(&ctx_a, Some(&cfg));
    let fp_b = container_query_context_fingerprint(&ctx_b, Some(&cfg));
    assert_ne!(
      fp_a, fp_b,
  ...
  fn resolve_intrinsic_sizes_image_width_only_preserves_natural_ratio() {
    ...
    assert_eq!(replaced.intrinsic_size, Some(Size::new(120.0, 60.0)));
  ...
  #[test]
  fn resolve_intrinsic_sizes_invalid_src_does_not_suppress_alt_sizing() {
    let mut renderer = FastRender::new().expect("init renderer");
    // Ensure fragment-only references resolve to an `about:` URL so accidental probes manifest as
    // placeholder 1×1 metadata instead of requiring network access.
    renderer.set_base_url("about:blank");
    let style = Arc::new(ComputedStyle::default());
    ...
    assert!(
      intrinsic.width > 1.0 && intrinsic.height > 1.0,
      "expected alt text sizing, got {intrinsic:?}"
    );
  }

  #[test]
  fn resolve_intrinsic_sizes_ignores_invalid_non_img_candidates() {
    #[derive(Clone)]
    struct PanicFetcher;

    impl ResourceFetcher for PanicFetcher {
      fn fetch(&self, _url: &str) -> Result<FetchedResource> {
        panic!("intrinsic sizing should not probe invalid URLs");
      }
    }
    ...
    let mut root = BoxNode::new_anonymous_block(
      style.clone(),
      vec![
    ...
    renderer.resolve_replaced_intrinsic_sizes(&mut root, Size::new(800.0, 600.0));
  }
  ...
  #[test]
  fn file_index_html_preserves_local_base_url_over_canonical() {
    let html = r#"<!doctype html><html><head>
        <link rel="canonical" href="https://good.example/app/">
        <link rel="stylesheet" href="assets/style.css">
      </head><body></body></html>"#;
    let document_url = "file:///tmp/bundle/index.html";
    let stylesheet_url = "file:///tmp/bundle/assets/style.css";
