//! Shared helpers for Criterion benchmarks and their unit tests.
//!
//! Benchmark targets under `benches/` are separate crates that depend on the `fastrender` library.
//! To keep benchmarks safe/deterministic (especially in CI), we centralize environment parsing and
//! fixture size limits here so they can be unit-tested without `#[path]` shims.
