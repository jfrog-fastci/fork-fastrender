...
///
/// This matches `crate::resource::DiskCacheConfig::default()` when built with `disk_cache`.
...
fn default_disk_cache_max_bytes() -> u64 {
  ...
  {
    crate::resource::DiskCacheConfig::default().max_bytes
  }
...
fn default_disk_cache_max_age_secs() -> u64 {
  #[cfg(feature = "disk_cache")]
  {
    crate::resource::DiskCacheConfig::default()
      .max_age
      .map(|age| age.as_secs())
      .unwrap_or(0)
  }
...
#[cfg(feature = "disk_cache")]
impl DiskCacheArgs {
  pub fn to_config(&self) -> crate::resource::DiskCacheConfig {
    crate::resource::DiskCacheConfig {
...
pub struct MemoryGuardArgs {
  ...
  ///
  /// When enabled, known allocation hot paths (pixmap/buffer allocations during paint, decoded
  /// image buffers, CSS parse inputs, display list construction) are accounted against the active
  /// stage heartbeat and the render aborts with a structured
  /// `RenderError::StageAllocationBudgetExceeded { stage, heartbeat, allocated_bytes, budget_bytes, context }`
  /// when the counter exceeds this value.
  #[arg(long, value_name = "MB", default_value_t = 0)]
