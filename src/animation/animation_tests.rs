...
#[test]
fn matrix_interpolation_2d_roundtrips_translation_with_skew() {
  // Regression: 2D matrix decomposition must be a true decomposition so interpolating a matrix
  // against itself produces the same matrix (even when the linear part contains shear/skew).
  let sheet = parse_stylesheet(
    "@keyframes skew { from { transform: matrix(1, 0, 0.5773503, 1, 128.85, 50); } to { transform: matrix(1, 0, 0.5773503, 1, 128.85, 50); } }",
  )
  .unwrap();
  let keyframes = sheet.collect_keyframes(&MediaContext::screen(800.0, 600.0));
  let rule = &keyframes[0];
  ...
  let (tx, ty, _tz, tw) = matrix.transform_point(0.0, 0.0, 0.0);
  assert!((tw - 1.0).abs() < 1e-6, "expected w≈1, got {tw}");
  assert!((tx - 128.85).abs() < 1e-3, "expected tx≈128.85, got {tx}");
  assert!((ty - 50.0).abs() < 1e-3, "expected ty≈50, got {ty}");

  let v = matrix.transform_direction(0.0, 1.0, 0.0);
  assert!(
    (v[0] - 0.5773503).abs() < 1e-3,
    "expected skew x≈0.577, got {:?}",
    v
  );
  assert!((v[1] - 1.0).abs() < 1e-3, "expected y≈1, got {:?}", v);
}
