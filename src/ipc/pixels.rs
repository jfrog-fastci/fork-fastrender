...
pub fn copy_packed_rgba_into_strided(
  ...
  let expected_src_len_u64 = u64::from(row_bytes_u32)
    ...
    .ok_or(IpcError::ArithmeticOverflow)?;
  let expected_src_len: usize = usize::try_from(expected_src_len_u64).map_err(|_| IpcError::ArithmeticOverflow)?;
  ...
  let required_dst_len_u64 = u64::from(dst_stride_bytes)
    .checked_mul(u64::from(height_px))
    .ok_or(IpcError::ArithmeticOverflow)?;
  let required_dst_len: usize = usize::try_from(required_dst_len_u64).map_err(|_| IpcError::ArithmeticOverflow)?;
  ...
  let row_bytes: usize = usize::try_from(row_bytes_u32).map_err(|_| IpcError::ArithmeticOverflow)?;
  let dst_stride: usize = usize::try_from(dst_stride_bytes).map_err(|_| IpcError::ArithmeticOverflow)?;
  let height: usize = usize::try_from(height_px).map_err(|_| IpcError::ArithmeticOverflow)?;
  ...
  for _ in 0..height {
    ...
    let src_row = src
      ...
      .ok_or_else(|| IpcError::ProtocolViolation {
        message: "src slice out of bounds".into(),
      })?;
    let dst_row = dst
      .get_mut(dst_offset..dst_end)
      .ok_or_else(|| IpcError::ProtocolViolation {
        message: "dst slice out of bounds".into(),
      })?;
