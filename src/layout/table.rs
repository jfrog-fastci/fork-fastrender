...
impl FormattingContext for TableFormattingContext {
  ...
  fn layout(
    ...
    // CSS 2.1 §17.5.2.1 (Fixed table layout): if the table has a definite used width and its
    // columns don't fill it, distribute the extra space over the columns.
    let table_width_is_definite = used_border_box_width.is_some() || specified_width.is_some();
    if table_width_is_definite
      && mode == DistributionMode::Fixed
      && available_content.is_finite()
      && !col_widths.is_empty()
    {
      fn distribute_extra_with_caps(
        col_widths: &mut [f32],
        constraints: &[ColumnConstraints],
        indices: &[usize],
        mut extra: f32,
      ) -> f32 {
        if extra <= 0.01 || indices.is_empty() {
          return extra;
        }
        ...
        let mut active: Vec<usize> = indices
          .iter()
          .copied()
          .filter(|idx| *idx < col_widths.len())
          .collect();

        // Iteratively distribute: columns that hit a finite max-cap drop out, and the remaining
        // extra is re-shared among the rest.
        let mut guard = 0usize;
        while extra > 0.01 && !active.is_empty() && guard < 64 {
          guard += 1;
          let share = extra / active.len() as f32;
          let mut consumed = 0.0;
          let mut next_active = Vec::with_capacity(active.len());

          for idx in active.into_iter() {
            let Some(width) = col_widths.get_mut(idx) else {
              continue;
            };
            let cap = constraints.get(idx).and_then(|c| {
              if c.has_max_cap && c.max_width.is_finite() {
                Some(c.max_width.max(c.min_width))
              } else {
                None
              }
            });
            let capacity = cap
              .map(|cap| (cap - *width).max(0.0))
              .unwrap_or(f32::INFINITY);
            if capacity <= 0.01 {
              continue;
            }

            let delta = if capacity.is_finite() {
              share.min(capacity)
            } else {
              share
            };
            *width += delta;
            ...
            if capacity.is_infinite() || (capacity - delta) > 0.01 {
      ...
      let mut current: f32 = col_widths.iter().sum();
      if available_content > current + 0.01 {
        let extra = available_content - current;
        // Prefer expanding "auto" columns (no fixed/percentage width). If there are none, expand
        // all columns (CSS 2.1 does not fully specify how to distribute in that case).
        let auto_indices: Vec<usize> = column_constraints
          .iter()
          .enumerate()
          .filter(|(_, c)| c.fixed_width.is_none() && c.percentage.is_none())
          .map(|(idx, _)| idx)
          .collect();

        let leftover = if auto_indices.is_empty() {
          let all: Vec<usize> = (0..col_widths.len()).collect();
          distribute_extra_with_caps(&mut col_widths, &column_constraints, &all, extra)
        } else {
          distribute_extra_with_caps(&mut col_widths, &column_constraints, &auto_indices, extra)
        };

        // If auto columns were capped (max-width) and couldn't absorb the full extra, spill the
        // remainder to other columns that can still grow.
        current = col_widths.iter().sum();
        if !auto_indices.is_empty() && leftover > 0.01 && available_content > current + 0.01 {
          let all: Vec<usize> = (0..col_widths.len()).collect();
          let extra = available_content - current;
          let _ = distribute_extra_with_caps(&mut col_widths, &column_constraints, &all, extra);
        }

        // If `leftover` remains here, every column hit a finite authored max-width cap; we keep
        // the caps and allow the table to have unused extra space.
      }
    }
