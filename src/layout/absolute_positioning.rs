...
impl AbsoluteLayout {
  ...
  pub fn layout_absolute(
    ...
    let natural_ratio = if input.is_replaced
      ...
      && input.intrinsic_size.width > 0.0
      && input.intrinsic_size.height > 0.0
    {
      Some(input.intrinsic_size.width / input.intrinsic_size.height)
    } else {
      None
    };
  ...
  fn compute_horizontal(
    ...
    let mut margin_left = if margin_left_auto {
      0.0
    } else {
      style.margin.left
    };
    let mut margin_right = if margin_right_auto {
      0.0
    } else {
      style.margin.right
    };
  ...
  fn compute_vertical(
    ...
    let mut margin_top = if margin_top_auto {
      0.0
    } else {
      style.margin.top
    };
    let mut margin_bottom = if margin_bottom_auto {
      0.0
    } else {
      style.margin.bottom
    };
...
pub(crate) fn resolve_positioned_style_with_anchors(
  ...
  resolved.font_style = match style.font_style {
    ...
    crate::style::types::FontStyle::Oblique(angle) => {
      crate::style::computed::FontStyle::Oblique(angle)
    }
  };
