...
fn table_fixed_layout_colgroup_width_applies_to_col_children() {
  ...
  assert!(
    gap_ab.abs() < 0.1,
    "expected cells A/B adjacent (gap={gap_ab})"
  );
  let gap_bc = c.x() - (b.x() + b.width());
  assert!(
    gap_bc.abs() < 0.1,
    "expected cells B/C adjacent (gap={gap_bc})"
  );
}
...
#[test]
fn table_fixed_layout_colgroup_width_applies_to_col_children_collapsed_border_model() {
  let html = r#"
    <html>
      <head>
        <style>
...
#[test]
fn table_fixed_layout_colgroup_width_applies_to_col_children_collapsed_border_model_rtl() {
  let html = r#"
    <html>
      <head>
        <style>
