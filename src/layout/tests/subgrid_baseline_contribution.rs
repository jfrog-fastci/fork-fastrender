...
#[test]
fn baseline_aligned_subgrid_items_contribute_to_parent_row_sizing() {
  ...
  let inner_large = make_text_item(21, 20.0, line_height, 1);
  let inner_small = make_text_item(22, 5.0, line_height, 2);
  ...
  assert!(
    ...
    "expected baseline alignment within the subgrid to increase the inherited row size (row={:.2}, max_item={:.2})",
    fragment.bounds.height(),
    max_item_height
  );
  ...
  assert!(
    ...
    "expected subgrid item baselines to align (a={baseline_a:.2}, b={baseline_b:.2})"
  );
}
