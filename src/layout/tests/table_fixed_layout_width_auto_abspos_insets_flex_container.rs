...
#[test]
fn table_layout_fixed_width_auto_abspos_insets_flex_container_uses_auto_layout() {
  // Regression test: an absolutely positioned table (`table-layout:fixed`, `width:auto`) that is
  // inset-stretched inside a positioned flex container should still use the *auto* table layout
  // algorithm. Flex's abspos relayout pass must not inject a synthetic `width:400px`, because that
  // flips the algorithm to fixed-layout.
  let html = r#"
    <html>
      <head>
        <style>
          body { margin: 0; }
          .flex { display:flex; position: relative; width: 400px; height: 120px; }
          td { padding: 0; border: 0; }
        </style>
      </head>
      <body>
        <div class="flex">
          <table style="position:absolute; left:0; right:0; top:0; table-layout: fixed; border-spacing:0; border:0">
            <tr>
  ...
  assert!(
    ...
    "expected inset-stretched table width ~400px, got {table_width}",
  );
  ...
  assert!(
    ...
    "expected wide second-row content to influence column width (auto layout fallback); max cell width {max_cell_width:.2}"
  );
}
