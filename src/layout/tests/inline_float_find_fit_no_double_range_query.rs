...
use crate::layout::inline::float_integration::{InlineFloatIntegration, InlineFloatIntegrationMut, LineSpaceOptions};
...
#[test]
fn inline_float_find_fit_in_containing_block_no_double_range_query() {
  let _lock = super::layout_profile_lock();
  ...
  with_thread_runtime_toggles(toggles, || {
    ...
    // Ensure `has_floats()` is true while leaving the queried range unconstrained.
    ctx.add_float_at(FloatSide::Left, 0.0, 10_000.0, 50.0, 10.0);
    ...
    for _ in 0..n {
      let space = integration.find_line_space_in_containing_block(0.0, 10.0, 200.0, opts);
      assert_eq!(space.y, 0.0);
      assert_eq!(space.left_edge, 10.0);
      assert_eq!(space.width, 200.0);
    }
    ...
    assert_eq!(
      stats.range_queries, n,
      "expected exactly one float range query per find_line_space_in_containing_block() call"
    );

    // Also cover the mutable integration variant which shares the same call-site logic.
    let mut ctx = FloatContext::new(500.0);
    ctx.add_float_at(FloatSide::Left, 0.0, 10_000.0, 50.0, 10.0);
    let integration = InlineFloatIntegrationMut::new(&mut ctx);
    ...
    assert_eq!(
      stats.range_queries, n,
      "expected exactly one float range query per InlineFloatIntegrationMut::find_line_space_in_containing_block() call"
    );
  });
}
