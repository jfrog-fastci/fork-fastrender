...
#[test]
fn table_cell_offsets_and_spans_use_cached_metrics_rtl() {
  let html = r#"
    <html>
      <head>
        <style>
          body { margin: 0; }
          table { border-collapse: separate; border-spacing: 10px 6px; table-layout: fixed; direction: rtl; }
          col.col1 { width: 40px; }
          col.col2 { width: 60px; }
          col.col3 { width: 50px; }
          td { padding: 0; margin: 0; border: 0; font-size: 12px; line-height: 12px; vertical-align: baseline; }
  ...
  assert!(
    (spacing_x1 - 10.0).abs() < 0.1 && (spacing_x1 - spacing_x2).abs() < 0.05,
    "horizontal spacing should match border-spacing in RTL (got {spacing_x1} and {spacing_x2})"
  );
