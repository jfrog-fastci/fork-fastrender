...
impl InlineFormattingContext {
  ...
  fn collect_inline_flow_segments_with_base_internal(
    ...
    let combinable_indices: Vec<usize> = box_node
      ...
      .filter_map(|(i, c)| {
        ...
        {
          if leading_edge_combinability(c, c.style.text_combine_upright)
  ...
  fn collect_inline_items_internal(
    ...
    whitespace: &mut CollapsibleWhitespaceState,
    base_direction: crate::style::types::Direction,
    float_writing_mode: WritingMode,
      ...
      match &child.box_type {
        ...
        BoxType::Inline(_) => {
          ...
          if has_floats {
            ...
            // inline stream. Ensure floated children nested inside inline boxes are surfaced at the
            // top level so the float context can place them.
            enum Piece {
              Segment(Vec<InlineItem>),
              Float(InlineItem),
            }
            ...
            if first_segment.is_none()
              && !(start_edge == 0.0
                ...
                && !establishes_abs_cb)
            {
              // There is no in-flow content after hoisting floats, but the inline box still has
              // edges or establishes a containing block; emit an empty inline fragment to preserve
              // those effects.
              first_segment = Some(0);
              last_segment = Some(0);
              emit_empty_segment = true;
  ...
  fn create_inline_items_from_normalized_with_base(
      ...
      .enumerate()
    {
      ...
      if seg_index < hard_offsets.len() {
        items.push(InlineItem::HardBreak(ClearSide::None));
      }
      start = end;
    }
  ...
  fn create_text_items_with_combine(
    ...
    while idx < chars.len() {
      ...
      if combinable_start {
        ...
        while end < chars.len() {
          ...
          if forced_set.contains(&next_off)
            || !can_combine_for_mode(next_ch, style.text_combine_upright)
          {
            break;
          }
          end += 1;
      ...
      while end < chars.len() {
        ...
        if forced_set.contains(&next_off)
          || can_combine_for_mode(chars[end].1, style.text_combine_upright)
        {
          break;
        }
        end += 1;
  ...
  fn append_inline_items_with_whitespace(
    ...
    for item in produced {
      match item {
        InlineItem::HardBreak(clear) => {
          whitespace.reset();
          items.push(InlineItem::HardBreak(clear));
        }
  ...
  fn inline_items_start_with_hard_break(items: &[InlineItem]) -> bool {
    for item in items {
      match item {
        ...
        InlineItem::HardBreak(_) => return true,
        InlineItem::InlineBox(b) => {
  ...
  fn create_tab_item(
    ...
    let tab_interval = match style.tab_size {
      ...
      TabSize::Length(len) => resolve_length_with_percentage_inline(
        len,
        None,
        style,
        &self.font_context,
    ...
    for (i, positioned) in items.iter().enumerate() {
      ...
      if rtl {
        let marker_gap = match &positioned.item {
          InlineItem::Text(t) if t.is_marker => {
            (item_width - (t.advance + t.paint_offset.abs())).max(0.0)
          }
    ...
    match item {
      ...
      InlineItem::InlineBox(box_item) => {
        ...
        >| {
          ...
          // resolve against the correct axes.
          let (cb_physical_width, cb_physical_height) =
            if crate::style::block_axis_is_horizontal(style.writing_mode) {
              (padding_rect.size.height, padding_rect.size.width)
            } else {
        ...
        if let Some(id) = box_id {
            ...
            || replaced_item.style.establishes_fixed_containing_block()
          {
            if let Some(map) = positioned_containing_blocks.as_deref_mut() {
              let style = replaced_item.style.as_ref();
              ...
              let border_for = |side: crate::style::PhysicalSide| -> f32 {
                match side {
                  crate::style::PhysicalSide::Left => border_left,
                  crate::style::PhysicalSide::Right => border_right,
                  crate::style::PhysicalSide::Top => border_top,
                  crate::style::PhysicalSide::Bottom => border_bottom,
                }
              };
              let border_inline_start = border_for(inline_start_side);
              let border_inline_end = border_for(inline_end_side);
              ...
              let padding_rect = Rect::from_xywh(
                bounds.x() + border_inline_start,
                bounds.y() + border_block_start,
                (bounds.width() - border_inline_start - border_inline_end).max(0.0),
                (bounds.height() - border_block_start - border_block_end).max(0.0),
              );
              let (cb_physical_width, cb_physical_height) =
                if crate::style::block_axis_is_horizontal(style.writing_mode) {
                  (padding_rect.size.height, padding_rect.size.width)
...
fn resolve_inline_axis_margins(
  ...
  let resolve_margin_side = |side: crate::style::PhysicalSide| -> f32 {
    ...
    raw
      .as_ref()
      .map(|l| resolve_length_for_width(*l, percentage_base, style, font_context, viewport, None))
      .unwrap_or(0.0)
  };
...
fn grapheme_boundary_breaks(
  ...
  for (byte_idx, _) in text.grapheme_indices(true).skip(1) {
    breaks.push(BreakOpportunity::allowed_with_kind(byte_idx, kind));
  }
  breaks.push(BreakOpportunity::allowed_with_kind(text.len(), kind));
  breaks
}
...
fn apply_break_properties(
  ...
  if has_duplicates {
    result.dedup_by(|a, b| {
      ...
      let is_mandatory = matches!(a.break_type, BreakType::Mandatory)
        || matches!(b.break_type, BreakType::Mandatory);
      if is_mandatory {
        b.break_type = BreakType::Mandatory;
        b.kind = BreakOpportunityKind::Normal;
      } else if matches!(a.kind, BreakOpportunityKind::Normal)
        || matches!(b.kind, BreakOpportunityKind::Normal)
      {
        b.kind = BreakOpportunityKind::Normal;
      } else {
        b.kind = BreakOpportunityKind::Emergency;
      }
...
impl InlineFormattingContext {
  ...
  fn apply_text_overflow(
    ...
    for line in lines {
      ...
      let start_side = style
        .text_overflow
        .start_for_direction(line.resolved_direction);
      let end_side = style
        .text_overflow
  ...
  pub fn layout_with_floats(
    ...
    if dump_text_enabled() {
      ...
      eprintln!(
        "ifc start display={:?} writing_mode={:?} font_size={:.2} children={} text_desc={} available_inline={:.2} available_block={:.2}",
        box_node.style.display,
        box_node.style.writing_mode,
    ...
    if inline_vertical {
      if let Some(used_border_box) = constraints.used_border_box_height.filter(|h| h.is_finite()) {
      ...
      for seg in &segments {
        match seg {
          InlineFlowSegment::InlineItems(items) => {
            ...
            text_items += items
              .iter()
              .filter(|i| matches!(i, InlineItem::Text(_)))
              .count();
          }
            ...
            InlineFlowSegment::InlineItems(items) => {
              ...
              for item in items.iter().take(10) {
                let label = match item {
                  ...
                  InlineItem::HardBreak(_) => "HardBreak".to_string(),
    ...
    // Track the maximum inline-axis extent contributed by block-level segments so the formatting
    // context can compute an intrinsic inline size when the available inline space is indefinite.
    let mut block_max_inline: f32 = 0.0;
    let mut deadline_counter = 0usize;
    ...
    let flush_pending = |pending: &mut Vec<InlineItem>,
     ...
     -> Result<Option<(f32, f32, f32)>, LayoutError> {
      ...
      let ends_with_hard_break = seg_lines
        .last()
        .is_some_and(|line| line.ends_with_hard_break);
      let mut seg_height = seg_lines
        .iter()
        .map(|l| l.y_offset + l.height)
        .fold(0.0, f32::max);
      // `LineBuilder` can apply float clearance after a hard break (legacy `<br clear=...>`).
      // When no line box is emitted at the cleared Y position (e.g. the clearing `<br>` is the last
      // inline content before a block-level sibling), we still need to advance by the cleared
      // offset so following flow content starts below floats.
      seg_height = seg_height.max(max_clear_offset);
      // `LineBuilder` does not emit an empty line box for the post-break cursor position when the
      // inline segment ends immediately after a hard break (`<br>`). Append a zero-height line at
      // the cursor position so:
      // - block layout observes the cleared offset (legacy `<br clear>`), and
      // - fragment trees preserve the expected extra empty line before a block sibling.
      if ends_with_hard_break {
        let mut cursor_line = Line::new();
        cursor_line.y_offset = seg_height;
        seg_lines.push(cursor_line);
      }
    ...
    for segment in segments {
      ...
      match segment {
        ...
        InlineFlowSegment::Block(block_node_ref) => {
          ...
          let percentage_base = available_inline.max(0.0);
          let inline_positive =
            crate::style::inline_axis_positive(style.writing_mode, style.direction);
          let block_positive = crate::style::block_axis_positive(style.writing_mode);
          let (inline_start_side, inline_end_side) =
          ...
          let resolve_margin_side = |side: crate::style::PhysicalSide| -> f32 {
            let raw = match side {
              crate::style::PhysicalSide::Left => block_node.style.margin_left,
              crate::style::PhysicalSide::Right => block_node.style.margin_right,
              crate::style::PhysicalSide::Top => block_node.style.margin_top,
              crate::style::PhysicalSide::Bottom => block_node.style.margin_bottom,
            };
            raw
              .as_ref()
              .map(|m| {
          ...
          let _margin_inline_start = resolve_margin_side(inline_start_side);
          let margin_inline_end = resolve_margin_side(inline_end_side);
          let margin_block_start = resolve_margin_side(block_start_side);
          let margin_block_end = resolve_margin_side(block_end_side);
          ...
          let fc_type = block_node
            ...
            .unwrap_or(FormattingContextType::Block);
          let fc = self.factory.get(fc_type);
          let child_constraints = if inline_vertical {
          ...
          translated = crate::layout::contexts::block::unconvert_fragment_axes_root(translated);
          let translate_y = line_offset + margin_block_start - translated.bounds.y();
          translated.translate_root_in_place(Point::new(0.0, translate_y));
          block_max_inline = block_max_inline
            .max(translated.bounds.x() + translated.bounds.width() + margin_inline_end);
          line_offset += margin_block_start + translated.bounds.height() + margin_block_end;
    ...
    let blocks_bottom = block_fragments
      ...
      .fold(0.0, f32::max);
    // `line_offset` tracks the cursor position as we lay out inline and block segments. Prefer it
    // when it exceeds the max fragment bounds so we account for float clearance that doesn't
    // materialize as a line box (e.g. terminal `<br clear>` used as a clearfix).
    let flow_height = total_height_lines.max(blocks_bottom).max(line_offset);
    ...
    let inline_extent = if available_inline.is_finite() {
      ...
      line_max_inline.max(float_max_inline).max(block_max_inline)
    };
    let block_extent = total_height;
    ...
    // Create containing fragment bounds in the formatting context's logical coordinate system
    // (x = inline axis, y = block axis) regardless of writing mode.
    let mut bounds = Rect::from_xywh(0.0, 0.0, inline_extent, block_extent);

    // `LayoutConstraints::used_border_box_{width,height}` are expressed in physical axes. Map them
    // into logical inline/block sizes so the fragment tree stays in logical coordinates.
    let used_inline_size = if inline_vertical {
      constraints.used_border_box_height
    } else {
      constraints.used_border_box_width
    };
    let used_block_size = if inline_vertical {
      constraints.used_border_box_width
    } else {
      constraints.used_border_box_height
    };
    if let Some(used_inline) = used_inline_size {
      if used_inline.is_finite() {
        bounds.size.width = used_inline.max(0.0);
      }
    }
    if let Some(used_block) = used_block_size {
      if used_block.is_finite() {
        bounds.size.height = used_block.max(0.0);
      }
    }
    ...
    if !positioned_children.is_empty() {
      ...
      let mut anchor_index_physical: Option<crate::layout::anchor_positioning::AnchorIndex> = None;
      if crate::style::block_axis_is_horizontal(style.writing_mode) {
        let style_arc = style_override
          .as_ref()
          .cloned()
          .unwrap_or_else(|| box_node.style.clone());
        let logical_root = FragmentNode::new_with_style(
          Rect::new(Point::ZERO, bounds.size),
          FragmentContent::Block {
            box_id: Some(root_box_id),
          },
        ...
        physical_index.insert_names_for_box(
          ...
          &style.anchor_names,
          crate::layout::anchor_positioning::AnchorBox {
            rect: Rect::new(Point::ZERO, physical_root.bounds.size),
            writing_mode: style.writing_mode,
            direction: style.direction,
          },
        );
      ...
      // Convert physical padding/border edges into this formatting context's logical axes. The
      // fragment tree produced by the inline formatting context uses logical coordinates
      // (x = inline axis, y = block axis), so containing blocks and static positions need to be
      ...
      let padding_for = |side: crate::style::PhysicalSide| -> f32 {
        match side {
          ...
          crate::style::PhysicalSide::Right => padding_right,
          crate::style::PhysicalSide::Top => padding_top,
          crate::style::PhysicalSide::Bottom => padding_bottom,
        }
      };
      let border_for = |side: crate::style::PhysicalSide| -> f32 {
        match side {
      ...
      let padding_inline_start = padding_for(inline_start_side);
      let padding_inline_end = padding_for(inline_end_side);
      let padding_block_start = padding_for(block_start_side);
      let padding_block_end = padding_for(block_end_side);
      let border_inline_start = border_for(inline_start_side);
      let border_inline_end = border_for(inline_end_side);
      let border_block_start = border_for(block_start_side);
      let border_block_end = border_for(block_end_side);
      ...
      let (padding_origin, padding_size) = if root_inline_container {
        (
          Point::new(-padding_inline_start, -padding_block_start),
          Size::new(
            (bounds.width() + padding_inline_start + padding_inline_end).max(0.0),
            (bounds.height() + padding_block_start + padding_block_end).max(0.0),
          ),
        )
      } else {
        (
          Point::new(border_inline_start, border_block_start),
          Size::new(
            (bounds.width() - border_inline_start - border_inline_end).max(0.0),
            (bounds.height() - border_block_start - border_block_end).max(0.0),
          ),
      ...
      let layout_positioned_child = |positioned_child: &PositionedChild| {
        ...
        // Absolute positioning logic (inset properties, anchors, etc.) works in physical
        // coordinates. Inline layout stores fragments (and thus containing blocks/static positions)
        ...
        let (anchors_for_cb, positioning_cb, static_position_for_abs) = if needs_physical_conversion
        {
          ...
          let physical_cb = ContainingBlock::with_viewport_and_bases(
            cb_rect_physical,
            viewport_size,
        ...
        let child_constraints = LayoutConstraints::new(
          AvailableSpace::Definite(positioning_cb.rect.size.width),
          positioning_cb
        ...
        if needs_relayout {
          ...
          if supports_used_border_box && width_auto && height_auto {
            ...
            let mut relayout_style = layout_child.style.clone();
            {
              ...
              s.width = Some(Length::px(border_size_physical.width));
              s.height = Some(Length::px(border_size_physical.height));
              s.width_keyword = None;
              s.height_keyword = None;
            }
    ...
    if root_inline_container {
      ...
      // Map physical padding/border edges to this formatting context's logical axes.
      let inline_positive = crate::style::inline_axis_positive(style.writing_mode, style.direction);
      let block_positive = crate::style::block_axis_positive(style.writing_mode);
      let (inline_start_side, inline_end_side) =
        if crate::style::inline_axis_is_horizontal(style.writing_mode) {
      ...
      let offset = Point::new(
        padding_inline_start + border_inline_start,
        padding_block_start + border_block_start,
      );
      ...
      // `used_border_box_{width,height}` are expressed in physical axes; map them into logical
      // inline/block sizes before deciding whether to expand the root bounds.
      let used_inline_size = if inline_vertical {
        constraints.used_border_box_height
      } else {
        constraints.used_border_box_width
  ...
  match &node.box_type {
    ...
    BoxType::Marker(marker_box) => {
      if let MarkerContent::Text(text) = &marker_box.content {
        let normalized =
          normalize_text_for_white_space(text, node.style.white_space, node.style.text_wrap);
        let mut saw_whitespace = match edge {
          CombineEdge::Leading => normalized.leading_collapsible,
          CombineEdge::Trailing => normalized.trailing_collapsible,
        };
...
fn marker_inline_end_margin(style: &ComputedStyle) -> Option<Length> {
  use crate::style::types::WritingMode::HorizontalTb;
  use crate::style::types::WritingMode::SidewaysLr;
  use crate::style::types::WritingMode::SidewaysRl;
  use crate::style::types::WritingMode::VerticalLr;
  use crate::style::types::WritingMode::VerticalRl;
...
fn determine_paragraph_direction(items: &[InlineItem]) -> Option<crate::style::types::Direction> {
  ...
  fn collect_text(item: &InlineItem, out: &mut String) {
    match item {
      ...
      InlineItem::HardBreak(_) => out.push('\n'),
...
pub(crate) fn push_embedding_level(
...
pub(crate) fn explicit_bidi_context(
  ...
  if !changed && !override_all {
    ...
    Some(crate::text::pipeline::ExplicitBidiContext {
      level,
      override_all,
    })
  }
...
#[allow(dead_code)]
pub(crate) fn apply_plaintext_paragraph_direction(
  items: &mut [InlineItem],
  direction: crate::style::types::Direction,
) {
  for item in items {
...
fn has_justify_opportunities(items: &[PositionedItem], mode: TextJustify) -> bool {
  ...
  let non_marker: Vec<&PositionedItem> =
...
mod tests {
  ...
  use crate::style::types::TextCombineUpright;
  use crate::style::types::TextJustify;
  use crate::style::types::TextOverflow;
  ...
  fn inline_respects_style_override_for_root() {
    ...
    let fragment =
      crate::layout::style_override::with_style_override(root.id, Arc::new(override_style), || {
  ...
  fn inline_block_width_max_content_uses_rebased_intrinsic_border_box_width() {
    ...
    let (intrinsic_min_base0, intrinsic_max_base0) = fc
      ...
      .expect("intrinsic sizes");
    let edges_base0 =
      horizontal_padding_and_borders(&base_style, 0.0, ifc.viewport_size, &ifc.font_context, None);
  ...
  fn text_overflow_with_outside_marker_keeps_ellipsis_in_content() {
    ...
    assert!(
      ellipsis_x >= 0.0,
      "ellipsis should appear within the content region"
    );
  }
  ...
  fn marker_outside_skips_intercharacter_justify() {
    ...
    assert!(
      marker_x < -5.0,
      "marker should stay outside; marker_x={}",
      marker_x
    );
  }
  ...
  fn text_combine_whitespace_at_boundaries_allows_internal_combination() {
    ...
    let boundary_second =
      compute_combine_boundary(&root.children, 1, TextCombineUpright::Digits(2));
    assert!(
  ...
  fn marker_image_inside_consumes_inline_space() {
    ...
    assert!(
      x > 17.0 && x < 19.5,
      "expected text after marker gap, got {}",
      x
    );
  }
  ...
  #[test]
  fn marker_outside_positions_inline_start_in_vertical_lr_rtl_direction() {
    let ifc = InlineFormattingContext::new();
    let mut root_style = ComputedStyle::default();
    root_style.writing_mode = WritingMode::VerticalLr;
    root_style.direction = crate::style::types::Direction::Rtl;
    ...
    assert!(
      marker_y > text_y,
      "rtl inline-start should map to inline-end (bottom) in vertical-lr; marker_y={}, text_y={}",
      marker_y,
      text_y
    );
  }
  ...
  fn marker_gap_uses_inline_end_margin_in_vertical_ltr() {
    ...
    let gap = marker_inline_gap(&style, &FontContext::new(), Size::new(800.0, 600.0), None);
    assert!(
  ...
  #[test]
  fn distribute_justify_rtl_fills_line_box_and_does_not_explode_fragments() {
    let mut root_style = ComputedStyle::default();
    ...
    assert!(
      count <= 4,
      "RTL distribute justify should not split long CJK into per-cluster fragments; count={count}"
    );
    ...
    assert!(
      (span - first_line.bounds.width()).abs() < 1.0,
      "RTL justified line should fill its line box (span={span:.2} box={:.2})",
      first_line.bounds.width()
    );
    assert!(
      (min_x - first_line.bounds.min_x()).abs() < 5.0,
      "RTL justified line should align to line box start (min_x={min_x:.2} line_min={:.2})",
      first_line.bounds.min_x()
    );
    assert!(
      (max_x - first_line.bounds.max_x()).abs() < 5.0,
      "RTL justified line should align to line box end (max_x={max_x:.2} line_max={:.2})",
      first_line.bounds.max_x()
    );
  }
  ...
  fn white_space_pre_preserves_tabs_as_items() {
    ...
    let ifc = InlineFormattingContext::new();
    let items = ifc
      .create_inline_items_for_text(&node, "a\tb", false)
      .unwrap();
  ...
  #[test]
  fn white_space_normal_trims_breaking_space() {
    let ifc = InlineFormattingContext::new();
    let mut style = ComputedStyle::default();
    style.white_space = WhiteSpace::Normal;
    let style = Arc::new(style);
  ...
  #[test]
  fn white_space_break_spaces_preserves_trailing_space() {
    let ifc = InlineFormattingContext::new();
    let mut style = ComputedStyle::default();
  ...
  #[test]
  fn white_space_normal_trims_breaking_space_across_text_nodes() {
    let ifc = InlineFormattingContext::new();
    let mut style = ComputedStyle::default();
    style.white_space = WhiteSpace::Normal;
    let style = Arc::new(style);
  ...
  fn pre_treats_form_feed_as_break() {
    ...
    style.white_space = WhiteSpace::Pre;
    let normalized =
      normalize_text_for_white_space("a\u{000C}b", style.white_space, style.text_wrap);
    assert_eq!(normalized.text, "ab");
    assert_eq!(normalized.forced_breaks.len(), 1);
    assert_eq!(normalized.forced_breaks[0].byte_offset, 1);
    assert!(!normalized.allow_soft_wrap);
  ...
  fn text_overflow_does_not_apply_when_inline_overflow_is_visible_in_vertical_writing_mode() {
    ...
    let root = BoxNode::new_block(
      ...
      vec![BoxNode::new_text(
        Arc::new(text_style),
        "vertical text that overflows but should not ellipsize when inline overflow is visible"
          .to_string(),
      )],
    );
  ...
  fn unicode_bidi_plaintext_uses_per_paragraph_base_direction() {
    ...
    assert!(
      inline_items
        .iter()
        .any(|item| matches!(item, InlineItem::HardBreak(_))),
  ...
  #[test]
  fn apply_break_properties_marks_emergency_breaks_for_break_word() {
    use crate::text::line_break::BreakOpportunityKind;
    ...
    assert_eq!(
      ...
      "UAX breaks should remain normal even when emergency breaks are present"
    );

    let emergency_break = result
      .iter()
      .find(|b| b.byte_offset == 7)
      .expect("emergency break after 'w'");
    assert_eq!(
      emergency_break.kind,
      BreakOpportunityKind::Emergency,
      "added breaks for break-word should be emergency-only"
    );
  }
  ...
  fn text_wrap_nowrap_keeps_mandatory_breaks() {
    ...
    assert!(
      fragment.children.len() > 1,
      "mandatory breaks should still split lines"
    );
  }
  ...
  #[test]
  fn overflow_wrap_anywhere_reduces_min_content_width() {
    let mut text_style = ComputedStyle::default();
    ...
    assert!(
      breaking_min < normal_min * 0.75,
      "overflow-wrap:anywhere should shrink min-content width by considering emergency breaks"
    );
  }
  ...
  fn normal_collapses_vertical_tab_to_space() {
    ...
    style.white_space = WhiteSpace::Normal;
    let normalized =
      normalize_text_for_white_space("a\u{000B} b", style.white_space, style.text_wrap);
  ...
  fn pre_line_treats_unicode_separators_as_breaks() {
    ...
    assert_eq!(normalized.text, "abcd");
    let offsets: Vec<usize> = normalized
      .forced_breaks
      .iter()
      .map(|b| b.byte_offset)
      .collect();
  ...
  fn tabs_advance_to_next_stop() {
    ...
    let ifc = InlineFormattingContext::new();
    let mut items = ifc
      .create_inline_items_for_text(&node, "ab\tc", false)
      .unwrap();
    let strut = ifc.compute_strut_metrics(&node.style);
    let lines = ifc
      .build_lines(
  ...
  fn text_wrap_balance_uses_subsequent_width_on_first_line() {
    ...
    let items = ifc
      .create_inline_items_for_text(&node, "hello world", false)
      .unwrap();
  ...
  fn text_align_last_auto_uses_start_on_final_justify_line() {
    ...
    assert!(
      child.bounds.x() < 1.0,
      "last line should start-align under auto"
    );
  }
  ...
  fn text_align_justify_single_line_respects_auto_last_line() {
    ...
    let trailing_gap =
      line.bounds.x() + line.bounds.width() - (last.bounds.x() + last.bounds.width());
    assert!(
            trailing_gap > 40.0,
  ...
  fn text_indent_hanging_skips_first_line() {
    ...
    assert!(
      first_x < 1.0,
      "first line should not be indented under hanging"
    );
    assert!(
      second_x >= 9.0,
      "subsequent lines should be indented under hanging"
    );
  }
  ...
  fn text_indent_hanging_indents_soft_wrapped_lines() {
    ...
    assert!(
      fragment.children.len() >= 2,
      "soft wrap should create multiple lines"
    );
    let first_x = fragment.children[0].children[0].bounds.x();
    let second_x = fragment.children[1].children[0].bounds.x();
    assert!(
      first_x < 1.0,
      "first line should remain unindented under hanging"
    );
    assert!(
      second_x >= 9.0,
      "soft-wrapped lines should receive hanging indent"
    );
  }
  ...
  #[test]
  fn min_content_excludes_trailing_collapsible_space_at_soft_wrap_break() {
    let ifc = InlineFormattingContext::new();
    let mut style = ComputedStyle::default();
    style.font_size = 16.0;
    style.white_space = WhiteSpace::Normal;

    let word1 = "Commonwealth";
    let word2 = "is";
    let text = format!("{word1} {word2}");

    let node = BoxNode::new_text(Arc::new(style.clone()), text);
    ...
    let w1 = ifc
      .create_text_item(
        &BoxNode::new_text(Arc::new(style.clone()), word1.to_string()),
        word1,
      )
      .unwrap()
      .advance;
    let w2 = ifc
      .create_text_item(
        &BoxNode::new_text(Arc::new(style.clone()), word2.to_string()),
        word2,
      )
      .unwrap()
      .advance;
  ...
  fn font_relative_lengths_use_metrics_and_adjustments() {
    ...
    let Some(x_height) = scaled.x_height else {
      return;
    };
    let Some(face) = font.as_ttf_face().ok() else {
      return;
    };
    let Some(advance) = face
      .glyph_index('0')
