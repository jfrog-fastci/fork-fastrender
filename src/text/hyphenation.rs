...
static PATTERN_CACHE: OnceLock<Mutex<HashMap<SupportedLanguage, Arc<HyphenationPatterns>>>> =
  OnceLock::new();
...
impl HyphenationPatterns {
  ...
  pub fn needs_special_handling(&self) -> bool {
    ...
    matches!(
      self.language,
      SupportedLanguage::German | SupportedLanguage::Dutch
    )
  }
}
...
impl Hyphenator {
  ...
  fn filter_break_points(&self, word: &str, mut breaks: Vec<usize>) -> Vec<usize> {
    ...
    // Filter breaks
    breaks
      .retain(|&pos| pos >= left_boundary && pos <= right_boundary && word.is_char_boundary(pos));
...
mod tests {
  ...
  fn filter_break_points_rejects_non_char_boundaries() {
    ...
    assert!(
      !filtered.contains(&3),
      "non-char-boundary break should be dropped"
    );
