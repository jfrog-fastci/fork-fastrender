...
use crate::tree::fragment_tree::{FragmentContent, FragmentNode, TextEmphasisOffset};
use crate::{FastRender, Point, RenderArtifactRequest, RenderArtifacts, RenderOptions};
...
#[test]
fn text_emphasis_marks_render_outside_ruby_annotations_in_vertical_lr_writing() {
  let html = r#"<!doctype html><html><head><style>
    body{margin:0;font-size:40px;writing-mode:vertical-lr;}
    ruby{ruby-position:over;}
    /* Inflate leading so the ruby/emphasis interaction must account for it. */
    span{line-height:3;text-emphasis-style:dot;text-emphasis-position:over left;}
  ...
  let block_start_on_right = false; // vertical-lr => block-start is physical left.
  let ruby_on_block_start = ruby_on_right == block_start_on_right;
  ...
  if ruby_on_block_start {
    assert!(
      base_offset.over > 0.0,
      "expected non-zero block-start emphasis offset for ruby interaction, got {base_offset:?}"
    );
  } else {
    assert!(
      base_offset.under > 0.0,
      "expected non-zero block-end emphasis offset for ruby interaction, got {base_offset:?}"
    );
  }
}
