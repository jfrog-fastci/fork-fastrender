  ...
  let monochrome_emoji_id = db
    ...
    .find(|face| {
      face
        .families
        .iter()
        .any(|(name, _)| name.eq_ignore_ascii_case("Noto Sans Emoji 2"))
    })
    .map(|face| face.id)
    .expect("fixture monochrome emoji face should be discoverable via fontdb metadata");

  assert_eq!(
    db.is_color_capable_font(monochrome_emoji_id),
    Some(false),
    "fixture monochrome emoji font should have no color tables"
  );
