...
use crate::css::types::{CssRule, FontFeatureValueType};
use crate::dom;
...
#[test]
fn font_variant_alternates_ornaments_named_multi_value_definition_is_ignored() {
  let (font_context, family) = load_fixture_font_context();

  let dom = dom::parse_html(r#"<div id="t">A</div>"#).expect("parse html");
  let css = format!(
    r#"
      @font-feature-values "{family}" {{ @ornaments {{ bad: 3 5; }} }}
      #t {{
        font-family: "{family}";
        font-size: 16px;
        font-variant-alternates: ornaments(bad);
      }}
    "#
  );
  let stylesheet = parse_stylesheet(&css).expect("stylesheet");
  ...
  assert!(
    seen.get(b"ornm").is_none(),
    "@ornaments definitions with multiple integers must be ignored (no ornm feature)"
  );
}
