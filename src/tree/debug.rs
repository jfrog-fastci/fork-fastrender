...
mod tests {
  ...
  fn test_with_dom_path() {
    let info =
      DebugInfo::new(Some("div".to_string()), None, vec![]).with_dom_path("html > body > div");
  ...
  fn test_tree_printer_single_node() {
    ...
    let node = BoxNode::new_block(style, FormattingContextType::Block, vec![]).with_debug_info(
      DebugInfo::new(Some("div".to_string()), Some("main".to_string()), vec![]),
    );
  ...
  fn test_tree_printer_with_children() {
    ...
    let style = Arc::new(ComputedStyle::default());
    let child =
      BoxNode::new_block(style.clone(), FormattingContextType::Block, vec![]).with_debug_info(
        DebugInfo::new(Some("div".to_string()), Some("child".to_string()), vec![]),
      );

    let parent =
      BoxNode::new_block(style, FormattingContextType::Block, vec![child]).with_debug_info(
