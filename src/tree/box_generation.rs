...
mod tests {
  ...
  fn serialized_inline_svg(
    html: &str,
    width: f32,
    height: f32,
  ) -> Option<crate::tree::box_tree::SvgContent> {
    let dom = dom::parse_html(html).ok()?;
    let stylesheet = crate::css::parser::extract_css(&dom).ok()?;
    let media = crate::style::media::MediaContext::screen(width, height);
    let styled = crate::style::cascade::apply_styles_with_media(&dom, &stylesheet, &media);
    let box_tree = generate_box_tree_result(&styled).ok()?;

    fn find_svg(node: &BoxNode) -> Option<crate::tree::box_tree::SvgContent> {
      if let crate::tree::box_tree::BoxType::Replaced(repl) = &node.box_type {
  ...
  fn svg_document_css_forced_on_injects_even_without_class_or_id() {
    ...
    let content = crate::debug::runtime::with_runtime_toggles(
  ...
  fn svg_document_css_embedding_policy_respects_svg_count_overrides_and_size_limit() {
    ...
    let disabled = crate::debug::runtime::with_runtime_toggles(
      Arc::new(crate::debug::runtime::RuntimeToggles::from_map(
        std::collections::HashMap::from([(
    ...
    let forced_on = crate::debug::runtime::with_runtime_toggles(
      Arc::new(crate::debug::runtime::RuntimeToggles::from_map(
        std::collections::HashMap::from([
    ...
    "#;
    let forced_off = crate::debug::runtime::with_runtime_toggles(
      Arc::new(crate::debug::runtime::RuntimeToggles::from_map(
        std::collections::HashMap::from([(
          "FASTR_SVG_EMBED_DOCUMENT_CSS".to_string(),
          "0".to_string(),
    ...
    let forced_on_oversized = crate::debug::runtime::with_runtime_toggles(
      Arc::new(crate::debug::runtime::RuntimeToggles::from_map(
        std::collections::HashMap::from([(
          "FASTR_SVG_EMBED_DOCUMENT_CSS".to_string(),
          "1".to_string(),
