...
pub(crate) fn current_git_sha() -> Option<String> {
  ...
  //
  // Important: env vars are checked on *every* call (even if we've previously cached a `git`
  // result). This keeps unit/integration tests deterministic when they temporarily override
  // `FASTR_GIT_SHA`/`GITHUB_SHA` in a process that may have already called `current_git_sha()`.
...
fn url_hint_from_cache_path(cache_path: &Path) -> String {
  cached_url_from_cache_meta(cache_path)
    .unwrap_or_else(|| common::file_url::file_url_for_path(cache_path))
}
...
fn render_worker(args: WorkerArgs) -> io::Result<()> {
  ...
  if progress.hotspot.trim().is_empty() {
    let stats = progress.diagnostics.as_ref().and_then(|d| d.stats.as_ref());
    progress.hotspot = infer_hotspot(stats, &progress.stages_ms).to_string();
  }
...
mod progress_tests {
  ...
  fn timeout_records_stage_and_hotspot() {
    ...
    assert_eq!(value["status"], serde_json::json!("timeout"));
    assert_eq!(
      value["hotspot"],
      serde_json::json!(hotspot_from_timeout_stage(RenderStage::Cascade))
    );
    assert_eq!(value["timeout_stage"], serde_json::json!("cascade"));
  }
  ...
  fn diagnostics_failure_stage_is_propagated() {
    ...
    let value = serde_json::to_value(&progress).expect("serialize progress");
    assert_eq!(value["failure_stage"], serde_json::json!("paint"));
  }
  ...
  fn heartbeat_stage_maps_to_timeout_stage() {
    ...
    assert_eq!(value["timeout_stage"], serde_json::json!("dom_parse"));
  }
  ...
  fn hard_timeout_prefers_heartbeat_stage() {
    ...
    assert_eq!(
      progress.hotspot,
      hotspot_from_heartbeat(heartbeat_stage).to_string()
    );
    assert!(
