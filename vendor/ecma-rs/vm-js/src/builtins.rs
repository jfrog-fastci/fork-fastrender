...
///
/// This is currently minimal: it supports creation/revocation and the spec `IsArray` interaction,
/// but does not implement Proxy trap semantics.
pub fn proxy_constructor_call(
  _vm: &mut Vm,
  _scope: &mut Scope<'_>,
  _host: &mut dyn VmHost,
...
pub fn proxy_revocable(
  ...
  scope.define_property(out, proxy_key, data_desc(Value::Object(proxy), true, true, true))?;
  let revoke_key = PropertyKey::from_string(scope.alloc_string("revoke")?);
  scope.define_property(out, revoke_key, data_desc(Value::Object(revoke_fn), true, true, true))?;
