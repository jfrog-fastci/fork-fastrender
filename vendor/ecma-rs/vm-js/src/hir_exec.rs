...
impl<'vm> HirEvaluator<'vm> {
  ...
  fn async_analysis_tick(&mut self, steps: &mut usize) -> Result<(), VmError> {
    // Budget async-function capability analysis so a single huge async function literal cannot do
    // unbounded work inside one statement/expression evaluation.
  ...
  fn hir_pat_contains_await_for_async_analysis(
    &mut self,
    script: &Arc<CompiledScript>,
    body: &hir_js::Body,
    pat_id: hir_js::PatId,
    ...
    let mut stack: Vec<hir_js::PatId> = vec![pat_id];
    while let Some(id) = stack.pop() {
      self.async_analysis_tick(steps)?;

      let pat = self.get_pat(body, id)?;
      match &pat.kind {
        hir_js::PatKind::Ident(_) => {}
        hir_js::PatKind::Rest(inner) => stack.push(**inner),
        hir_js::PatKind::Array(arr) => {
          ...
          for elem in &arr.elements {
            ...
            if let Some(default_value) = elem.default_value {
              if self.hir_expr_contains_await_for_async_analysis(script, body, default_value, steps)? {
                return Ok(true);
              }
            }
          }
        ...
        hir_js::PatKind::Object(obj) => {
          ...
          for prop in &obj.props {
            if let hir_js::ObjectKey::Computed(expr) = prop.key {
              if self.hir_expr_contains_await_for_async_analysis(script, body, expr, steps)? {
                return Ok(true);
              }
            }
  ...
  fn hir_expr_contains_await_for_async_analysis(
    &mut self,
    script: &Arc<CompiledScript>,
    body: &hir_js::Body,
    ...
    let mut stack: Vec<hir_js::ExprId> = vec![expr_id];
    while let Some(id) = stack.pop() {
      self.async_analysis_tick(steps)?;

      let expr = self.get_expr(body, id)?;
      match &expr.kind {
        hir_js::ExprKind::Await { .. } | hir_js::ExprKind::Yield { .. } => return Ok(true),

        hir_js::ExprKind::Unary { expr, .. }
        | hir_js::ExprKind::Update { expr, .. }
        | hir_js::ExprKind::NonNull { expr }
        | hir_js::ExprKind::Instantiation { expr, .. }
        | hir_js::ExprKind::TypeAssertion { expr, .. } => stack.push(*expr),
        ...
        hir_js::ExprKind::Assignment { target, value, .. } => {
          if self.hir_pat_contains_await_for_async_analysis(script, body, *target, steps)? {
            return Ok(true);
          }
          stack.push(*value);
        }
        ...
        hir_js::ExprKind::Array(arr) => {
          for elem in &arr.elements {
            match elem {
              hir_js::ArrayElement::Expr(e) | hir_js::ArrayElement::Spread(e) => stack.push(*e),
        ...
        hir_js::ExprKind::Object(obj) => {
          for prop in &obj.properties {
            match prop {
              ...
              hir_js::ObjectProperty::Spread(expr) => stack.push(*expr),
            }
          }
        }

        // Nested function bodies are not executed when the function value is created.
        hir_js::ExprKind::FunctionExpr { .. } => {}

        hir_js::ExprKind::ClassExpr { body: class_body, .. } => {
          if self.hir_class_body_contains_await_for_async_analysis(script, *class_body, steps)? {
            return Ok(true);
          }
        }
        ...
        hir_js::ExprKind::Satisfies { expr, .. } => stack.push(*expr),
        ...
        | hir_js::ExprKind::NewTarget
        | hir_js::ExprKind::Jsx(_) => {}
      }
    }
    Ok(false)
  }

  fn hir_body_contains_await_for_async_analysis(
    &mut self,
    script: &Arc<CompiledScript>,
    ...
    self.async_analysis_tick(steps)?;
    let Some(body) = script.hir.body(body_id) else {
      return Err(VmError::InvariantViolation(
    ...
    for stmt_id in &body.root_stmts {
      if self.hir_stmt_contains_await_for_async_analysis(script, body, *stmt_id, steps)? {
        return Ok(true);
      }
    }
    Ok(false)
  ...
  fn hir_class_body_contains_await_for_async_analysis(
    &mut self,
    script: &Arc<CompiledScript>,
    body_id: hir_js::BodyId,
    steps: &mut usize,
    ...
    let Some(body) = script.hir.body(body_id) else {
      return Err(VmError::InvariantViolation(
        "hir class body id missing from compiled script",
      ));
    };
    ...
    // Root statements model class-level evaluation (e.g. decorator expressions).
    for stmt_id in &body.root_stmts {
      if self.hir_stmt_contains_await_for_async_analysis(script, body, *stmt_id, steps)? {
        return Ok(true);
      }
    ...
    if let Some(extends) = class.extends {
      if self.hir_expr_contains_await_for_async_analysis(script, body, extends, steps)? {
        return Ok(true);
      }
    }
    ...
    for member in &class.members {
      match &member.kind {
        ...
        hir_js::ClassMemberKind::Method { key, .. } | hir_js::ClassMemberKind::Field { key, .. } => {
          if let hir_js::ClassMemberKey::Computed(expr) = key {
            if self.hir_expr_contains_await_for_async_analysis(script, body, *expr, steps)? {
              return Ok(true);
            }
          }
        ...
        hir_js::ClassMemberKind::StaticBlock { body: static_body, .. } => {
          if self.hir_body_contains_await_for_async_analysis(script, *static_body, steps)? {
            return Ok(true);
          }
        }
      }

      // Field initializers are evaluated at runtime (during construction or class evaluation for
      // static fields). Await inside them is not supported by the compiled async executor yet, so
      // conservatively require AST fallback.
      if let hir_js::ClassMemberKind::Field {
        initializer: Some(init_body),
        ..
      } = &member.kind
      {
        if self.hir_body_contains_await_for_async_analysis(script, *init_body, steps)? {
          return Ok(true);
        }
      }
    }
  ...
  fn hir_stmt_contains_await_for_async_analysis(
    &mut self,
    script: &Arc<CompiledScript>,
    body: &hir_js::Body,
    ...
    let mut stack: Vec<hir_js::StmtId> = vec![stmt_id];
    while let Some(id) = stack.pop() {
      self.async_analysis_tick(steps)?;
      let stmt = self.get_stmt(body, id)?;
      match &stmt.kind {
        ...
        hir_js::StmtKind::Var(decl) => {
          for declarator in &decl.declarators {
            if self.hir_pat_contains_await_for_async_analysis(script, body, declarator.pat, steps)? {
              return Ok(true);
            }
            if let Some(init) = declarator.init {
              if self.hir_expr_contains_await_for_async_analysis(script, body, init, steps)? {
                return Ok(true);
              }
            }
          }
        ...
        hir_js::StmtKind::If {
          ...
          if self.hir_expr_contains_await_for_async_analysis(script, body, *test, steps)? {
            return Ok(true);
          }
          stack.push(*consequent);
        ...
        hir_js::StmtKind::For {
          ...
          if let Some(update) = update {
            if self.hir_expr_contains_await_for_async_analysis(script, body, *update, steps)? {
              return Ok(true);
            }
          }
          stack.push(*inner);
        ...
        hir_js::StmtKind::ForIn {
          ...
          // Treat `for await..of` as containing an await so nested uses can conservatively opt into
          // AST fallback.
          if *await_ {
            return Ok(true);
          }
          match left {
            hir_js::ForHead::Pat(pat) => {
              if self.hir_pat_contains_await_for_async_analysis(script, body, *pat, steps)? {
                return Ok(true);
              }
            }
          ...
          if self.hir_expr_contains_await_for_async_analysis(script, body, *right, steps)? {
            return Ok(true);
          }
          stack.push(*inner);
        }
        hir_js::StmtKind::Switch { discriminant, cases } => {
          if self.hir_expr_contains_await_for_async_analysis(script, body, *discriminant, steps)? {
            return Ok(true);
          }
          for case in cases {
        ...
        hir_js::StmtKind::Try {
          ...
          if let Some(catch) = catch {
            if let Some(param) = catch.param {
              if self.hir_pat_contains_await_for_async_analysis(script, body, param, steps)? {
                return Ok(true);
              }
            }
          ...
          if let Some(finally_stmt) = finally_block {
            stack.push(*finally_stmt);
          }
        }
        hir_js::StmtKind::Decl(def_id) => {
          // Only descend into class definitions; nested function bodies are not executed when
          // creating the function value.
          let def = script
            .hir
            .def(*def_id)
          ...
          if decl_body.kind == hir_js::BodyKind::Class {
            if self.hir_class_body_contains_await_for_async_analysis(script, body_id, steps)? {
              return Ok(true);
            }
          }
        }
        hir_js::StmtKind::Labeled { body: inner, .. } => stack.push(*inner),
        hir_js::StmtKind::With { object, body: inner } => {
          if self.hir_expr_contains_await_for_async_analysis(script, body, *object, steps)? {
            return Ok(true);
          }
          stack.push(*inner);
        }
  ...
  /// Returns `true` if this async function body can execute via the compiled async executor (HIR)
  /// without hitting unsupported `await` forms.
  fn async_function_body_is_hir_supported(
    &mut self,
    script: &Arc<CompiledScript>,
    body_id: hir_js::BodyId,
  ) -> Result<bool, VmError> {
    ...
    // Only analyze async (non-generator) functions here.
    ...
    match &func_meta.body {
      hir_js::FunctionBody::Expr(expr_id) => {
        ...
        if let hir_js::ExprKind::Await { expr: awaited_expr } = expr.kind {
          // Support expression-bodied async arrow functions of the form `async () => await <expr>`.
          // Ensure the awaited expression itself contains no await.
          return Ok(!self.hir_expr_contains_await_for_async_analysis(
            script,
            body,
            awaited_expr,
            &mut steps,
        ...
        Ok(!self.hir_expr_contains_await_for_async_analysis(
          script,
          body,
          *expr_id,
          &mut steps,
      ...
      hir_js::FunctionBody::Block(stmts) => {
        fn compound_assign_op_supported(op: hir_js::AssignOp) -> bool {
          matches!(
            op,
        ...
        fn logical_assign_op_supported(op: hir_js::AssignOp) -> bool {
          matches!(
            op,
        ...
        fn assignment_target_is_supported<'a>(
          ...
          match &pat.kind {
            hir_js::PatKind::Ident(_) => Ok(true),
            hir_js::PatKind::AssignTarget(expr_id) => {
              let expr = evaluator.get_expr(body, *expr_id)?;
              Ok(matches!(expr.kind, hir_js::ExprKind::Ident(_) | hir_js::ExprKind::Member(_)))
            }
            _ => Ok(false),
          }
        }

        fn assignment_target_has_no_await<'a>(
          evaluator: &mut HirEvaluator<'a>,
          script: &Arc<CompiledScript>,
          body: &hir_js::Body,
          target: hir_js::PatId,
          ...
          Ok(!evaluator.hir_pat_contains_await_for_async_analysis(script, body, target, steps)?)
        }

        fn direct_await_operand_has_no_await<'a>(
          evaluator: &mut HirEvaluator<'a>,
          script: &Arc<CompiledScript>,
          body: &hir_js::Body,
          awaited_expr: hir_js::ExprId,
          steps: &mut usize,
        ) -> Result<bool, VmError> {
          Ok(!evaluator.hir_expr_contains_await_for_async_analysis(script, body, awaited_expr, steps)?)
        }

        fn object_pat_allows_direct_await_in_keys_and_defaults<'a>(
          evaluator: &mut HirEvaluator<'a>,
          script: &Arc<CompiledScript>,
          body: &hir_js::Body,
          pat_id: hir_js::PatId,
          ...
          for prop in obj_pat.props.iter() {
            ...
            if let hir_js::ObjectKey::Computed(expr_id) = &prop.key {
              ...
              } else if evaluator.hir_expr_contains_await_for_async_analysis(script, body, *expr_id, steps)? {
                return Ok(false);
              }
            }
            ...
            if let Some(default_expr_id) = prop.default_value {
              let default_expr = evaluator.get_expr(body, default_expr_id)?;
              if let hir_js::ExprKind::Await { expr: awaited_expr } = default_expr.kind {
                if !direct_await_operand_has_no_await(evaluator, script, body, awaited_expr, steps)? {
                  return Ok(false);
                }
              } else if evaluator.hir_expr_contains_await_for_async_analysis(script, body, default_expr_id, steps)? {
                return Ok(false);
              }
            }
            ...
            if value_is_assignment_target {
              if !assignment_target_is_supported(evaluator, body, prop.value)? {
                return Ok(false);
              }
              if !assignment_target_has_no_await(evaluator, script, body, prop.value, steps)? {
                return Ok(false);
              }
            } else if evaluator.hir_pat_contains_await_for_async_analysis(script, body, prop.value, steps)? {
              return Ok(false);
            }
          }
        ...
        fn for_head_allows_direct_object_pat_awaits<'a>(
          evaluator: &mut HirEvaluator<'a>,
          script: &Arc<CompiledScript>,
          body: &hir_js::Body,
          head: &hir_js::ForHead,
          ...
          match head {
            hir_js::ForHead::Pat(pat_id) => {
              ...
              if evaluator.hir_pat_contains_await_for_async_analysis(script, body, *pat_id, steps)? {
                return Ok(false);
              }
              Ok(true)
            }
            hir_js::ForHead::Var(var_decl) => {
              ...
              for decl in var_decl.declarators.iter() {
                ...
                if evaluator.hir_pat_contains_await_for_async_analysis(script, body, decl.pat, steps)? {
                  head_pat_contains_await = true;
                  break;
                }
                if let Some(init) = decl.init {
                  if evaluator.hir_expr_contains_await_for_async_analysis(script, body, init, steps)? {
                    return Ok(false);
                  }
                }
              }
              ...
              object_pat_allows_direct_await_in_keys_and_defaults(
                evaluator,
                script,
                body,
                decl.pat,
        ...
        fn for_triple_expr_is_supported<'a>(
          ...
          let expr = evaluator.get_expr(body, expr_id)?;
          if let hir_js::ExprKind::Await { expr: awaited_expr } = expr.kind {
            return direct_await_operand_has_no_await(evaluator, script, body, awaited_expr, steps);
          }
          if allow_assignment_await {
            if let hir_js::ExprKind::Assignment { op, target, value } = &expr.kind {
              ...
              if let hir_js::ExprKind::Await { expr: awaited_expr } = rhs.kind {
                ...
                if !assignment_target_is_supported(evaluator, body, *target)? {
                  return Ok(false);
                }
                if !assignment_target_has_no_await(evaluator, script, body, *target, steps)? {
                  return Ok(false);
                }
                return direct_await_operand_has_no_await(evaluator, script, body, awaited_expr, steps);
              }
          ...
          Ok(!evaluator.hir_expr_contains_await_for_async_analysis(script, body, expr_id, steps)?)
        }

        for (i, stmt_id) in stmts.iter().enumerate() {
          // Budget per statement list entry.
          if i % 32 == 0 {
            self.vm.tick()?;
          }
          ...
          match &stmt.kind {
            // Supported direct await statement forms (top-level only).
            hir_js::StmtKind::Expr(expr_id) => {
              let expr = self.get_expr(body, *expr_id)?;
              if let hir_js::ExprKind::Await { expr: awaited_expr } = expr.kind {
              ...
              if let hir_js::ExprKind::Assignment { op, target, value } = &expr.kind {
                let rhs = self.get_expr(body, *value)?;
                if let hir_js::ExprKind::Await { expr: awaited_expr } = rhs.kind {
                  if *op == hir_js::AssignOp::Assign
                    || compound_assign_op_supported(*op)
                    || logical_assign_op_supported(*op)
                ...
                // Support object destructuring assignment statements where the pattern contains
                // `await` in a computed key or default value (`AsyncDestructuringAssignState`).
                if *op == hir_js::AssignOp::Assign {
                  let target_pat = self.get_pat(body, *target)?;
                  if matches!(&target_pat.kind, hir_js::PatKind::Object(_))
                    && self.hir_pat_contains_await_for_async_analysis(script, body, *target, &mut steps)?
                  {
                    // The RHS is evaluated synchronously before the pattern is processed, so it must
                    // not contain any await.
                    if self.hir_expr_contains_await_for_async_analysis(script, body, *value, &mut steps)? {
                      return Ok(false);
                    }
              ...
              if self.hir_expr_contains_await_for_async_analysis(script, body, *expr_id, &mut steps)? {
                return Ok(false);
              }
            }
            ...
            hir_js::StmtKind::For {
              ...
              // `for (...)` loops with `await` in init/test/update are handled by `ForTripleAwaitState`.
              // The loop body itself is still evaluated synchronously, so it must not contain `await`.
              if self.hir_stmt_contains_await_for_async_analysis(script, body, *inner, &mut steps)? {
                return Ok(false);
              }
              ...
              if let Some(init) = init {
                match init {
                  hir_js::ForInit::Expr(expr_id) => {
                    if !for_triple_expr_is_supported(self, script, body, *expr_id, /* allow_assignment_await */ true, &mut steps)? {
                      return Ok(false);
                    }
                  }
                  hir_js::ForInit::Var(var_decl) => {
              ...
              if let Some(test) = test {
                if !for_triple_expr_is_supported(self, script, body, *test, /* allow_assignment_await */ false, &mut steps)? {
                  return Ok(false);
                }
              }

              if let Some(update) = update {
                if !for_triple_expr_is_supported(self, script, body, *update, /* allow_assignment_await */ true, &mut steps)? {
                  return Ok(false);
                }
              }
            }
            ...
            // yet.
            _ => {
              if self.hir_stmt_contains_await_for_async_analysis(script, body, *stmt_id, &mut steps)? {
                return Ok(false);
              }
            }
          }
    ...
    let (callee_value, this_value) = match &tag_expr.kind {
      hir_js::ExprKind::Member(member) => {
        ...
        if matches!(object_expr.kind, hir_js::ExprKind::Super) {
          ...
          // Spec: for `super[expr]`, `ToPropertyKey` is observed before `GetSuperBase`.
    ...
    match &target_expr.kind {
      ...
      hir_js::ExprKind::Member(member) => {
        ...
        if matches!(object_expr.kind, hir_js::ExprKind::Super) {
          ...
          // Spec: for `super[expr]++`, `ToPropertyKey` must be observed before `GetSuperBase`.
  ...
  fn eval_member_chain(
    ...
    if matches!(object_expr.kind, hir_js::ExprKind::Super) {
      ...
      // - and then capture the super base (`GetSuperBase`).
      //
      // This allows key conversion side effects (e.g. prototype mutation) to affect the base object
      // used for the super property lookup.
      let (key, obj) = match &member.property {
        hir_js::ObjectKey::Computed(expr_id) => {
...
#[derive(Debug)]
enum AsyncObjectPatternBindingAwait {
  /// Suspended while evaluating a computed property key of the form `await <expr>`.
  ComputedKey,
  /// Suspended while evaluating a property default value of the form `await <expr>`.
  DefaultValue { value_pat: hir_js::PatId },
}
...
///
/// This is a minimal state machine used by the compiled async evaluator to support `await`
/// expressions in object pattern *computed keys* and *default values* (e.g. `({ x = await p } = o)`
/// and `({ [await p]: x } = o)`).
///
/// Note: This intentionally only supports:
/// - `ObjectPat` (not array patterns),
/// - direct `await <expr>` forms in computed keys / defaults.
#[derive(Debug)]
struct AsyncObjectPatternBindingState {
  pat_id: hir_js::PatId,
  src_value_root: Option<RootId>,
  obj_root: Option<RootId>,
  prop_index: usize,
  awaiting: Option<AsyncObjectPatternBindingAwait>,
}

impl AsyncObjectPatternBindingState {
  fn new(
    evaluator: &mut HirEvaluator<'_>,
    scope: &mut Scope<'_>,
    pat_id: hir_js::PatId,
    ...
    // Root the RHS value across root registration + boxing: both operations can allocate and
    // trigger GC.
    let mut root_scope = scope.reborrow();
    root_scope.push_root(value)?;
  ...
  fn teardown(&mut self, heap: &mut crate::Heap) {
    if let Some(root) = self.src_value_root.take() {
      heap.remove_root(root);
    }
    if let Some(root) = self.obj_root.take() {
      heap.remove_root(root);
    }
  }
  ...
  ) -> Result<AsyncObjectPatternBindingPoll, VmError> {
    // If we previously suspended, first consume the resume value.
    if let Some(awaiting) = self.awaiting.take() {
      let Some(resume_value) = resume_value else {
        return Err(VmError::InvariantViolation(
          "async object pattern binding missing resume value",
        ));
      };
      let resumed = resume_value?;

      match awaiting {
        AsyncObjectPatternBindingAwait::ComputedKey => {
          // Resume computed-key evaluation: `await` result becomes the computed key expression
          // value.
          //
          // Note: `ToPropertyKey` can invoke user code. Root the resumed value across conversion.
          let mut key_scope = scope.reborrow();
          key_scope.push_root(resumed)?;
          let key = key_scope.to_property_key(evaluator.vm, &mut *evaluator.host, &mut *evaluator.hooks, resumed)?;
          root_property_key(&mut key_scope, key)?;
          // Continue processing the current property using the computed key.
          match self.process_property_with_key(evaluator, &mut key_scope, body, key, binding_kind)? {
        ...
        AsyncObjectPatternBindingAwait::DefaultValue { value_pat } => {
          // Resume default evaluation: bind the resumed value to the stored subpattern.
          evaluator.bind_pattern(scope, body, value_pat, resumed, binding_kind)?;
          self.prop_index = self.prop_index.saturating_add(1);
        }
      }
    } else if resume_value.is_some() {
      return Err(VmError::InvariantViolation(
        "async object pattern binding received unexpected resume value",
      ));
    }

    loop {
      let pat = evaluator.get_pat(body, self.pat_id)?;
      let hir_js::PatKind::Object(obj_pat) = &pat.kind else {
        return Err(VmError::InvariantViolation(
          "async object pattern binding used with non-object pattern",
        ));
      };

      if self.prop_index >= obj_pat.props.len() {
        if obj_pat.rest.is_some() {
          return Err(VmError::Unimplemented(
            "async object pattern binding with rest (hir-js compiled path)",
          ));
        }
        // Binding complete.
        self.teardown(scope.heap_mut());
        return Ok(AsyncObjectPatternBindingPoll::Complete);
      }

      // Budget object destructuring by pattern size (matches `bind_object_pattern`).
      evaluator.vm.tick()?;

      let prop = &obj_pat.props[self.prop_index];
      ...
      let key = match &prop.key {
        hir_js::ObjectKey::Computed(expr_id) => {
          let expr = evaluator.get_expr(body, *expr_id)?;
          if let hir_js::ExprKind::Await { expr: awaited_expr } = expr.kind {
            // Match `eval_expr` tick behaviour for evaluating the `await` expression itself.
            evaluator.vm.tick()?;
        ...
        _ => evaluator.eval_object_key(scope, body, &prop.key)?,
      };

      // Root key while performing property access + defaults: `GetV` can invoke user code and
      // allocate.
      let mut prop_scope = scope.reborrow();
      root_property_key(&mut prop_scope, key)?;

      match self.process_property_with_key(evaluator, &mut prop_scope, body, key, binding_kind)? {
        AsyncObjectPatternBindingStep::Complete => {
          self.prop_index = self.prop_index.saturating_add(1);
        }
        AsyncObjectPatternBindingStep::Await { await_value } => {
  ...
  fn process_property_with_key(
    ...
    key: PropertyKey,
    binding_kind: PatBindingKind,
  ) -> Result<AsyncObjectPatternBindingStep, VmError> {
    let src_value_root = self
      .src_value_root
      .ok_or(VmError::InvariantViolation("missing async object pattern src value root"))?;
    let obj_root = self
      .obj_root
      .ok_or(VmError::InvariantViolation("missing async object pattern object root"))?;
    let src_value = scope
      .heap()
      .get_root(src_value_root)
      .ok_or(VmError::InvariantViolation("missing async object pattern src value"))?;
    let Value::Object(obj) = scope
      .heap()
      .get_root(obj_root)
      .ok_or(VmError::InvariantViolation("missing async object pattern object value"))?
    else {
      return Err(VmError::InvariantViolation(
        "async object pattern object root is not an object",
      ));
    };

    // Ensure receiver/object are rooted across `[[Get]]` / default evaluation.
    ...
    let mut prop_value = scope.get_with_host_and_hooks(
      evaluator.vm,
      &mut *evaluator.host,
      &mut *evaluator.hooks,
      obj,
    ...
    if matches!(prop_value, Value::Undefined) {
      if let Some(default_expr) = prop.default_value {
        let default_node = evaluator.get_expr(body, default_expr)?;
        if let hir_js::ExprKind::Await { expr: awaited_expr } = default_node.kind {
          // Match `eval_expr` tick behaviour for evaluating the `await` expression itself.
          evaluator.vm.tick()?;
          let await_value = evaluator.eval_expr(scope, body, awaited_expr)?;
    ...
    evaluator.bind_pattern(scope, body, prop.value, prop_value, binding_kind)?;
    Ok(AsyncObjectPatternBindingStep::Complete)
  }
}
  ...
  fn poll(
    ...
    match &mut self.stage {
      ...
      ForAwaitOfStage::AwaitNext => {
        ...
        // Bind the loop head.
        //
        // The synchronous HIR evaluator does not support `await` expressions, so when a loop head
        // destructuring pattern contains an `await` in a computed key or default value we must
        // evaluate the pattern binding via a state machine that can suspend.
            ...
            if let hir_js::PatKind::Object(obj_pat) = &pat.kind {
              ...
              if has_await {
                let binding_kind = match var_decl.kind {
                  ...
                  hir_js::VarDeclKind::Const
                  | hir_js::VarDeclKind::Using
                  | hir_js::VarDeclKind::AwaitUsing => PatBindingKind::Const,
                };

                let mut bind_state =
                  AsyncObjectPatternBindingState::new(evaluator, &mut iter_scope, declarator.pat, iter_value)?;
                match bind_state.poll(evaluator, &mut iter_scope, body, None, binding_kind) {
      ...
      ForAwaitOfStage::AwaitHeadBind { iter_env_created } => {
        let Some(resume_value) = resume_value else {
          return Err(VmError::InvariantViolation(
            "for-await-of head binding await missing resume value",
          ));
        };
        ...
        let (_pat_id, binding_kind) = match &self.left {
          hir_js::ForHead::Var(var_decl) => {
            if var_decl.declarators.len() != 1 || var_decl.declarators[0].init.is_some() {
              return Err(VmError::Unimplemented(
                "for-await-of head async binding for multiple declarators/initializers (hir-js compiled path)",
              ));
            }
            let declarator = &var_decl.declarators[0];
          ...
          hir_js::ForHead::Pat(_) => {
            return Err(VmError::InvariantViolation(
              "for-await-of AwaitHeadBind used for non-var head",
            ))
          }
        };
 
        let Some(mut bind_state) = self.head_binding_state.take() else {
          return Err(VmError::InvariantViolation(
            "for-await-of awaiting head binding missing binding state",
          ));
        };
 
        match bind_state.poll(evaluator, scope, body, Some(resume_value), binding_kind) {
          Ok(AsyncObjectPatternBindingPoll::Complete) => {
...
impl ForOfState {
  fn new(
    ...
    Ok(Self {
      ...
      iterator_done: false,
      head_binding_state: None,
      stage: ForOfStage::Init,
    })
  }

  fn iterator_record(&self, heap: &crate::Heap) -> Result<iterator::IteratorRecord, VmError> {
    let iterator_root = self
      .iterator_root
      .ok_or(VmError::InvariantViolation("missing for-of iterator root"))?;
    let next_method_root = self
      .next_method_root
      .ok_or(VmError::InvariantViolation("missing for-of next method root"))?;
    let iterator = heap
      .get_root(iterator_root)
      .ok_or(VmError::InvariantViolation("missing for-of iterator root value"))?;
    let next_method = heap
      .get_root(next_method_root)
      .ok_or(VmError::InvariantViolation("missing for-of next method root value"))?;
    Ok(iterator::IteratorRecord {
      iterator,
      next_method,
      done: self.iterator_done,
    })
  }
  ...
  ) -> Result<ForOfPoll, VmError> {
    loop {
      match &mut self.stage {
        ForOfStage::Init => {
          if resume_value.is_some() {
            return Err(VmError::InvariantViolation("for-of init received resume value"));
          }
        ...
        ForOfStage::Iterate => {
          if resume_value.is_some() {
            return Err(VmError::InvariantViolation(
              "for-of iterate received unexpected resume value",
            ));
          }
          ...
          let v_root = self
            .v_root
            .ok_or(VmError::InvariantViolation("missing for-of loop value root"))?;

          // Tick once per iteration so `for (x of xs) {}` is budgeted even when the body is empty.
          evaluator.vm.tick()?;

          let mut record = self.iterator_record(scope.heap())?;
          ...
          self.iterator_done = record.done;

          let next_value = match next_value {
            Ok(v) => v,
            // Spec: `ForIn/OfBodyEvaluation` does not perform `IteratorClose` on step errors.
            Err(err) => {
              self.cleanup_roots(scope.heap_mut());
              return Err(err);
            }
          ...
          let Some(iter_value) = next_value else {
            let out = self.v;
            self.cleanup_roots(scope.heap_mut());
            return Ok(ForOfPoll::Complete(Flow::Normal(Some(out))));
          };
          ...
          // Bind head, allowing suspension for `await` in destructuring defaults/computed keys.
          let mut bound: bool = false;
          if let hir_js::ForHead::Var(var_decl) = &self.left {
            if var_decl.declarators.len() == 1 && var_decl.declarators[0].init.is_none() {
              let declarator = &var_decl.declarators[0];
          ...
          match flow {
            ...
            Flow::Continue(Some(label), value) => {
              let out_flow = Flow::Continue(Some(label), value).update_empty(Some(self.v));
              if let Some(v) = out_flow.value() {
                iter_scope.push_root(v)?;
              }
              let record = self.iterator_record(iter_scope.heap())?;
              ...
              self.cleanup_roots(iter_scope.heap_mut());
              return Ok(ForOfPoll::Complete(out_flow));
            }
            Flow::Break(None, break_value) => {
              let out = break_value.unwrap_or(self.v);
              if let Some(v) = break_value {
                iter_scope.push_root(v)?;
              }
              let record = self.iterator_record(iter_scope.heap())?;
            ...
            Flow::Break(Some(label), break_value) => {
              let out_flow = Flow::Break(Some(label), break_value).update_empty(Some(self.v));
              if let Some(v) = out_flow.value() {
                iter_scope.push_root(v)?;
              }
              let record = self.iterator_record(iter_scope.heap())?;
        ...
        ForOfStage::AwaitHeadBind { iter_env_created } => {
          let Some(resume) = resume_value.take() else {
            return Err(VmError::InvariantViolation(
              "for-of awaiting head bind missing resume value",
            ));
          };
          ...
          let (binding_kind, body_stmt) = match &self.left {
            hir_js::ForHead::Var(var_decl) => {
              let kind = match var_decl.kind {
                hir_js::VarDeclKind::Var => PatBindingKind::Var,
                hir_js::VarDeclKind::Let => PatBindingKind::Let,
            ...
            hir_js::ForHead::Pat(_) => {
              return Err(VmError::InvariantViolation(
                "for-of AwaitHeadBind used for non-var head",
              ))
            }
          };
          ...
          let Some(mut bind_state) = self.head_binding_state.take() else {
            return Err(VmError::InvariantViolation(
              "for-of awaiting head bind missing binding state",
            ));
          };

          match bind_state.poll(evaluator, scope, body, Some(resume), binding_kind) {
            Ok(AsyncObjectPatternBindingPoll::Complete) => {
            ...
            Err(err) => {
              ...
              let record = self.iterator_record(scope.heap())?;
              match iterator::iterator_close(
                evaluator.vm,
                &mut *evaluator.host,
                &mut *evaluator.hooks,
          ...
          // Binding finished; run body in the current per-iteration env.
          let mut iter_scope = scope.reborrow();
          let flow = match evaluator.eval_stmt(&mut iter_scope, body, body_stmt) {
            Ok(f) => f,
            Err(err) => {
              if *iter_env_created {
                evaluator.env.set_lexical_env(iter_scope.heap_mut(), outer_lex);
          ...
          // Body completed; switch back to the iterate stage.
          self.stage = ForOfStage::Iterate;
...
struct PendingAssignment {
  reference: AssignmentReference,
  /// Compound assignment operator to apply after the awaited RHS resolves.
  ///
  /// `None` indicates a plain `=` assignment.
...
impl PendingAssignment {
  fn teardown(&mut self, heap: &mut crate::Heap) {
    ...
    if let Some(id) = self.left_root.take() {
      if heap.get_root(id).is_some() {
        heap.remove_root(id);
      }
    }
          ...
          ForTripleAwaitStage::Update => {
            ...
            if let Some(iter_env) = self.iter_env {
              let outer_lex = self
                ...
                .ok_or(VmError::InvariantViolation("missing for-triple outer lex env"))?;
              let next_env =
                evaluator.create_for_triple_per_iteration_env(scope, outer_lex, iter_env)?;
...
#[derive(Debug)]
enum AsyncDestructuringAssignAwait {
  ComputedKey,
  DefaultValue { target: AssignmentReference },
}
...
#[derive(Debug)]
struct AsyncDestructuringAssignState {
  pat_id: hir_js::PatId,
  src_value_root: Option<RootId>,
  obj_root: Option<RootId>,
  prop_index: usize,
  awaiting: Option<AsyncDestructuringAssignAwait>,
  pending_base_root: Option<RootId>,
  pending_key_root: Option<RootId>,
  pending_super_base_root: Option<RootId>,
}

impl AsyncDestructuringAssignState {
  fn new(
    evaluator: &mut HirEvaluator<'_>,
    scope: &mut Scope<'_>,
    pat_id: hir_js::PatId,
    ...
    // Root the input value across root registration + boxing: both operations can allocate and
    // trigger GC.
    let mut root_scope = scope.reborrow();
    root_scope.push_root(value)?;
    let src_value_root = root_scope.heap_mut().add_root(value)?;
    ...
    Ok(Self {
      ...
      prop_index: 0,
      awaiting: None,
      pending_base_root: None,
      pending_key_root: None,
      pending_super_base_root: None,
    })
  }
  ...
  fn teardown(&mut self, heap: &mut crate::Heap) {
    ...
    if let Some(root) = self.pending_key_root.take() {
      heap.remove_root(root);
    }
    if let Some(root) = self.pending_super_base_root.take() {
      heap.remove_root(root);
    }
  }
  ...
  fn poll(
    ...
    body: &hir_js::Body,
    resume_value: Option<Result<Value, VmError>>,
  ) -> Result<AsyncDestructuringAssignPoll, VmError> {
    if let Some(awaiting) = self.awaiting.take() {
      let Some(resume_value) = resume_value else {
        return Err(VmError::InvariantViolation(
          "async destructuring assignment missing resume value",
        ));
      };
      let resumed = resume_value?;

      match awaiting {
        AsyncDestructuringAssignAwait::ComputedKey => {
          let mut key_scope = scope.reborrow();
          key_scope.push_root(resumed)?;
          let key = key_scope.to_property_key(evaluator.vm, &mut *evaluator.host, &mut *evaluator.hooks, resumed)?;
          root_property_key(&mut key_scope, key)?;
          match self.process_property_with_key(evaluator, &mut key_scope, body, key)? {
        ...
        AsyncDestructuringAssignAwait::DefaultValue { target } => {
          let mut assign_scope = scope.reborrow();
      ...
      return Err(VmError::InvariantViolation(
        "async destructuring assignment received unexpected resume value",
      ));
    }
    ...
    loop {
      ...
      let hir_js::PatKind::Object(obj_pat) = &pat.kind else {
        return Err(VmError::Unimplemented(
          "async destructuring assignment pattern kind (hir-js compiled path)",
        ));
      };
      if obj_pat.rest.is_some() {
        return Err(VmError::Unimplemented(
          "async destructuring assignment with rest (hir-js compiled path)",
        ));
      }

      if self.prop_index >= obj_pat.props.len() {
        self.teardown(scope.heap_mut());
        return Ok(AsyncDestructuringAssignPoll::Complete);
      }
      ...
      let mut prop_scope = scope.reborrow();
      root_property_key(&mut prop_scope, key)?;
      match self.process_property_with_key(evaluator, &mut prop_scope, body, key)? {
        AsyncDestructuringAssignPoll::Complete => {
          self.prop_index = self.prop_index.saturating_add(1);
        }
        AsyncDestructuringAssignPoll::Await { await_value } => {
  ...
  fn process_property_with_key(
    ...
    let src_value_root = self
      .src_value_root
      .ok_or(VmError::InvariantViolation("missing destructuring assignment src value root"))?;
    let obj_root = self
      .obj_root
      .ok_or(VmError::InvariantViolation("missing destructuring assignment object root"))?;
    let src_value = scope
      .heap()
      .get_root(src_value_root)
      .ok_or(VmError::InvariantViolation("missing destructuring assignment src value"))?;
    let Value::Object(obj) = scope
      .heap()
      .get_root(obj_root)
      .ok_or(VmError::InvariantViolation("missing destructuring assignment object value"))?
    else {
      return Err(VmError::InvariantViolation(
        "destructuring assignment object root is not an object",
      ));
    };
    ...
    let hir_js::PatKind::Object(obj_pat) = &pat.kind else {
      return Err(VmError::InvariantViolation(
        "async destructuring assignment used with non-object pattern",
      ));
    };
    let prop = &obj_pat.props[self.prop_index];

    // Evaluate assignment target reference before accessing the RHS property, matching
    // `KeyedDestructuringAssignmentEvaluation`.
    let reference = evaluator.eval_assignment_reference(scope, body, prop.value)?;
    ...
    if matches!(prop_value, Value::Undefined) {
      if let Some(default_expr) = prop.default_value {
        let default_node = evaluator.get_expr(body, default_expr)?;
        if let hir_js::ExprKind::Await { expr: awaited_expr } = default_node.kind {
          // Root the assignment reference components across await-value evaluation.
          evaluator.root_assignment_reference(scope, &reference)?;
...
impl HirAsyncState {
  ...
  pub(crate) fn new(script: Arc<CompiledScript>, body_id: hir_js::BodyId) -> Result<Self, VmError> {
    let body = script
      ...
      .body(body_id)
      .ok_or(VmError::InvariantViolation("hir body id missing from compiled script"))?;
    let (body_kind, in_root_stmt_list) = match body.kind {
      ...
      hir_js::BodyKind::Function => {
        ...
        (body_kind, /* in_root_stmt_list */ false)
      }
      other => {
    ...
    loop {
      if let Some(active) = &mut self.active {
        match active {
          ...
          HirAsyncActive::DestructuringAssign(state) => {
            let poll = state.poll(evaluator, scope, body, resume_value.take());
            match poll {
              Ok(AsyncDestructuringAssignPoll::Complete) => {
                self.active = None;
          ...
          HirAsyncActive::AwaitAssignmentStmt {
            ...
            let assign_res: Result<(), VmError> = (|| {
              ...
              if let Some(op) = pending.assign_op {
                ...
                let left = assign_scope
                  .heap()
                  .get_root(left_root)
                  .ok_or(VmError::InvariantViolation(
                    "hir async compound assignment missing left value root value",
                  ))?;

                assign_scope.push_roots(&[left, resumed_value])?;
                let out =
                  evaluator.apply_compound_assignment_op(&mut assign_scope, op, left, resumed_value)?;
                assign_scope.push_root(out)?;
                evaluator.put_value_to_assignment_reference(&mut assign_scope, &pending.reference, out)?;
              } else {
                assign_scope.push_root(resumed_value)?;
      ...
      // Destructuring assignment patterns evaluate the RHS first, but may then suspend while
      // evaluating defaults or computed keys in the pattern.
      if let hir_js::StmtKind::Expr(expr_id) = &stmt.kind {
        ...
        {
          let pat = evaluator.get_pat(body, *target)?;
          if matches!(&pat.kind, hir_js::PatKind::Object(_)) {
            let hir_js::PatKind::Object(obj_pat) = &pat.kind else {
              unreachable!();
            };
            let mut has_await_in_pattern: bool = false;
            for prop in &obj_pat.props {
              if let hir_js::ObjectKey::Computed(expr_id) = &prop.key {
                let expr = evaluator.get_expr(body, *expr_id)?;
                if matches!(expr.kind, hir_js::ExprKind::Await { .. }) {
                  has_await_in_pattern = true;
                  break;
                }
              }
              if let Some(default_expr) = prop.default_value {
            ...
            if has_await_in_pattern {
              ...
              let mut assign_scope = scope.reborrow();
              assign_scope.push_root(rhs_value)?;
              let state = match AsyncDestructuringAssignState::new(
                evaluator,
                &mut assign_scope,
                *target,
                rhs_value,
            ...
            if let hir_js::ExprKind::Await { expr: awaited_expr } = &rhs.kind {
              ...
              let await_value = match evaluator.eval_expr(&mut await_scope, body, *awaited_expr) {
                ...
                Err(err) => {
                  ...
                  // Remove any compound-assignment LHS root before propagating.
                  if let Some(id) = left_root.take() {
                    await_scope.heap_mut().remove_root(id);
                  }
                  return match err {
              ...
              // Root the await argument value across persistent root creation.
              await_scope.push_root(await_value)?;

              // Create persistent roots for the assignment reference base/key so we can resume after
              // the await boundary.
              let mut base_root: Option<RootId> = None;
              let mut key_root: Option<RootId> = None;
              let assign_root_res: Result<(), VmError> = (|| {
                match &reference {
                  ...
                  AssignmentReference::Property { base, key } => {
                    ...
                    let b = await_scope.heap_mut().add_root(*base)?;
                    base_root = Some(b);
                    match await_scope.heap_mut().add_root(key_value) {
                      Ok(id) => key_root = Some(id),
                      Err(err) => {
                        await_scope.heap_mut().remove_root(b);
                        base_root = None;
                        return Err(err);
                      }
                    }
                  ...
                  AssignmentReference::SuperProperty { super_base, key, .. } => {
                    ...
                    let base_value = (*super_base).map(Value::Object).unwrap_or(Value::Null);
                    let b = await_scope.heap_mut().add_root(base_value)?;
                    base_root = Some(b);
                    match await_scope.heap_mut().add_root(key_value) {
                      Ok(id) => key_root = Some(id),
                      Err(err) => {
              ...
              let pending = PendingAssignment {
                reference,
                assign_op: compound_op.then_some(*op),
                base_root,
                key_root,
                left_root,
              };
...
#[cfg(test)]
mod async_function_hir_exec_tests {
  use crate::exec::AsyncFrame;
  use crate::function::{CallHandler, FunctionData};
  use crate::vm::VmAsyncContinuation;
  use crate::{CompiledScript, Heap, HeapLimits, JsRuntime, PromiseState, Value, Vm, VmError, VmOptions};

  #[test]
  fn compiled_async_function_suspends_via_hir_async_frame() -> Result<(), VmError> {
  ...
  #[test]
  fn compiled_async_function_assignment_stmt_direct_await_executes_via_hir() -> Result<(), VmError> {
    let vm = Vm::new(VmOptions::default());
    // Async functions allocate Promise/job machinery; use a slightly larger heap to avoid spurious
    // OOMs as builtin surface area grows.
    let heap = Heap::new(HeapLimits::new(4 * 1024 * 1024, 4 * 1024 * 1024));
  ...
  #[test]
  fn compiled_async_function_logical_assignment_stmt_direct_await_executes_via_hir() -> Result<(), VmError> {
    let vm = Vm::new(VmOptions::default());
    // Async functions allocate Promise/job machinery; use a slightly larger heap to avoid spurious
    // OOMs as builtin surface area grows.
    let heap = Heap::new(HeapLimits::new(4 * 1024 * 1024, 4 * 1024 * 1024));
    ...
    let script = CompiledScript::compile_script(
      ...
      r#"
        async function f() {
          ...
          // Should short-circuit (x is non-nullish) and must not evaluate the awaited operand.
          x ??= await (x = 999, Promise.resolve(4));
          return x;
        }
        f;
      "#,
    ...
    assert!(
      matches!(func_data, FunctionData::None),
      "expected logical assignment await to execute via HIR (no per-function AST fallback), got {func_data:?}"
    );
  ...
  #[test]
  fn compiled_async_function_for_triple_init_logical_assignment_direct_await_executes_via_hir() -> Result<(), VmError> {
    let vm = Vm::new(VmOptions::default());
    // Async functions allocate Promise/job machinery; use a slightly larger heap to avoid spurious
    // OOMs as builtin surface area grows.
    let heap = Heap::new(HeapLimits::new(4 * 1024 * 1024, 4 * 1024 * 1024));
    ...
    let script = CompiledScript::compile_script(
      ...
      r#"
        async function f() {
          let x = 0;
          for (x ||= await Promise.resolve(2); x < 3; x++) {
            break;
          }
          return x;
        }
    ...
    assert!(
      matches!(func_data, FunctionData::None),
      "expected logical assignment await in for-loop head to execute via HIR (no per-function AST fallback), got {func_data:?}"
    );
...
mod compiled_hir_async_await_semantics_tests {
  ...
  #[test]
  fn compiled_async_arrow_expr_body_direct_await() -> Result<(), VmError> {
    run_compiled_async_fn_case(
      r#"
        let f = async () => await Promise.resolve(1);
        this.__f = f;
        this.__p = f();
        this.__p;
      "#,
  ...
  #[test]
  fn compiled_async_direct_await_expr_stmt() -> Result<(), VmError> {
    run_compiled_async_fn_case(
      r#"
        async function f() {
  ...
  fn compiled_async_throw_await() -> Result<(), VmError> {
    run_compiled_async_fn_case(
      r#"
        ...
        this.__f = f;
        this.__p = f().catch(e => e);
        this.__p;
      "#,
      ExpectedValue::String("boom"),
    )
  }
...
mod async_for_await_of_async_iterator_close_tests {
  ...
  #[test]
  fn compiled_hir_async_fn_for_await_of_throw_awaits_async_iterator_close_before_catch() -> Result<(), VmError> {
    let vm = Vm::new(VmOptions::default());
    let heap = Heap::new(HeapLimits::new(1024 * 1024, 1024 * 1024));
    let mut rt = JsRuntime::new(vm, heap)?;
