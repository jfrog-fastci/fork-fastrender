...
#[test]
fn for_await_of_await_next_then_getter_throw_does_not_close_iterator() -> Result<(), VmError> {
  let mut rt = new_runtime();
  ...
  let value = rt.exec_script(
    r#"
       ...
       iterable[Symbol.asyncIterator] = function () {
         return {
           next() {
             // `await nextValue` performs thenable assimilation; errors while retrieving `then`
             // are iterator step errors and must not trigger AsyncIteratorClose.
             return {
               get then() {
                 throw "boom";
               },
...
#[test]
fn for_await_of_await_next_then_method_throw_does_not_close_iterator() -> Result<(), VmError> {
  let mut rt = new_runtime();
  ...
  let value = rt.exec_script(
    r#"
       ...
       iterable[Symbol.asyncIterator] = function () {
         return {
           next() {
             // Thenable job throws while awaiting `nextResult`.
             return {
               then() {
                 throw "boom";
               },
