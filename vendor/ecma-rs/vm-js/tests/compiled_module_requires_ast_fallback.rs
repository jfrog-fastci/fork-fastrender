...
#[test]
fn compiled_module_requires_ast_fallback_is_respected_for_compiled_importers_in_evaluate() -> Result<(), VmError>
{
  let (mut vm, mut heap, mut realm) = new_vm_heap_realm()?;
  let mut host = ();
  let mut hooks = MicrotaskQueue::new();
  ...
  let result = (|| -> Result<(), VmError> {
    ...
    // Evaluate via the SCC evaluator (ModuleGraph::evaluate / execute_scc). This should execute B via
    // the compiled executor while executing A via the AST interpreter.
    let promise = graph.evaluate(
      &mut vm,
      &mut heap,
      realm.global_object(),
