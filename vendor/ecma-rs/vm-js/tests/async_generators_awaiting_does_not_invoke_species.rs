...
fn async_generators_supported(rt: &mut JsRuntime) -> Result<bool, VmError> {
  // vm-js currently surfaces `async function*` as a throwable `SyntaxError` (feature-detectable via
  // try/catch). These tests should start enforcing the semantics once async generators are
  // implemented.
  let value = rt.exec_script(
    r#"
      try {
        ...
        if (e && e.message === "async generator functions") false;
        else throw e;
      }
    "#,
  )?;
  let Value::Bool(ok) = value else {
    return Err(VmError::Unimplemented(
      "async_generators_supported: expected script to return a boolean",
    ));
  };
  Ok(ok)
}
