...
#[test]
fn await_in_class_extends_expression_sets_has_tla() {
  // `await` in a class heritage expression is evaluated during module instantiation, so it counts
  // as top-level await.
  let record = parse("class C extends (await p) {} export {};");
  assert!(record.has_tla);
}

#[test]
fn await_using_sets_has_tla() {
  // `await using` requires async evaluation even when the initializer expression itself contains no
  // AwaitExpression.
