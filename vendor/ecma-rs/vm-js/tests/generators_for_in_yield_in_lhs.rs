...
#[test]
fn generator_for_in_object_pattern_prop_assignment_target_computed_member_to_property_key_is_delayed_until_putvalue(
)
{
  let mut rt = new_runtime();
  let value = rt
      ...
      r#"
        (() => {
          ...
          function* g() {
            ...
            for ({missing: obj[yield 0]} in {abc: 0}) { return log.join("|"); }
          }
...
#[test]
fn generator_for_in_array_pattern_elem_assignment_target_computed_member_to_property_key_is_delayed_until_putvalue(
)
{
  let mut rt = new_runtime();
  let value = rt
    .exec_script(
      r#"
        (() => {
          ...
          function* g() {
            var obj = { set k(v) { log.push("set"); this._k = v; } };
            for ([obj[yield 0]] in {abc: 0}) { return log.join("|"); }
          }
...
#[test]
fn generator_for_in_array_pattern_rest_assignment_target_computed_member_to_property_key_is_delayed_until_putvalue(
)
{
  let mut rt = new_runtime();
  let value = rt
