...
#[test]
fn generator_super_logical_and_assignment_with_yield_in_computed_key_and_rhs_yield_star_captures_key_super_base_and_decision_across_yield_star(
) {
  let mut rt = new_runtime();
  let value = rt
    .exec_script(
      r#"
        ...
        class D extends B1 {
          constructor(){ super(); this._x = 0; this._y = 1; }
          *gen() {
            const r = (super[(yield "key", k)] &&= (yield* rhs()));
            return r === 5 && this._x === 0 && this._y === 5 && log.join(",") === "get1y,set1y:5";
          }
...
#[test]
fn generator_super_nullish_coalescing_assignment_with_yield_in_computed_key_and_rhs_yield_star_captures_key_super_base_and_decision_across_yield_star(
) {
  let mut rt = new_runtime();
  let value = rt
    .exec_script(
      r#"
        ...
        class D extends B1 {
          constructor(){ super(); this._x = 0; this._y = null; }
          *gen() {
            const r = (super[(yield "key", k)] ??= (yield* rhs()));
            return r === 5 && this._x === 0 && this._y === 5 && log.join(",") === "get1y,set1y:5";
          }
