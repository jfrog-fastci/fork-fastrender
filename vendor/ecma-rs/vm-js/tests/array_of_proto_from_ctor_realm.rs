...
fn array_of_proto_from_ctor_realm() -> Result<(), VmError> {
  ...
  let (c_obj, c_root): (vm_js::GcObject, RootId) = {
    ...
    assert!(scope.ordinary_set(&mut vm, c_obj, prototype_key, Value::Null, Value::Object(c_obj))?);
  ...
  let mut current = Realm::new(&mut vm, &mut heap)?;
  let current_intr = *current.intrinsics();
