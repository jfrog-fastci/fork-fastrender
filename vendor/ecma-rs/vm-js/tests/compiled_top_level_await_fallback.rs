...
#[test]
fn compiled_script_await_using_in_nested_block_falls_back() -> Result<(), VmError> {
...
#[test]
fn compiled_script_top_level_labelled_for_await_of_break_invokes_iterator_return() -> Result<(), VmError> {
  let mut rt = new_runtime();

  let script = CompiledScript::compile_script(
    rt.heap_mut(),
    "compiled_top_level_labelled_for_await_of_break_invokes_return.js",
    r#"
      var out = "";
      var return_calls = 0;
      var iter = {
      ...
      outer: for await (var x of iterable) {
        out = x;
        break outer;
      }
      out
  ...
  assert!(
    !script.top_level_await_requires_ast_fallback,
    "labelled top-level for-await-of should be supported by the HIR async classic-script executor"
  );
  assert!(
    !script.requires_ast_fallback,
    "labelled top-level for-await-of should not trigger a full AST fallback"
  );
