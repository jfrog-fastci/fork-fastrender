...
#[test]
fn compiled_module_top_level_try_for_await_of_throw_awaits_async_iterator_close_before_running_catch(
) -> Result<(), VmError> {
  let mut rt = new_runtime();
  let mut hooks = MicrotaskQueue::new();
  let mut host = ();
  ...
  let result = (|| -> Result<(), VmError> {
    let compiled = CompiledScript::compile_module(
      ...
      r#"
        ...
        export let caught = "";
        export let returnCalls = 0;
        export let closeResolved = false;
        globalThis.__resolveClose = null;
    ...
    assert!(
      !compiled.top_level_await_requires_ast_fallback,
      "top-level try/catch around a for-await-of loop should be supported by the compiled module TLA executor"
    );
    assert!(
      !compiled.requires_ast_fallback,
      "supported compiled module TLA shapes should not trigger the general compiled-module AST fallback"
    ...
    // Drive module evaluation. It should suspend in AsyncIteratorClose while awaiting
    // iterator.return(), so the evaluation promise remains pending after this checkpoint.
    let errors = hooks.perform_microtask_checkpoint(&mut rt);
    if let Some(err) = errors.into_iter().next() {
      return Err(err);
    }

    {
      ...
      assert_eq!(
        ...
        "module evaluation should await iterator close before running catch"
      );
      ...
      assert_eq!(scope.heap().get_string(actual)?.to_utf8_lossy(), "a");
      let Value::String(caught) = ns_get(vm, &mut host, &mut hooks, &mut scope, ns, "caught")? else {
        panic!("expected module export 'caught' to be a string");
      };
      assert_eq!(scope.heap().get_string(caught)?.to_utf8_lossy(), "");
      assert_eq!(
        ns_get(vm, &mut host, &mut hooks, &mut scope, ns, "returnCalls")?,
        Value::Number(1.0),
        "abrupt completion should invoke iterator.return() before suspending"
      );
      assert_eq!(
        ns_get(vm, &mut host, &mut hooks, &mut scope, ns, "closeResolved")?,
        Value::Bool(false),
      );

      // Resolve the pending close promise.
      let resolve_close = ns_get(vm, &mut host, &mut hooks, &mut scope, global_object, "__resolveClose")?;
      let Value::Object(_) = resolve_close else {
        panic!("expected globalThis.__resolveClose to be a function object");
      };
    ...
    // Now that iterator.return() has resolved, module evaluation can complete and run the catch
    // clause.
    let errors = hooks.perform_microtask_checkpoint(&mut rt);
    if let Some(err) = errors.into_iter().next() {
      return Err(err);
    ...
    assert_eq!(scope.heap().get_string(caught)?.to_utf8_lossy(), "boom");
    assert_eq!(
      ns_get(vm, &mut host, &mut hooks, &mut scope, ns, "returnCalls")?,
      Value::Number(1.0),
    );
...
#[test]
fn compiled_module_top_level_var_decl_object_pat_with_await_in_default_executes() -> Result<(), VmError> {
  let mut rt = new_runtime();
  let mut hooks = MicrotaskQueue::new();
  let mut host = ();
  ...
  let result = (|| -> Result<(), VmError> {
    let compiled = CompiledScript::compile_module(
      ...
      r#"
        export let x;
        const { y = await Promise.resolve("a") } = {};
        x = y;
      "#,
    )?;
    assert!(
      !compiled.top_level_await_requires_ast_fallback,
      "top-level var declarations with await in object-pattern defaults should be supported by the compiled module TLA executor"
    );

    let mut record = SourceTextModuleRecord::parse_source(rt.heap_mut(), compiled.source.clone())?;
    assert!(
      record.has_tla,
      "await in a top-level binding pattern should mark the module as `[[HasTLA]]`"
    );
    record.compiled = Some(compiled);
    record.clear_ast();
...
#[test]
fn compiled_module_top_level_var_decl_object_pat_with_await_in_computed_key_executes() -> Result<(), VmError> {
  let mut rt = new_runtime();
  let mut hooks = MicrotaskQueue::new();
  let mut host = ();
  ...
  let result = (|| -> Result<(), VmError> {
    let compiled = CompiledScript::compile_module(
      ...
      r#"
        export let x;
        const { [await Promise.resolve("k")]: v } = { k: "a" };
        x = v;
      "#,
    )?;
    assert!(
      !compiled.top_level_await_requires_ast_fallback,
      "top-level var declarations with await in object-pattern computed keys should be supported by the compiled module TLA executor"
    );
...
#[test]
fn compiled_module_top_level_class_decl_with_await_in_extends_awaits_before_running_static_block(
) -> Result<(), VmError> {
  let mut rt = new_runtime();
  let mut hooks = MicrotaskQueue::new();
  let mut host = ();
  ...
  let result = (|| -> Result<(), VmError> {
    ...
    assert!(
      !compiled.top_level_await_requires_ast_fallback,
      "top-level class declarations with await in `extends` should be supported by the compiled module TLA executor"
    );
    assert!(
      !compiled.requires_ast_fallback,
      "supported compiled module TLA shapes should not trigger the general compiled-module AST fallback"
    ...
    let mut record = SourceTextModuleRecord::parse_source(rt.heap_mut(), compiled.source.clone())?;
    assert!(record.has_tla, "await in class heritage should mark the module as `[[HasTLA]]`");
    record.compiled = Some(compiled);
    record.clear_ast();
...

    {
      ...
      assert_eq!(
        scope.heap().promise_state(promise_obj)?,
        PromiseState::Pending,
        "module evaluation should suspend at `await` in class `extends`"
      );
      ...
      assert_eq!(
        ...
        "",
        "class static block should not run before the awaited heritage resolves"
      );
    ...
    // Now that class heritage has resolved, module evaluation can complete and run the static block
    ...
    assert_eq!(scope.heap().get_string(actual)?.to_utf8_lossy(), "sdone");
    Ok(())
  })();
...
#[test]
fn compiled_module_top_level_class_decl_with_await_in_computed_member_key_executes() -> Result<(), VmError> {
  let mut rt = new_runtime();
  let mut hooks = MicrotaskQueue::new();
  let mut host = ();
  ...
  let result = (|| -> Result<(), VmError> {
    ...
    assert!(
      !compiled.top_level_await_requires_ast_fallback,
      "top-level class declarations with await in computed member keys should be supported by the compiled module TLA executor"
    );
    assert!(
      !compiled.requires_ast_fallback,
      "supported compiled module TLA shapes should not trigger the general compiled-module AST fallback"
    ...
    assert!(
      record.has_tla,
      "await in a class computed key should mark the module as `[[HasTLA]]`"
    );
    record.compiled = Some(compiled);
    record.clear_ast();
...

    {
      ...
      assert_eq!(
        scope.heap().promise_state(promise_obj)?,
        PromiseState::Pending,
        "module evaluation should suspend at `await` in class computed key"
      );
      ...
      assert_eq!(
        scope.heap().get_string(actual)?.to_utf8_lossy(),
        "",
        "class static block should not run before the awaited computed key resolves"
      );
