...
#[test]
fn await_in_for_of_lhs_destructuring_default_value() -> Result<(), VmError> {
  let mut rt = new_runtime();
  ...
  let value = rt.exec_script(
    r#"
      ...
      async function f() {
        var loop_out = "";
        for (const { x = await Promise.resolve("ok") } of [ {} ]) {
          loop_out = x;
        }
        return loop_out;
      }
...
fn await_in_for_of_lhs_destructuring_computed_key() -> Result<(), VmError> {
  ...
  let value = rt.exec_script(
    r#"
      var out = "";
      async function f() {
        for (const { [await Promise.resolve("x")]: v } of [ { x: "ok" } ]) {
          return v;
        }
        return "bad";
      }
...
#[test]
fn await_in_for_of_lhs_array_pattern_default() -> Result<(), VmError> {
  let mut rt = new_runtime();
  ...
  let value = rt.exec_script(
    r#"
      ...
      async function f() {
        var s = "";
        for (const [x = await Promise.resolve("a")] of [[]]) {
          s = s + x;
        }
        return s;
      }
...
#[test]
fn await_in_for_await_of_lhs_array_pattern_default() -> Result<(), VmError> {
  let mut rt = new_runtime();
...
#[test]
fn await_in_for_in_lhs_object_pattern_computed_key() -> Result<(), VmError> {
  let mut rt = new_runtime();
  ...
  let value = rt.exec_script(
    r#"
      ...
      async function f() {
        var s = "";
        for (const { [await Promise.resolve("0")]: c } in { ab: 1 }) {
          s = s + c;
        }
        return s;
      }
