...
fn well_known_symbols_are_shared_across_realms_on_same_heap() -> Result<(), VmError> {
  ...
  // enough that baseline intrinsic growth doesn't intermittently trip OOM as the engine grows.
  ...
  let mut realm_b = match Realm::new(&mut vm, &mut heap) {
    Ok(r) => r,
    Err(err) => {
      // Avoid leaking persistent roots (and tripping the Realm drop assertion) if the second realm
      // fails to initialize.
  ...
  let result: Result<(), VmError> = (|| {
    ...
    // Property attributes: well-known symbol properties are non-writable, non-enumerable,
    // non-configurable.
    ...
    for (name, expected) in [
      ...
      assert!(
        !desc.configurable,
        "Symbol.{name} should be non-configurable"
      );
      match desc.kind {
  ...
  // Avoid leaking persistent roots (and tripping the Realm drop assertion), even if the test body
  // returns an error.
  realm_a.teardown(&mut heap);
