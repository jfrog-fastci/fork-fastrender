...
pub fn is_unimplemented_async_generator_error(
  ...
  // `SyntaxError("async generator functions")` object at runtime. Treat that exact error as "not
  // supported" so tests can land before the feature is fully implemented.
  ...
  Ok(scope.heap().get_string(message_s)?.to_utf8_lossy() == ASYNC_GENERATOR_UNIMPLEMENTED_MESSAGE)
}
