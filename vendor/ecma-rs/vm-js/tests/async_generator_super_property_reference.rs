...
#[test]
fn async_generator_arrow_captures_lexical_super_and_observes_dynamic_prototype_across_yield_star(
) -> Result<(), VmError> {
  let mut rt = new_runtime();
...
#[test]
fn async_generator_arrow_captures_lexical_super_and_observes_dynamic_prototype_across_await(
) -> Result<(), VmError> {
  let mut rt = new_runtime();
  ...
  rt.exec_script(
    r#"
      ...
      async function f() {
        const it = (new D()).gen();
        // Start the generator and let it suspend at `await`.
        const p = it.next();
        Object.setPrototypeOf(D.prototype, proto2);
        const r0 = await p;
