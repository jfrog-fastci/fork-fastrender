...
#[test]
fn module_evaluate_supports_compiled_modules_with_function_decls() -> Result<(), VmError> {
  let (mut vm, mut heap, mut realm) = new_vm_heap_realm()?;
  let mut hooks = MicrotaskQueue::new();
  let mut host = ();
  ...
  graph.add_module_with_specifier(
    "a.js",
    SourceTextModuleRecord::parse_compiled(
      &mut heap,
      "a.js",
      r#"
        function f(){ return 1; }
...
#[test]
fn module_evaluate_supports_compiled_default_export_named_function_decl() -> Result<(), VmError> {
  let (mut vm, mut heap, mut realm) = new_vm_heap_realm()?;
  let mut hooks = MicrotaskQueue::new();
  let mut host = ();
  ...
  let Value::String(name) = ns_get(&mut vm, &mut host, &mut hooks, &mut scope, ns_b, "n")? else {
    panic!("expected b.n to be a string");
  };
  assert_eq!(scope.heap().get_string(name)?.to_utf8_lossy(), "foo");
