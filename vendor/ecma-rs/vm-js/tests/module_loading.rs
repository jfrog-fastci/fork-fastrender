...
fn req(specifier: &str, attributes: Vec<ImportAttribute>) -> ModuleRequest {
  ModuleRequest::new(JsString::from_str(specifier).unwrap(), attributes)
}
...
fn module_requests_equal_checks_specifier_and_attributes() {
  ...
  let different_specifier = req("./bar.mjs", vec![a_type_json.clone()]);
  assert!(!module_requests_equal(&left, &different_specifier));

  // Different attributes => not equal.
  let different_attrs = req("./foo.mjs", vec![a_type_css]);
