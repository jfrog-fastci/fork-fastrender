...
This single pinned file is the source of truth for both:

* RegExp `Canonicalize` (`/iu` and `/iv`) via `regexp_case_fold`, and
* `scf(cp)` used by `v`-mode `MaybeSimpleCaseFolding`.
...
* `vendor/ecma-rs/vm-js/src/regexp_case_folding.rs` exposes `regexp_case_fold`, a thin wrapper used by
  RegExp `Canonicalize` (`/iu` and `/iv`). It delegates to the shared `scf` table below.
 ...
 * Case folding:
   ...
   * `vendor/ecma-rs/vm-js/src/regexp_case_folding.rs` — `regexp_case_fold` wrapper around `scf`,
     used by RegExp `Canonicalize` (`/iu` and `/iv`).
