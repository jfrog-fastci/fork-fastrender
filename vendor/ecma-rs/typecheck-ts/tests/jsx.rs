...
#[test]
fn jsx_discriminated_union_props_contextual_typing_with_intrinsic_key() {
  let mut options = CompilerOptions::default();
  options.no_default_lib = true;
  options.jsx = Some(JsxMode::React);
  options.no_implicit_any = true;
...
"#;
  ...
  assert!(
    diagnostics.is_empty(),
    "expected no diagnostics (including implicit any) for discriminated union JSX props with intrinsic key, got {diagnostics:?}"
  );
...
type Props =
  ...
  | { kind: "b"; onClick: (ev: { y: string }) => void };
function Foo(props: Props): JSX.Element { return null as any; }
<Foo key="k" kind="a" onClick={(ev) => { const s = ev.y; }} />;
"#;
  let host = TestHost::new(options)
    .with_lib(jsx)
    .with_file(entry.clone(), negative);
  ...
  assert!(
    diagnostics
      .iter()
      .any(|d| d.code.as_str() == codes::PROPERTY_DOES_NOT_EXIST.as_str()),
    "expected PROPERTY_DOES_NOT_EXIST (TS2339) for wrong discriminated union JSX prop usage with intrinsic key, got {diagnostics:?}"
  );
}

#[test]
fn jsx_discriminated_union_props_contextual_typing_from_const_member_expr() {
  let mut options = CompilerOptions::default();
  options.no_default_lib = true;
  options.jsx = Some(JsxMode::React);
  options.no_implicit_any = true;
...
type Props =
  ...
  | { kind: "b"; onClick: (ev: { y: string }) => void };
function Foo(props: Props): JSX.Element { return null as any; }
<Foo kind={Kind.A} onClick={(ev) => { const n: number = ev.x; }} />;
<Foo kind={Kind.B} onClick={(ev) => { const s: string = ev.y; }} />;
"#;
  let host = TestHost::new(options.clone())
    .with_lib(jsx.clone())
    .with_file(entry.clone(), source);
  ...
  assert!(
    diagnostics.is_empty(),
    "expected no diagnostics (including implicit any) for discriminated union JSX props with const member expr discriminant, got {diagnostics:?}"
  );
...
type Props =
  ...
  | { kind: "b"; onClick: (ev: { y: string }) => void };
function Foo(props: Props): JSX.Element { return null as any; }
<Foo kind={Kind.A} onClick={(ev) => { const s = ev.y; }} />;
"#;
  let host = TestHost::new(options)
    .with_lib(jsx)
    .with_file(entry.clone(), negative);
  ...
  assert!(
    diagnostics
      .iter()
      .any(|d| d.code.as_str() == codes::PROPERTY_DOES_NOT_EXIST.as_str()),
    "expected PROPERTY_DOES_NOT_EXIST (TS2339) for wrong discriminated union JSX prop usage with const member expr discriminant, got {diagnostics:?}"
  );
}
