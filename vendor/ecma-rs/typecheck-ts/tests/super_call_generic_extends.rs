...
#[test]
fn super_call_in_derived_constructor_instantiates_extends_type_args() {
  ...
  let source = r#"
class Base<T> { constructor(x: T) {} }
class Derived extends Base<number> { constructor() { super("no"); } }
"#;
...
#[test]
fn super_call_in_derived_constructor_records_instantiated_call_signature() {
  let mut host = MemoryHost::with_options(CompilerOptions {
    no_default_lib: true,
    ..CompilerOptions::default()
  });
  ...
  let source = r#"
class Base<T> { constructor(x: T) {} }
class Derived extends Base<number> { constructor() { super(1); } }
"#;
  host.insert(file.clone(), Arc::from(source));
