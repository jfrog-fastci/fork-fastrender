...
#[test]
fn optional_chain_call_chain_adds_undefined() {
  let src = r#"
function f(x) {
  const y = x?.a.b();
  return y;
...
#[test]
fn optional_chain_call_chain_truthiness_narrows_base() {
  let src = r#"
function f(x) {
  if (x?.a.b()) {
    return x;
...
#[test]
fn optional_chain_member_chain_equality_narrows_base() {
  let src = r#"
function f(x, y) {
  if (x?.a.b === y) {
    return x;
...
"#;
  ...
  let mut initial = HashMap::new();
  initial.insert(name_id(lowered.names.as_ref(), "x"), init_ty);
  initial.insert(name_id(lowered.names.as_ref(), "y"), prim.number);
...
#[test]
fn optional_chain_call_chain_equality_narrows_base() {
  let src = r#"
function f(x, y) {
  if (x?.a.b() === y) {
    return x;
...
#[test]
fn optional_chain_call_chain_inequality_undefined_narrows_base() {
  let src = r#"
function f(x) {
  if (x?.a.b() !== undefined) {
    return x;
...
"#;
  ...
  assert_eq!(
    TypeDisplay::new(&store, ret_types[1]).to_string(),
    TypeDisplay::new(&store, prim.null).to_string()
  );
}
...
"#;
  ...
  initial.insert(
    name_id(lowered.names.as_ref(), "x"),
    store.union(vec![callable, prim.null]),
  );
...
#[test]
fn optional_call_does_not_add_undefined_when_base_non_nullish() {
  let src = r#"
function f(x) {
  const y = x?.();
  return y;
...
"#;
  ...
  let mut initial = HashMap::new();
  initial.insert(name_id(lowered.names.as_ref(), "x"), callable);
  ...
  assert_eq!(
    TypeDisplay::new(&store, res.return_types()[0]).to_string(),
    TypeDisplay::new(&store, prim.number).to_string()
  );
}
