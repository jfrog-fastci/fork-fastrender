...
#[test]
fn skip_lib_check_keeps_dts_module_resolution_diagnostics() {
  ...
  assert!(
    ...
    "expected unresolved module diagnostics from .d.ts (import/export specifiers) even when skip_lib_check is enabled, got {diagnostics:?}"
  );
  assert!(
    !diagnostics.iter().any(|diag| {
...
#[test]
fn skip_lib_check_keeps_dts_triple_slash_reference_diagnostics() {
  let lib_key = FileKey::new("broken.d.ts");
  let entry_key = FileKey::new("entry.ts");
...
#[test]
fn skip_lib_check_keeps_dts_triple_slash_reference_types_diagnostics() {
  let lib_key = FileKey::new("broken.d.ts");
  let entry_key = FileKey::new("entry.ts");
...
#[test]
fn skip_lib_check_keeps_dts_triple_slash_reference_lib_diagnostics() {
  let lib_key = FileKey::new("broken.d.ts");
  let entry_key = FileKey::new("entry.ts");
...
#[test]
fn skip_lib_check_does_not_suppress_dts_parse_errors() {
  // `skipLibCheck` suppresses semantic diagnostics from `.d.ts` files, but
  // TypeScript still reports syntax/parse errors in declaration files.
  //
  // `typecheck-ts` parse errors use `PS*` codes from `parse-js`, and should not
  // be filtered out.
  let lib_key = FileKey::new("broken.d.ts");
  let entry_key = FileKey::new("entry.ts");
  let lib_source = "declare const value: ;";
  ...
  assert!(
    diagnostics
      .iter()
      .any(|diag| diag.primary.file == lib_id && diag.code.as_str().starts_with("PS")),
    "expected parse error diagnostic from .d.ts even when skip_lib_check is enabled, got {diagnostics:?}"
  );
}

#[test]
fn skip_lib_check_does_not_suppress_user_ts_diagnostics() {
  let lib_key = FileKey::new("broken.d.ts");
  let entry_key = FileKey::new("entry.ts");
  ...
  host.add_lib(LibFile {
    ...
    name: Arc::from("broken.d.ts"),
    kind: FileKind::Dts,
    text: Arc::from("declare const value: MissingType;"),
  });
  host.insert(entry_key.clone(), "const n: number = \"oops\";");
  let program = Program::new(host, vec![entry_key.clone()]);
  ...
  assert!(
    ...
    "expected TS2322 diagnostics in user .ts sources to remain visible when skip_lib_check is enabled, got {diagnostics:?}"
  );
}
