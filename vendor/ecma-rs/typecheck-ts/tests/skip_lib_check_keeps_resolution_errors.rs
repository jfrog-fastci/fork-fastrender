...
#[test]
fn skip_lib_check_keeps_resolution_errors_in_dts_files() {
...
"#;
  ...
  assert!(
    !diagnostics
      .iter()
      .any(|diag| diag.primary.file == lib_id && diag.code.as_str() == codes::UNRESOLVED_TYPE_REFERENCE.as_str()),
    "expected semantic diagnostics from .d.ts to be suppressed when skip_lib_check is enabled, got {diagnostics:?}"
  );
}
