...
impl<'a> Parser<'a> {
  ...
  /// Execute `f` while treating identifier references to `arguments` as a syntax error in class
  /// initialization code.
  ///
  /// This models the `ContainsArguments` early errors for:
  /// - class field initializers, and
  /// - `static {}` initialization blocks.
  ///
  /// The restriction is cleared by nested non-arrow function bodies via
  /// [`Parser::with_arguments_bound_in_class_init`], matching the grammar's function boundary.
  pub(crate) fn with_disallow_arguments_in_class_init<R>(
    &mut self,
    f: impl FnOnce(&mut Self) -> SyntaxResult<R>,
  ) -> SyntaxResult<R> {
    ...
    // class is nested inside a non-arrow function. Only nested non-arrow functions within the
    // initializer/block should re-enable it.
    self.arguments_allowed = 0;
