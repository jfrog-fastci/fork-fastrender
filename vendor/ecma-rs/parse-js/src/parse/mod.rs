...
impl<'a> Parser<'a> {
  ...
  /// Execute `f` within a "class initialization" parsing context where identifier references to
  /// `arguments` are disallowed.
  ///
  /// This implements the `ContainsArguments` early errors for:
  /// - class field initializers, and
  /// - `static {}` initialization blocks.
  ///
  /// Nested non-arrow function bodies temporarily clear this restriction via
  /// [`Parser::with_arguments_bound_in_class_init`], matching the grammar's function-body boundary.
  pub(crate) fn with_disallow_arguments_in_class_init<R>(
    ...
    // `arguments` is syntactically disallowed at the top level of class initializers even when a
    // surrounding function scope would otherwise provide an `arguments` binding. Only nested
    // non-arrow functions within the initializer itself should re-enable it.
    self.arguments_allowed = 0;
    let res = f(self);
  ...
  /// Validate that `name` is not the disallowed identifier reference `arguments` in a class
  /// initializer context.
  ///
  /// When parsing class field initializers and `static {}` blocks, `arguments` is a syntax error
  /// unless it is provided by a nested non-arrow function scope.
  pub(crate) fn validate_arguments_not_disallowed_in_class_init(
    ...
    Err(loc.error(
      SyntaxErrorType::ExpectedSyntax("'arguments' is not allowed in class initializer"),
      None,
    ))
  }
