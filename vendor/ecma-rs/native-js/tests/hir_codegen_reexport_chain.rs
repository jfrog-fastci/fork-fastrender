...
#[test]
fn hir_codegen_supports_multi_hop_reexport_chains() {
...
"#;
  ...
  let c_src = r#"
export { foo, x as y } from "./b.ts";
"#;

  let main_src = r#"
import { foo, y } from "./c.ts";
...
"#;
  ...
  host.insert(c_key.clone(), c_src);
  host.insert(main_key.clone(), main_src);
  host.link(b_key.clone(), "./a.ts", a_key.clone());
  host.link(c_key.clone(), "./b.ts", b_key.clone());
  host.link(main_key.clone(), "./c.ts", c_key.clone());
  ...
  let file_c = program.file_id(&c_key).expect("file c id");
  let file_main = program.file_id(&main_key).expect("file main id");
  let strict_diags = strict::validate(&program, &[file_a, file_b, file_c, file_main]);
