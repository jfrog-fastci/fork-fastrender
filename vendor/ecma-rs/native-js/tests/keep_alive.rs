...
fn keep_alive_keeps_owner_in_statepoint_live_set() {
  ...
  let before = r#"
declare void @may_gc()
declare void @rt_keep_alive_gc_ref_gc(ptr addrspace(1))
...
"#;
  ...
  let func = {
    ...
    for line in after.lines() {
      if !in_func && line.contains("define") && line.contains("@test") {
        in_func = true;
      }
      if in_func {
        out.push(line);
    ...
    assert!(in_func, "function @test not found in IR:\n{after}");
    out.join("\n")
  };
  ...
  assert!(
    func.contains("rt_keep_alive_gc_ref_gc"),
    "expected keep-alive call to remain in function IR:\n{func}"
  );
