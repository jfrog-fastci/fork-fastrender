...
#[test]
fn hir_codegen_fn_return_gcptr_array() {
  ...
  let mk_sig = ir
    .lines()
    .find(|line| line.contains("define") && line.contains("_mk("))
    .expect("expected `mk` definition in IR");
  assert!(
    mk_sig.contains("ptr addrspace(1)"),
    "expected `mk` signature to return `ptr addrspace(1)`:\n{mk_sig}\n\nIR:\n{ir}"
  );
