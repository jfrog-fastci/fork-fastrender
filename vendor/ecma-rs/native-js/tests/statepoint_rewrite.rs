...
#[test]
fn invoke_indirect_scalar_return_has_elementtype_and_gc_result() {
  // Like `invoke_scalar_return_uses_gc_result`, but uses an *indirect* invoke through a runtime
  // `ptr` function pointer. LLVM 18 requires the rewritten statepoint's callee operand to carry
  // `elementtype(<fn-ty>)` even in the indirect case.
  let before = r#"
  declare i64 @bar()
  declare i64 @baz()
  ...
  define i64 @test(i1 %cond) gc "coreclr" personality ptr @dummy_personality {
  entry:
    %fp = select i1 %cond, ptr @bar, ptr @baz
    %x = invoke i64 %fp()
            to label %cont unwind label %lpad
  ...
  let statepoint_line = func
    ...
    .unwrap_or_else(|| panic!("missing invoke gc.statepoint call in function:\n{func}"));
  assert!(
    statepoint_line.contains("ptr elementtype(i64 ()) %fp"),
    "expected indirect invoke callee to be annotated with elementtype(i64 ()) on %fp, got:\n{statepoint_line}\n\n{func}"
  );
  let statepoint_token = assigned_ssa(statepoint_line)
    .unwrap_or_else(|| panic!("unexpected statepoint line (not an assignment): {statepoint_line}"));

  assert!(
    !func.contains("invoke i64 %fp()"),
    "expected indirect invoke to be rewritten (no direct `invoke i64 %fp()`):\n{func}"
  );
...
#[test]
fn rewrite_statepoints_rejects_musttail_call_in_gc_function() {
  init_llvm();
...
"#;
  ...
  let err = passes::rewrite_statepoints_for_gc(&module, &tm).unwrap_err();
  assert!(
    matches!(err, passes::PassError::UnsupportedMustTailCallInGcFunction { .. }),
    "expected UnsupportedMustTailCallInGcFunction, got: {err}"
  );
}
