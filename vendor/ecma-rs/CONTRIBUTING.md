...
This repository pins the Rust toolchain (including `rustfmt`) via `rust-toolchain.toml` so formatting output is reproducible across CI and developer machines. If `bash scripts/cargo_agent.sh fmt --all --check` fails unexpectedly, ensure you're using the pinned toolchain.
...
- Running a cheap sanity check (`bash scripts/cargo_agent.sh check -p typecheck-ts-harness --locked`)
...
To apply formatting fixes locally (as opposed to checking formatting), run `./format` (it runs `bash scripts/cargo_agent.sh fmt --all` and also runs prettier when `npx` is available).
