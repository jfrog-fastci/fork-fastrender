...
#[test]
fn difftsc_types_versions_cases_match_baselines() {
  let (_dir, suite) =
    common::temp_difftsc_suite(&["types_versions_subpath", "types_versions_first_match_order"]);
  ...
  for name in ["types_versions_subpath", "types_versions_first_match_order"] {
    let case = results
      .iter()
