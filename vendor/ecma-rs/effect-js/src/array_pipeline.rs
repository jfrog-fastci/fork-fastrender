...
mod typed {
  ...
  use hir_js::{BinaryOp, ExprKind, FunctionBody, PatKind};
  use hir_js::{StmtId, StmtKind, TypeArenas, TypeExprId, TypeExprKind};
  ...
  fn callsite_info_for_callback(
    ...
    let associative =
      callback.and_then(|_| infer_associative_inline_callback(lowered, body, analysis_expr));
  ...
  fn resolve_callback_for_analysis(
    lowered: &hir_js::LowerResult,
    body: BodyId,
    callback_expr: ExprId,
  ) -> ExprId {
  ...
  fn infer_associative_inline_callback(
    ...
    let ExprKind::Binary { op, left, right } = &cb_body_data.exprs.get(expr.0 as usize)?.kind
    else {
      return None;
    };
  ...
  pub fn plan_array_chains_typed(
    ...
    for pat in patterns {
      ...
      let mut stages: Vec<ArrayStage> =
        Vec::with_capacity(ops.len() + terminal.as_ref().map(|_| 1).unwrap_or(0));
      ...
      for op in ops {
        ...
        let output_len = api
          .map(crate::properties::output_length_relation)
          .unwrap_or(OutputLengthRelation::Unknown);

        let parallelizable = if is_short_circuit(kind) {
          api.is_some_and(|api| api.parallelizable == Some(true))
            && callsite.callback_may_throw == Some(false)
        } else {
      ...
      for idx in 0..stages.len() {
        ...
        if crate::properties::fusable_with(cur_api, next_api)
          || crate::properties::fusable_with(next_api, cur_api)
        {
...
#[cfg(feature = "typed")]
pub use typed::plan_array_chains_typed;
