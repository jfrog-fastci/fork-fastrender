...
const INDEX_TS_MIXED_RECV_KEY: &str = r#"
export {};
...
const obj: Obj = { m: new Map(), k: "x" };
const v = obj.m.has(obj.k) ? obj["m"].get(obj["k"]) : 123;
"#;
...
#[test]
fn recognizes_map_get_or_default_conditional_with_mixed_member_syntax() {
  let index_key = FileKey::new("index.ts");

  let mut host = es2015_host();
  host.insert(index_key.clone(), INDEX_TS_MIXED_RECV_KEY);
  ...
  assert_eq!(matches.len(), 1, "expected one MapGetOrDefault pattern");
  let (_map_expr, _key_expr, default_expr) = matches[0];
  assert_eq!(
    default_expr, alternate,
    "expected MapGetOrDefault default to point at conditional alternate"
  );
