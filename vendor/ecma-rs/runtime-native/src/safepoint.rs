...
fn maybe_call_safepoint_hook(cursor: FrameCursor) {
  ...
  // Treat the hook as `extern "C-unwind"` so we can safely catch panics without unwinding across a
  // C ABI boundary.
  crate::ffi::abort_on_callback_panic(|| unsafe {
    let hook: extern "C-unwind" fn(FrameCursor) = std::mem::transmute(ptr);
    hook(cursor);
  });
}
