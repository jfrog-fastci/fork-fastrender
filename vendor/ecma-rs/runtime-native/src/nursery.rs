...
impl NurserySpace {
  ...
  pub fn new(size_bytes: usize) -> io::Result<Self> {
    ...
    #[cfg(not(unix))]
    {
      let layout = std::alloc::Layout::from_size_align(size_bytes, page)
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "invalid nursery layout"))?;
...
impl ThreadNursery {
  ...
  /// trigger GC.
  #[inline]
  pub fn alloc(&mut self, size: usize, align: usize, nursery: &NurserySpace) -> Option<*mut u8> {
