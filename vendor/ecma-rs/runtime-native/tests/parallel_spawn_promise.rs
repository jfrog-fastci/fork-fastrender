use runtime_native::abi::{LegacyPromiseRef, PromiseRef, RtCoroStatus, RtCoroutineHeader, RtShapeDescriptor, RtShapeId, ValueRef};
...
fn ensure_shape_table() {
  SHAPE_TABLE_ONCE.call_once(|| unsafe {
    static SHAPES: [RtShapeDescriptor; 5] = [
      ...
      // GC-managed promise object used for testing `rt_promise_payload_ptr` inline-payload semantics.
      RtShapeDescriptor {
        // The test promise object layout is `PromiseHeader + u32 payload` and must be padded to the
        // GC object alignment.
        size: ((mem::size_of::<PromiseHeader>() + mem::size_of::<u32>() + 7) & !7) as u32,
        align: 8,
        flags: 0,
        ptr_offsets: EMPTY_PTR_OFFSETS.as_ptr(),
        ptr_offsets_len: 0,
  ...
  unsafe {
    match (*coro).header.state {
      0 => {
        ...
        runtime_native::rt_coro_await_legacy(&mut (*coro).header, legacy_from_promise_ref(promise), 1);
...
#[test]
fn promise_payload_ptr_returns_inline_payload_for_gc_managed_promises() {
  let _rt = TestRuntimeGuard::new();

  ensure_shape_table();

  // Allocate a minimal native async-ABI `Promise<T>` object in the GC heap and verify that the
  // exported `rt_promise_payload_ptr` helper points at the inline payload immediately after the
  // `PromiseHeader` prefix.
  let size = ((mem::size_of::<PromiseHeader>() + mem::size_of::<u32>() + 7) & !7) as usize;
  let mut obj = runtime_native::rt_alloc_pinned(size, RtShapeId(5));

  // Root it so the GC cannot reclaim the object while we use raw pointers from Rust (Rust stacks
  // are not stackmap-scanned).
  let handle = runtime_native::rt_gc_register_root_slot(&mut obj as *mut *mut u8);
  ...
  let expected = unsafe { obj.add(mem::size_of::<PromiseHeader>()) };
  assert_eq!(payload, expected);

  // Sanity: the payload pointer is writable.
  unsafe {
    *(payload as *mut u32) = 123;
    assert_eq!(*(payload as *const u32), 123);
  }
