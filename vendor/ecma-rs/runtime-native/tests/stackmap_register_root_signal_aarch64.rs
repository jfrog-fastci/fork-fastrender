...
unsafe extern "C" fn sigtrap_handler(
  ...
  // following instruction (here: `brk #0x1`, i.e. `llvm.trap`).
  //
  ...
  let (trap_pc, callsite) = stackmaps
    .lookup(pc)
    .map(|c| (pc, c))
    .or_else(|| pc.checked_sub(4).and_then(|p| stackmaps.lookup(p).map(|c| (p, c))))
    .expect("callsite record for SIGTRAP PC");
  ...
  // Skip the `brk` instruction emitted by `llvm.trap` so execution resumes.
  ctx
    .set_dwarf_reg_u64(DWARF_REG_IP, trap_pc + 4)
    .expect("set PC");
...
fn signal_handler_can_rewrite_register_root_via_stackmaps() {
  ...
  // Minimal IR that:
  // - records one live value in a stackmap (ideally in a register), and
  // - immediately traps so our SIGTRAP handler runs at the stackmap PC.
  fs::write(
    &ll,
    r#"
...
"#,
  ...
  unsafe {
    ...
    STACKMAPS
      .set(stackmaps)
      .ok()
      .expect("STACKMAPS initialized once");

    // Install SIGTRAP handler with SA_SIGINFO so we can modify the ucontext.
    ...
    assert!(HANDLED.load(Ordering::Acquire), "SIGTRAP handler did not run");
