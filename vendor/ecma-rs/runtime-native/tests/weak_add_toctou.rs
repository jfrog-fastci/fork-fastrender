...
#[test]
fn weak_add_from_raw_pointer_is_moving_gc_safe_under_lock_contention() {
  let _rt = TestRuntimeGuard::new();
  threading::register_current_thread(ThreadKind::Main);

  runtime_native::gc::weak::debug_clear_global_weak_handles_for_tests();
  let (slots, free) = runtime_native::gc::weak::debug_global_weak_handles_table_sizes();
  assert_eq!(
    slots, free,
    "expected no live global weak handles after test runtime reset"
  );
  ...
  std::thread::scope(|scope_threads| {
    // Thread A holds the global weak-handle table lock.
    let (a_locked_tx, a_locked_rx) = mpsc::channel::<()>();
    let (a_release_tx, a_release_rx) = mpsc::channel::<()>();

    // Thread C attempts to create a weak handle from a raw GC pointer while the lock is held.
    ...
    a_locked_rx
      ...
      .expect("thread A should acquire the global weak-handle table lock");
    ...
    scope_threads.spawn(move || {
      ...
      let handle = runtime_native::rt_weak_add(ptr);
    ...
    // Start thread C's weak-add attempt (it should block on the weak-handle lock).
    ...
    loop {
      ...
      if start.elapsed() > TIMEOUT {
        panic!("thread C did not enter a GC-safe region while blocked on the weak-handle table lock");
      }
    ...
    // Release the lock so thread C can finish creating the weak handle.
    a_release_tx.send(()).unwrap();

    let handle = c_done_rx
      .recv_timeout(TIMEOUT)
      .expect("thread C should finish creating the weak handle");

    assert_eq!(
      runtime_native::rt_weak_get(handle) as usize,
      relocated as usize,
      "weak handle must resolve to the relocated pointer, not the stale nursery address"
    );

    runtime_native::rt_weak_remove(handle);
    let (slots, free) = runtime_native::gc::weak::debug_global_weak_handles_table_sizes();
    assert_eq!(slots, free, "expected no live weak handles after removal");
  });
