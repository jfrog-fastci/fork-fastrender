...
#[test]
fn gc_collect_evacuates_nursery_child() {
  let _rt = TestRuntimeGuard::new();
  if std::env::var_os("RT_GC_COLLECT_CHILD").is_none() {
    return;
  }

  ensure_shape_table();

  // Force the thread-local nursery TLAB to have a large cursor offset, so we
  // can detect whether it is invalidated after `rt_gc_collect` resets the
  // nursery.
  for _ in 0..4096 {
  ...
  // Allocate a rooted young object.
  let mut root = rt_alloc(256, RtShapeId(1));
  let root_before = root as usize;
  assert!(
  ...
  assert!(
    ...
    "expected rooted object to be evacuated to old-gen by rt_gc_collect"
  );

  // Allocation after GC should restart from the beginning of the nursery (TLAB
  // invalidated).
  let after = rt_alloc(256, RtShapeId(1)) as usize;
  assert!(
    (young_start as usize..young_end as usize).contains(&after),
    "expected post-GC allocation to be in the nursery"
  );
  assert!(
    after < young_start as usize + 64 * 1024,
    "expected post-GC allocation to restart near nursery start (got offset {})",
    after - young_start as usize
  );
...
#[test]
fn gc_collect_evacuates_nursery() {
  let exe = std::env::current_exe().expect("current_exe");

  let status = Command::new(exe)
    .env("RT_GC_COLLECT_CHILD", "1")
    .arg("--exact")
    .arg("gc_collect_evacuates_nursery_child")
    .status()
    .expect("spawn child");
