...
#[test]
fn teardown_clears_registry_but_keeps_pins_until_cancel_ack_for_read() {
  let _rt = TestRuntimeGuard::new();
  let io_rt = IoRuntime::new();

  // Keep the write end open but never write to it, so reads block in poll().
  let (rfd, wfd) = pipe();
  ...
  let debug = IoOpDebugHooks::pause_before_finish();
  let promise = io_rt
    .read_with_debug_hooks(rfd, &view, 0..view.length(), &[root_obj], Some(debug.clone()))
    .unwrap();
  ...
  // Trigger teardown while the read is blocked.
  io_rt.teardown();
  assert_eq!(io_rt.debug_registry_len(), 0, "teardown must clear the registry immediately");
...
#[test]
fn teardown_detaches_queued_completion_tasks_for_read() {
  let _rt = TestRuntimeGuard::new();
  let io_rt = IoRuntime::new();

  let (rfd, wfd) = pipe();

  // Write one byte so the read can complete and enqueue the completion task.
  let byte = [0xAAu8; 1];
  let rc = unsafe { libc::write(wfd.as_raw_fd(), byte.as_ptr().cast::<libc::c_void>(), 1) };
  assert_eq!(rc, 1);

  let buf = ArrayBuffer::from_bytes(vec![0u8]).unwrap();
  let view = Uint8Array::view(&buf, 0, buf.byte_len()).unwrap();
  let root_obj = Box::into_raw(Box::new(0u8)) as *mut u8;
  let debug = IoOpDebugHooks::pause_before_finish();
  let promise = io_rt
...
#[test]
fn teardown_keeps_backing_store_pins_until_cancel_ack_for_read_iovecs() {
  let _rt = TestRuntimeGuard::new();
  let io_rt = IoRuntime::new();

  // Keep the write end open but never write, so the read eventually blocks.
  let (rfd, wfd) = pipe();
  ...
  let debug = IoOpDebugHooks::pause_before_finish();
  let promise = io_rt
    .read_iovecs_with_debug_hooks(rfd, iovecs, &[root_obj], Some(debug.clone()))
    .unwrap();
