...
use runtime_native::rt_queue_microtask_rooted;
...
extern "C" fn record_ptr_and_magic(data: *mut u8) {
  // Observe the pointer passed through `async_rt::gc::Root` (stored in the microtask queue).
...
fn root_new_unchecked_is_safe_under_handle_table_lock_contention() {
  ...
  std::thread::scope(|scope| {
    ...
    // Thread C attempts to allocate a rooted microtask while the lock is held; it should block in
    ...
    scope.spawn(move || {
      ...
      rt_queue_microtask_rooted(record_ptr_and_magic, ptr);
    ...
    // Wait for thread C to finish the enqueue call (it should unblock after the GC completes and
    ...
    loop {
      ...
      if start.elapsed() > TIMEOUT {
        panic!("timeout waiting for rooted enqueue call to complete after GC");
      }
      threading::safepoint_poll();
      std::thread::yield_now();
    }
  ...
  // Run the microtask to record the pointer it observed.
  ...
  loop {
    ...
    assert!(Instant::now() < deadline, "timeout waiting for microtask to run");
    // Drain microtasks only (non-blocking) but yield to allow other threads to run too.
    std::thread::yield_now();
  }
}
