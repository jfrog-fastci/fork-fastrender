...
mod tests {
  ...
  fn string_object_prefers_domstring_over_frozen_array_overload_without_probing_iterator() {
    ...
    // If overload resolution tried to probe @@iterator for FrozenArray matching, it would trigger
    // this getter and throw. The special-case (d) must treat String objects as strings when a
    // string overload is present.
    let throwing_getter = rt
      .alloc_function_value(|rt, _this, _args| Err(rt.throw_type_error("getter must not run")))
      .unwrap();
    let iter_key = rt.symbol_iterator().unwrap();
  ...
  #[test]
  fn string_object_prefers_domstring_over_async_sequence_overload_without_probing_iterators() {
    let mut rt = VmJsRuntime::new();

    // Overloads: f(async sequence<any>) vs f(DOMString)
    let overloads = vec![
      OverloadSig {
        args: vec![OverloadArg {
  ...
  #[test]
  fn string_object_prefers_domstring_over_object_overload() {
    let mut rt = VmJsRuntime::new();

    // Overloads: f(object) vs f(DOMString)
    let overloads = vec![
      OverloadSig {
        args: vec![OverloadArg {
          ty: IdlType::Object,
  ...
  #[test]
  fn string_object_prefers_enum_over_sequence_overload_without_probing_iterator() {
    let mut rt = VmJsRuntime::new();

    // Overloads: f(sequence<any>) vs f(MyEnum)
    let overloads = vec![
      OverloadSig {
        args: vec![OverloadArg {
          ty: IdlType::Sequence(Box::new(IdlType::Any)),
    ...
    // If overload resolution tried to probe @@iterator for sequence matching, it would trigger this
    // getter and throw. The special-case (d) must treat String objects as strings when a string
    // overload (including enums) is present.
    let throwing_getter = rt
      .alloc_function_value(|rt, _this, _args| Err(rt.throw_type_error("getter must not run")))
      .unwrap();
    let iter_key = rt.symbol_iterator().unwrap();
  ...
  #[test]
  fn string_object_prefers_enum_over_frozen_array_overload_without_probing_iterator() {
    let mut rt = VmJsRuntime::new();

    // Overloads: f(FrozenArray<any>) vs f(MyEnum)
    let overloads = vec![
      OverloadSig {
        args: vec![OverloadArg {
          ty: IdlType::FrozenArray(Box::new(IdlType::Any)),
    ...
    // If overload resolution tried to probe @@iterator for FrozenArray matching, it would trigger
    // this getter and throw. The special-case (d) must treat String objects as strings when a
    // string overload (including enums) is present.
    let throwing_getter = rt
      .alloc_function_value(|rt, _this, _args| Err(rt.throw_type_error("getter must not run")))
      .unwrap();
    let iter_key = rt.symbol_iterator().unwrap();
  ...
  #[test]
  fn string_object_prefers_enum_over_record_overload_without_probing_properties() {
    let mut rt = VmJsRuntime::new();

    // Overloads: f(record<DOMString, any>) vs f(MyEnum)
    let overloads = vec![
      OverloadSig {
        args: vec![OverloadArg {
          ty: IdlType::Record(
    ...
    // If overload resolution selected the record overload, it would enumerate properties and access
    // this enumerable accessor and throw. The special-case (d) must treat String objects as strings
    // when a string overload (including enums) is present.
    let throwing_getter = rt
      .alloc_function_value(|rt, _this, _args| Err(rt.throw_type_error("getter must not run")))
      .unwrap();
    let key_value = rt.alloc_string_value("x").unwrap();
  ...
  #[test]
  fn string_object_prefers_enum_over_dictionary_overload() {
    let mut rt = VmJsRuntime::new();

    // Overloads: f(MyDict) vs f(MyEnum)
    let overloads = vec![
      OverloadSig {
        args: vec![OverloadArg {
          ty: IdlType::Named(NamedType {
  ...
  #[test]
  fn string_object_prefers_enum_over_object_overload() {
    let mut rt = VmJsRuntime::new();

    // Overloads: f(object) vs f(MyEnum)
    let overloads = vec![
      OverloadSig {
        args: vec![OverloadArg {
          ty: IdlType::Object,
  ...
  fn string_object_union_prefers_domstring_over_sequence_without_probing_iterator() {
    ...
    // One overload: f((sequence<any> or DOMString))
    ...
    // If union conversion tried to probe @@iterator for sequence conversion, it would trigger this
    // getter and throw. The special-case (d) must treat String objects as strings when a string
    // member is present.
    let throwing_getter = rt
      .alloc_function_value(|rt, _this, _args| Err(rt.throw_type_error("getter must not run")))
  ...
  #[test]
  fn string_object_union_prefers_domstring_over_frozen_array_without_probing_iterator() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((FrozenArray<any> or DOMString))
    let union_ty = IdlType::Union(vec![
    ...
    // If union conversion tried to probe @@iterator for FrozenArray conversion, it would trigger
    // this getter and throw. The special-case (d) must treat String objects as strings when a
  ...
  #[test]
  fn string_object_union_prefers_domstring_over_async_sequence_without_probing_iterators() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((async sequence<any> or DOMString))
    let union_ty = IdlType::Union(vec![
    ...
    // it would trigger these getters and throw. The special-case (d) must treat String objects as
    // strings when a string member is present.
    let throwing_getter = rt
      .alloc_function_value(|rt, _this, _args| Err(rt.throw_type_error("getter must not run")))
      .unwrap();
    let async_iter_key = rt.symbol_async_iterator().unwrap();
  ...
  #[test]
  fn string_object_union_prefers_domstring_over_object() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((object or DOMString))
    let union_ty = IdlType::Union(vec![
      IdlType::Object,
      IdlType::String(StringType::DomString),
    ]);
  ...
  #[test]
  fn string_object_union_prefers_domstring_over_dictionary() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((MyDict or DOMString))
  ...
  #[test]
  fn string_object_union_prefers_enum_over_sequence_without_probing_iterator() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((sequence<any> or MyEnum))
    let enum_ty = IdlType::Named(NamedType {
      name: "MyEnum".into(),
      kind: NamedTypeKind::Enum,
    });
    let union_ty = IdlType::Union(vec![IdlType::Sequence(Box::new(IdlType::Any)), enum_ty]);
    let overloads = vec![OverloadSig {
      args: vec![OverloadArg {
        ty: union_ty,
        optionality: Optionality::Required,
    ...
    // If union conversion tried to probe @@iterator for sequence conversion, it would trigger this
    // getter and throw. The special-case (d) must treat String objects as strings when a string
    // member (including enums) is present.
    let throwing_getter = rt
      .alloc_function_value(|rt, _this, _args| Err(rt.throw_type_error("getter must not run")))
      .unwrap();
    let iter_key = rt.symbol_iterator().unwrap();
  ...
  #[test]
  fn string_object_union_prefers_enum_over_frozen_array_without_probing_iterator() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((FrozenArray<any> or MyEnum))
    let enum_ty = IdlType::Named(NamedType {
      name: "MyEnum".into(),
      kind: NamedTypeKind::Enum,
    });
    let union_ty = IdlType::Union(vec![IdlType::FrozenArray(Box::new(IdlType::Any)), enum_ty]);
    let overloads = vec![OverloadSig {
      args: vec![OverloadArg {
        ty: union_ty,
        optionality: Optionality::Required,
    ...
    // If union conversion tried to probe @@iterator for FrozenArray conversion, it would trigger
    // this getter and throw. The special-case (d) must treat String objects as strings when a
    // string member (including enums) is present.
    let throwing_getter = rt
      .alloc_function_value(|rt, _this, _args| Err(rt.throw_type_error("getter must not run")))
      .unwrap();
    let iter_key = rt.symbol_iterator().unwrap();
  ...
  #[test]
  fn string_object_union_prefers_enum_over_object() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((object or MyEnum))
    let enum_ty = IdlType::Named(NamedType {
      name: "MyEnum".into(),
      kind: NamedTypeKind::Enum,
    });
    let union_ty = IdlType::Union(vec![IdlType::Object, enum_ty]);
    let overloads = vec![OverloadSig {
      args: vec![OverloadArg {
        ty: union_ty,
        optionality: Optionality::Required,
  ...
  #[test]
  fn string_object_union_prefers_enum_over_dictionary() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((MyDict or MyEnum))
    let enum_ty = IdlType::Named(NamedType {
      name: "MyEnum".into(),
      kind: NamedTypeKind::Enum,
    });
    ...
    let union_ty = IdlType::Union(vec![dict_ty, enum_ty]);
    let overloads = vec![OverloadSig {
      args: vec![OverloadArg {
        ty: union_ty,
        optionality: Optionality::Required,
  ...
  #[test]
  fn string_object_union_prefers_enum_over_record_without_probing_properties() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((record<DOMString, any> or MyEnum))
    let enum_ty = IdlType::Named(NamedType {
      name: "MyEnum".into(),
      kind: NamedTypeKind::Enum,
    });
    ...
    // If union conversion tried to treat the value as a record and enumerate properties, it would
    // access this enumerable accessor and throw.
    let throwing_getter = rt
      .alloc_function_value(|rt, _this, _args| Err(rt.throw_type_error("getter must not run")))
      .unwrap();
    let key_value = rt.alloc_string_value("x").unwrap();
  ...
  #[test]
  fn string_object_union_prefers_domstring_over_record_without_probing_properties() {
    let mut rt = VmJsRuntime::new();

    // One overload: f((record<DOMString, any> or DOMString))
    let union_ty = IdlType::Union(vec![
      IdlType::Record(
        Box::new(IdlType::String(StringType::DomString)),
        Box::new(IdlType::Any),
