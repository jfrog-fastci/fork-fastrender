...
//!
//! The spec-shaped conversion and overload-resolution algorithms live in `webidl`. This crate keeps
//! a legacy `VmJsRuntime` implementation and
...
pub trait WebIdlBindingsRuntime<Host>: WebIdlJsRuntime {
  ...
  /// (`handleEvent` for `EventListener`, `acceptNode` for `NodeFilter`, `lookupNamespaceURI` for
  /// `XPathNSResolver`, etc).
