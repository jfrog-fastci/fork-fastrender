...
#[test]
fn lowers_object_member_store_to_field_store_with_offset_meta() {
  let src = r#"
  ...
  assert_eq!(
    matches.len(),
    1,
    "expected exactly one FieldStore for obj.x, got {}",
    matches.len()
  );
  let inst = matches[0];
  let meta = inst
    ...
    .field_access
    .as_ref()
    .expect("FieldStore should carry field_access meta");
  ...
  assert!(
    ...
    "numeric field stores should not require a write barrier"
  );
}
