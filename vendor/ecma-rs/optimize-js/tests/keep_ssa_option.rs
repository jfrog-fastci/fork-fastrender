...
use optimize_js::{
  compile_source, compile_source_with_cfg_options, CompileCfgOptions, Program, TopLevelMode,
};
...
#[test]
fn default_compile_output_has_no_phi_nodes() {
  let program =
    compile_source(join_program_source(), TopLevelMode::Module, false).expect("compile");
...
#[test]
fn keep_ssa_compile_retains_phi_nodes() {
  let options = CompileCfgOptions {
    keep_ssa: true,
    ..CompileCfgOptions::default()
  };
  let program =
    compile_source_with_cfg_options(join_program_source(), TopLevelMode::Module, false, options)
      .expect("compile");
  let insts = collect_all_insts(&program);
  assert!(
...
#[test]
fn keep_ssa_compile_is_deterministic() {
  let options = CompileCfgOptions {
    keep_ssa: true,
    ..CompileCfgOptions::default()
  };
