...
use crate::{
  BlockContainerStyle, BlockItemStyle, BoxGenerationMode, BoxSizing, LayoutBlockContainer,
  TextAlign,
};
...
fn compute_inner(
  ...
  let resolved_padding = raw_padding.resolve_or_zero(Some(container_outer_width), |val, basis| {
    tree.calc(val, basis)
  });
  let resolved_border = raw_border.resolve_or_zero(Some(container_outer_width), |val, basis| {
    tree.calc(val, basis)
  });
    ...
    .filter(|item| item.position != Position::Absolute)
  {
    ...
    let width = known_dimensions.width.unwrap_or_else(|| {
      let item_x_margin_sum = item
        .margin
        .resolve_or_zero(available_space.width.into_option(), |val, basis| {
          tree.calc(val, basis)
        })
        .horizontal_axis_sum();
...
fn perform_final_layout_on_in_flow_children(
  ...
  for item in items.iter_mut() {
    if item.position == Position::Absolute {
      ...
      let item_margin = item.margin.map(|margin| {
        margin.resolve_to_option(container_outer_width, |val, basis| tree.calc(val, basis))
      });
      ...
      let x_axis_auto_margin_size = {
        let auto_margin_count =
          item_margin.left.is_none() as u8 + item_margin.right.is_none() as u8;
    ...
    .filter(|item| item.position == Position::Absolute)
  {
    ...
    let location = Point {
      ...
      y: top
        ...
        .or(
          bottom
            .map(|bottom| area_size.height - final_size.height - bottom - resolved_margin.bottom),
        )
