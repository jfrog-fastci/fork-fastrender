...
- `xtask/src/webidl_bindings_codegen.rs` (`bash scripts/cargo_agent.sh xtask webidl-bindings`)
  - generates committed Rust glue from the snapshot world:
    ...
    - `src/js/webidl/bindings/generated_legacy.rs` (legacy `webidl-js-runtime` bindings wrappers; backend `legacy`)
...
- `crates/webidl-vm-js`
  - runtime helpers for installing/invoking generated WebIDL bindings in `vm-js` realms
