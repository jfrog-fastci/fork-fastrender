...
- `vendor/ecma-rs/webidl`
  - authoritative WebIDL IR (`webidl::ir`) + spec algorithms (conversions, overload resolution)
- `vendor/ecma-rs/webidl-vm-js`
  - canonical `vm-js` adapter + shared runtime helpers for installing/invoking generated WebIDL
    bindings in `vm-js` realms
- `vendor/ecma-rs/webidl-runtime`
  - legacy heap-only WebIDL runtime adapter (used by early scaffolding and some unit tests)
  - Cargo package name: `webidl-js-runtime` (Rust crate name: `webidl_js_runtime`).
  - re-exported as `fastrender::js::webidl::legacy` while migration is in progress
- `src/js/webidl/*`
  - FastRender-specific bindings integration (realm installation, host dispatch, and re-exports)

Note: the only FastRender-local crate under `crates/` is `crates/js-wpt-dom-runner` (tooling). All
generic JS/WebIDL infrastructure should live in `vendor/ecma-rs/`.
...
- `vendor/ecma-rs/webidl-runtime` builds on `vendor/ecma-rs/vm-js`, which supports `HeapLimits`.
