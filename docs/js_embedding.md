...
- `xtask/src/webidl_bindings_codegen.rs` (`bash scripts/cargo_agent.sh xtask webidl-bindings`)
  - generates committed Rust glue under `src/js/webidl/bindings/` from the snapshot world:
    - `src/js/webidl/bindings/generated/mod.rs` (Window-facing bindings wrappers)
    - (DOM bindings are not generated by this xtask; they are implemented directly against `vm-js`
      realms in `src/js/legacy/vm_dom.rs`)
    - (the deprecated `VmJsRuntime` DOM scaffold can still be generated with
      `bash scripts/cargo_agent.sh xtask webidl-bindings --backend legacy`, but it is not committed or used by tests)
