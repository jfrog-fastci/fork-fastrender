...
- `xtask/src/webidl_bindings_codegen.rs` (`bash scripts/cargo_agent.sh xtask webidl-bindings`)
  - generates committed Rust glue from the snapshot world:
    ...
    - `src/js/webidl/bindings/generated_legacy.rs` (legacy `webidl-js-runtime` (heap-only) bindings wrappers; backend `legacy`)
...
- The legacy heap-only WebIDL runtime (Cargo package `webidl-js-runtime`, Rust crate `webidl_js_runtime`)
