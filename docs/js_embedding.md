...
- `xtask/src/webidl_bindings_codegen.rs` (`bash scripts/cargo_agent.sh xtask webidl-bindings`)
  - generates committed Rust glue from the snapshot world:
    - `src/js/webidl/bindings/generated/mod.rs` (`vm-js` realm Window/Worker bindings wrappers)
    - `src/js/webidl/bindings/generated_legacy.rs` (legacy `webidl-js-runtime` bindings wrappers; deprecated)
    - `src/js/legacy/dom_generated.rs` (legacy `VmJsRuntime` DOM scaffold; deprecated)
