...
- `FASTR_BROWSER_TRACE_OUT=/path/to/trace.json` – write a Chrome trace of the windowed `browser`
  ...
  - Use `FASTR_TRACE_MAX_EVENTS=<N>` to cap the number of trace events retained per trace (default
    200000). Excess events are dropped deterministically and counted in the trace output JSON.
...
- `FASTR_PERF_LOG=0|1` – emit a machine-readable performance log (JSONL) from the `browser` binary.
  - Default: `0` (off).
  - CLI equivalents:
    - `browser --perf-log` (logs to stdout)
    - `browser --perf-log-out <path>` (logs to a file instead of stdout; creates parent directories)
...
### Browser session file (tabs + UI config persistence)
...
- `FASTR_BROWSER_SESSION_PATH=/path/to/fastrender_session.json` – override where the browser session file is stored.
  ...
  - Note: even when tabs are not restored (e.g. `browser <url>`, `--no-restore`, or safe mode after a
...
- `FASTR_TRACE_MAX_EVENTS=<N>` – cap the number of Chrome trace events retained per trace (default 200000). This applies to both `FASTR_TRACE_OUT` (renderer pipeline) and windowed browser UI traces (`browser --trace-out` / `FASTR_BROWSER_TRACE_OUT`). Excess events are dropped deterministically and counted in the output trace JSON.
