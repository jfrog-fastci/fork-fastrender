...
- `FASTR_BROWSER_SESSION_PATH=/path/to/fastrender_session.json` – override where the browser session file is stored.
  ...
  - Note: even when tabs are not restored (e.g. `browser <url>` or `--no-restore`), the browser may
    still load the session file to reuse those persisted chrome settings.
