...
1. **AppContainer (preferred)** with **zero capabilities** (blocks outbound network + most filesystem).
   - Defense in depth: remove the broad `ALL APPLICATION PACKAGES` group (SID `S-1-15-2-1`) from the
     ...
     (`SpawnConfig::all_application_packages_hardened`; enabled by default in
      `fastrender::sandbox::windows::SpawnConfig::default()`;
...
- `tests/sandbox/windows_sandbox_env_sanitization.rs` (spawn-time environment sanitization; avoids leaking secrets into the renderer)
- `tests/sandbox/windows_appcontainer_temp_dir.rs` (AppContainer temp dir override is writable; `TEMP`/`TMP` point at a sandbox-accessible location)
