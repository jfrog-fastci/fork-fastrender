...
`DiskCachingFetcher` uses per-entry `.lock` files while writing cache entries.
`DiskCacheConfig::lock_stale_after` controls how quickly stale locks are removed
after an abrupt writer termination (on Unix, the lockfile also records a PID for
immediate cleanup when the process is no longer alive).
...
`FastRenderConfig` also carries a `ResourceAccessPolicy` for subresource access. By default it mirrors browser defaults (block `file://` from HTTP(S) documents, allow cross-origin loads). Set `with_block_mixed_content(true)` to forbid HTTP under HTTPS; `with_same_origin_subresources(true)` to reject cross-origin CSS/images/fonts when the document origin is known (this does not block cross-origin iframe/embed document navigation); and `with_allowed_subresource_origins(...)` to permit an explicit allowlist alongside the document origin.
...
bash scripts/run_limited.sh --as 64G -- \
  bash scripts/cargo_agent.sh run --bin inspect_frag -- file.html --query "#main .item:nth-child(2)"
```
