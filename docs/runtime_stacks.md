...
- **Desktop browser UI embedding:** the windowed `browser` app’s worker currently renders via
  `api::BrowserDocument` but also maintains a JS-capable `api::BrowserTab` and best-effort syncs its
  `dom2` snapshot into the renderer DOM before painting (see [`docs/browser_ui.md`](browser_ui.md)).
...
- `BrowserTab`
  - `BrowserDocumentDom2`
  - `EventLoop<BrowserTabHost>` (tasks + microtasks + timers + rAF + idle callback queues)
  - `BrowserTabJsExecutor` (e.g. `VmJsBrowserTabExecutor`)
  - navigation/history state
- `BrowserDocumentJs`
  - `BrowserDocumentDom2`
  - `EventLoop<BrowserDocumentJs>` (tasks + microtasks + timers + rAF + idle callback queues)
...
- `BrowserDocumentDom2`,
- an `EventLoop<BrowserDocumentJs>` (tasks + microtasks + timers + `requestAnimationFrame`),
