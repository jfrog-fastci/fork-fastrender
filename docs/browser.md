...
- When run **with** a URL, it opens that URL and does not restore tabs unless `--restore` is provided
  (which tries restoring the previous session even when a URL is provided).
...
- `FASTR_TRACE_MAX_EVENTS=<N>` – cap the number of trace events retained per trace (default 200000).
  This applies to both `FASTR_TRACE_OUT` (renderer pipeline) and windowed browser UI traces (`browser --trace-out` / `FASTR_BROWSER_TRACE_OUT`).
...
- `browser --trace-out /path/to/trace.json` (or `FASTR_BROWSER_TRACE_OUT=...`; legacy alias: `FASTR_PERF_TRACE_OUT`; Perfetto/Chrome trace; written on exit; cap retention with `FASTR_TRACE_MAX_EVENTS=<N>`)
