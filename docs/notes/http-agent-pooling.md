...
- Run `bash scripts/cargo_agent.sh bench --bench http_agent_pool` to compare the pooled `ureq` agent against a fresh agent per request on a local keep-alive server. (The benchmark uses `http://` so it exercises the `ureq` path; `https://` requests would select `reqwest` under `FASTR_HTTP_BACKEND=auto`.)
