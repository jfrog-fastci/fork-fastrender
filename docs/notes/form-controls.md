...
UPDATE_PAGES_GOLDEN=1 \
  ...
  scripts/run_limited.sh --as 8G --cpu 120 -- cargo test -j 1 pages_regression_suite
```
...
```
UPDATE_GOLDEN=1 \
  scripts/run_limited.sh --as 8G --cpu 120 -- cargo test -j 1 form_controls_reference_image_matches_golden
```
