...
- Disabled, inert, required, and invalid states are derived from element attributes. Focus/focus-visible state is provided via internal `InteractionState` (so author CSS cannot observe it via attribute selectors); `inert`/`data-fastr-inert=true` suppresses focus markers.
- `:user-valid` / `:user-invalid` are gated by HTML “user validity”, which flips after user interaction or a submission attempt. FastRender tracks this as internal interaction state (not as `data-fastr-*` DOM attributes).
...
- Vendor aliasing:
  ...
  - `@supports` handling for vendor properties (intentionally conservative to avoid inverting feature queries):
    - `src/css/supports.rs::supports_declaration` (tests: unit tests in `src/css/supports.rs`)
- Painting:
...
- Box-generation unit tests (run with `--lib`):
  - `appearance_none_form_controls_generate_fallback_children` (`src/tree/box_generation/tests.rs`)
  - `appearance_none_disables_form_control_replacement_and_generates_placeholder_text` (`src/tree/box_generation/tests.rs`)
  - `webkit_appearance_none_disables_form_control_replacement` (`src/tree/box_generation/tests.rs`)
  - `moz_appearance_none_disables_form_control_replacement` (`src/tree/box_generation/tests.rs`)
...
```
UPDATE_GOLDEN=1 \
  bash scripts/cargo_agent.sh test -p fastrender --test integration form_controls_reference_image_matches_golden
```
