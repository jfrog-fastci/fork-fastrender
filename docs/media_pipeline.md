...
| Audio output plumbing | ✅ (not wired to HTMLMediaElement yet) | [`src/media/audio/`](../src/media/audio/) (real output via `audio_cpal`; deterministic file output via `audio_wav`; null backend is the always-available fallback) |
...
- Audio output tuning + backend selection (used by `src/media/audio/mod.rs`, `src/media/audio/config.rs`,
  ...
  - `FASTR_AUDIO_IDLE_TIMEOUT_MS` (CPAL backend idle suspend)
  - `FASTR_AUDIO_DEFAULT_SAMPLE_RATE_HZ` / `FASTR_AUDIO_DEFAULT_CHANNELS` (null/WAV defaults)
