...
The pageset-oriented CLI binaries (`fetch_pages`, `prefetch_assets`, `render_pages`, `fetch_and_render`, and `pageset_progress`) all build their network fetcher through
[`cli_utils::render_pipeline::build_http_fetcher`](../src/cli_utils/render_pipeline.rs) (imported as `fastrender::cli_utils as common` in the bins), so they honor the `FASTR_HTTP_*` environment variables documented in [`docs/env-vars.md#http-fetch-tuning`](env-vars.md#http-fetch-tuning).
