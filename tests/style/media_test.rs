...
fn test_bootstrap_breakpoints() {
  ...
  for (query_str, should_match, should_not_match) in breakpoints {
    let query =
      MediaQuery::parse(query_str).unwrap_or_else(|_| panic!("Failed to parse: {}", query_str));
...
fn test_orientation_detection() {
  ...
  for (name, width, height) in portrait_cases {
    ...
    assert!(
      !ctx.evaluate(&landscape_query),
      "{} should not be landscape",
      name
    );
  }
  ...
  for (name, width, height) in landscape_cases {
    ...
    assert!(
      !ctx.evaluate(&portrait_query),
      "{} should not be portrait",
      name
    );
  }
}
...
fn media_length_features_accept_calc_and_reject_invalid_units() {
  ...
  assert!(
    bogus_unit.is_err(),
    "unknown length units should fail parsing"
  );
}
...
fn media_range_syntax_requires_valid_lengths() {
  ...
  assert!(
    bad_range.is_err(),
    "invalid range operators should fail parsing"
  );
}
...
fn media_resolution_rejects_invalid_units() {
  ...
  assert!(
    invalid_res.is_err(),
    "resolution with unknown units should fail"
  );
}
...
fn test_prefers_contrast() {
  let normal_ctx = MediaContext::screen(1024.0, 768.0);
  let more_ctx =
    MediaContext::screen(1024.0, 768.0).with_prefers_contrast(ContrastPreference::More);
  let less_ctx =
    MediaContext::screen(1024.0, 768.0).with_prefers_contrast(ContrastPreference::Less);
  let custom_ctx =
    MediaContext::screen(1024.0, 768.0).with_prefers_contrast(ContrastPreference::Custom);
...
fn media_query_cache_invalidation_on_viewport_change() {
  ...
  assert!(
    wide.evaluate_with_cache(&query, Some(&mut cache)),
    "wide viewport should match"
  );
...
fn media_query_cache_reuses_existing_entry() {
  ...
  assert_eq!(
    cache.len(),
    first_len,
    "cache should not grow on repeat evaluation"
  );
}
...
fn test_complex_queries() {
  ...
  // Multiple conditions
  let query =
    MediaQuery::parse("screen and (min-width: 768px) and (orientation: landscape)").unwrap();
