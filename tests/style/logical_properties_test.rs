...
#[test]
fn logical_padding_maps_in_vertical_writing_mode_direction_rtl() {
  let mut style = ComputedStyle::default();
  style.writing_mode = WritingMode::VerticalRl;
  style.direction = Direction::Rtl;
  apply_declaration(
...
fn margin_and_inset_accept_calc_zero() {
  ...
  apply_declaration(
    ...
    &decl(
      "margin",
      PropertyValue::Multiple(vec![PropertyValue::Length(zero)]),
    ),
    &ComputedStyle::default(),
    16.0,
    16.0,
...
#[test]
fn logical_padding_maps_in_sideways_writing_mode() {
  let mut style = ComputedStyle::default();
  ...
  // Sideways-rl: inline axis is vertical (top → bottom), block axis is horizontal right → left.
...
#[test]
fn logical_padding_maps_in_sideways_writing_mode_direction_rtl() {
  let mut style = ComputedStyle::default();
  style.writing_mode = WritingMode::SidewaysRl;
  style.direction = Direction::Rtl;
  apply_declaration(
  ...
  resolve_pending_logical_properties(&mut style);
  assert_eq!(style.padding_bottom, Length::px(5.0));
  assert_eq!(style.padding_top, Length::px(7.0));
  assert_eq!(style.padding_right, Length::px(2.0));
  assert_eq!(style.padding_left, Length::px(4.0));
}
...
fn inline_and_block_sizes_map_to_physical_axes() {
  ...
  assert_eq!(style.width_keyword, None);
...
#[test]
fn inline_and_block_sizes_map_in_sideways_writing_mode() {
  let mut style = ComputedStyle::default();
  style.writing_mode = WritingMode::SidewaysRl;
  apply_declaration(
    &mut style,
    &decl("inline-size", PropertyValue::Length(Length::px(30.0))),
    &ComputedStyle::default(),
    16.0,
    16.0,
  );
  apply_declaration(
    &mut style,
    &decl("block-size", PropertyValue::Length(Length::px(50.0))),
    &ComputedStyle::default(),
    16.0,
    16.0,
  );
