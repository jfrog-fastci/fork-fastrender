...
fn cascade_handles_many_keyword_declarations_under_budget() {
  ...
  let media = MediaContext::screen(1280.0, 720.0);
  // This test runs inside the large `style_tests` binary, which executes many other style/layout
  // checks in parallel. Keep the budget generous enough to avoid noisy multi-threaded
  // CI/agent environments, but still low enough to catch gross regressions.
  let budget = Duration::from_millis(3000);
  let (elapsed, samples, styled) = apply_styles_best_of(budget, || {
    apply_styles_with_media(&dom, &stylesheet, &media)
  });
