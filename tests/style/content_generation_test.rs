...
fn test_generator_attr_with_fallback() {
  let gen = ContentGenerator::new();
  let content = ContentValue::Items(vec![ContentItem::attr_with_fallback(
    "missing-attr",
    "Fallback Value",
  )]);
  let mut ctx = ContentContext::new();
...
fn test_is_text_only() {
  ...
  assert!(ContentGenerator::is_text_only(&ContentValue::from_string(
    "Hello"
  )));
...
fn test_content_value_display() {
  ...
  assert_eq!(
    format!("{}", ContentValue::from_string("Hello")),
    "\"Hello\""
  );
}
...
fn test_content_item_display() {
  ...
  assert_eq!(
    format!(
      "{}",
      ContentItem::counter_styled("n", CounterStyle::LowerRoman)
    ),
    "counter(n, lower-roman)"
  );
  assert_eq!(
    format!("{}", ContentItem::counters("n", ".")),
    "counters(n, \".\")"
  );
...
fn test_chapter_section_workflow() {
  ...
  ctx.set_attribute("title", "Introduction");
  assert_eq!(
    gen.generate(&chapter_content, &mut ctx),
