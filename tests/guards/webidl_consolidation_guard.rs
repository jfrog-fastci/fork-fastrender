...
const FORBIDDEN_WORKSPACE_WEBIDL_CRATE_SUFFIXES: [&str; 4] =
  ["ir", "bindings-core", "vm-js", "js-runtime"];

const VENDORED_WEBIDL_VM_JS_PATH_FRAGMENT: &str = "vendor/ecma-rs/webidl-vm-js";
const VENDORED_WEBIDL_JS_RUNTIME_PATH_FRAGMENT: &str = "vendor/ecma-rs/webidl-runtime";
...
#[test]
fn workspace_uses_vendored_webidl_js_runtime_when_present() {
  let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
  let manifest_path = repo_root.join("Cargo.toml");
  let manifest_src = fs::read_to_string(&manifest_path)
    .unwrap_or_else(|err| panic!("failed to read {}: {err}", manifest_path.display()));
  ...
  if root_dep.is_none() && workspace_dep.is_none() {
    return;
  }

  let root_dep_path = root_dep.and_then(dep_path);
  let workspace_dep_path = workspace_dep.and_then(dep_path);

  assert!(
    root_dep_path == Some(VENDORED_WEBIDL_JS_RUNTIME_PATH_FRAGMENT)
