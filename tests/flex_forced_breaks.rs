...
#[test]
fn flex_item_forced_break_after_right_inserts_blank_page() {
  // Regression: for row flex containers, forced page breaks authored on flex items propagate to
  // the flex line boundary. Page-side constraints like `break-after:right` must be attached to
  // that line boundary so pagination can insert a blank page when needed.
  let html = r#"
    <html>
      <head>
        <style>
  ...
  // Keep each page in its own coordinate space to make position checks stable.
