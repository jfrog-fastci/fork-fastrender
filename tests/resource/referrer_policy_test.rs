...
body { font-family: "TestFont"; }"#
            ...
            if path == "/doc_response_policy_invalid_header_img.html" {
              extra_headers.push_str("Referrer-Policy: bogus-policy\r\n");
            }
            if path == "/doc_response_policy_value_list_img.html" {
              extra_headers.push_str("Referrer-Policy: no-referrer, origin\r\n");
            }
...
fn img_invalid_referrerpolicy_attribute_uses_document_meta_policy() {
  ...
  let html = r#"<!doctype html>
    <html>
      ...
      <body>
        <img src="/img.png" referrerpolicy="bogus-policy" style="width: 10px; height: 10px">
      </body>
    </html>"#;
  let document_url = format!("{}/page.html", server.base_url);
  ...
  assert_eq!(
    header_value(&img_req.headers, "referer").as_deref(),
    Some(expected_referer.as_str()),
    "expected invalid img referrerpolicy attribute to be ignored and document meta policy to apply; got:\n{}",
    img_req.headers
  );
}
...
fn meta_invalid_referrer_policy_is_ignored_and_default_policy_applies_for_images() {
  ...
  assert_eq!(
    header_value(&img_req.headers, "referer").as_deref(),
    Some(expected_referer.as_str()),
    "expected invalid meta Referrer-Policy value to be ignored and default strict-origin-when-cross-origin policy to apply; got:\n{}",
    img_req.headers
  );
}
...
fn meta_referrer_policy_outside_head_is_ignored() {
  ...
  assert_eq!(
    header_value(&img_req.headers, "referer").as_deref(),
    Some(expected_referer.as_str()),
    "expected meta Referrer-Policy outside <head> to be ignored and default strict-origin-when-cross-origin policy to apply; got:\n{}",
    img_req.headers
  );
}
...
fn meta_referrer_policy_no_referrer_allows_iframe_referrerpolicy_override_for_nested_requests() {
  ...
  let html = r#"<!doctype html>
    <html>
      ...
      <body>
        <iframe src="/frame_with_style_import.html" referrerpolicy="origin" style="width: 10px; height: 10px"></iframe>
      </body>
    </html>"#;
  let document_url = format!("{}/page.html", server.base_url);
...
fn document_response_referrer_policy_invalid_token_is_ignored_for_images() {
  ...
  for path in ["/doc_response_policy_invalid_header_img.html", "/img.png"] {
    server.wait_for_request(
      |req| req.path == path,
      &format!("expected {path} request to be issued for the test fixture"),
    );
  ...
  assert_eq!(
    header_value(&img_req.headers, "referer").as_deref(),
    Some(doc_url.as_str()),
    "expected invalid document response Referrer-Policy value to be ignored and default strict-origin-when-cross-origin policy to apply; got:\n{}",
    img_req.headers
  );
}
...
fn document_response_referrer_policy_value_list_last_token_wins_for_images() {
  ...
  for path in ["/doc_response_policy_value_list_img.html", "/img.png"] {
    server.wait_for_request(
      |req| req.path == path,
      &format!("expected {path} request to be issued for the test fixture"),
    );
  ...
  assert_eq!(
    header_value(&img_req.headers, "referer").as_deref(),
    Some(expected_referer.as_str()),
    "expected document response Referrer-Policy value list to use the last token for /img.png; got:\n{}",
    img_req.headers
  );
}
