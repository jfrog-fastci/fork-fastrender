...
fn img_referrerpolicy_no_referrer_omits_referer_header() {
  ...
  server.wait_for_request(
    ...
    "expected image request for <img referrerpolicy=\"no-referrer\">",
  );
...
fn img_referrerpolicy_overrides_document_referrer_policy() {
  ...
  server.wait_for_request(
    |req| req.path == "/img.png",
    "expected image request for <img referrerpolicy=\"origin\">",
  );
...
fn iframe_referrerpolicy_overrides_document_referrer_policy() {
  ...
  server.wait_for_request(
    ...
    "expected iframe navigation request for <iframe referrerpolicy=\"origin\">",
  );
  let expected_origin = server.origin();
  let captured = server.shutdown_and_join();
...
fn iframe_src_referrerpolicy_applies_to_iframe_subresource_requests() {
  ...
  server.wait_for_request(
    |req| req.path == "/img.png",
    "expected iframe subresource request for <iframe referrerpolicy=\"origin\">",
  );
  let expected_origin = server.origin();
  let captured = server.shutdown_and_join();
  let img_requests: Vec<_> = captured.iter().filter(|r| r.path == "/img.png").collect();
...
fn link_stylesheet_referrerpolicy_overrides_document_policy_and_applies_to_imports() {
  ...
  server.wait_for_request(
    ...
    "expected stylesheet request for <link referrerpolicy=\"origin\">",
  );
  server.wait_for_request(
    |req| req.path == "/import.css",
    "expected @import request for <link referrerpolicy=\"origin\">",
  );
  let expected_origin = server.origin();
  let captured = server.shutdown_and_join();
...
fn iframe_srcdoc_referrerpolicy_applies_to_srcdoc_subresource_requests() {
  ...
  let html = r#"<!doctype html>
    <html><head>
      ...
      <iframe
        srcdoc="<!doctype html><html><head><style>html, body { margin: 0; padding: 0; }</style></head><body><img src='/img.png' style='width: 10px; height: 10px'></body></html>"
  ...
  server.wait_for_request(
    |req| req.path == "/img.png",
    "expected iframe srcdoc subresource request for <iframe referrerpolicy=\"origin\">",
  );
  let expected_origin = server.origin();
  let captured = server.shutdown_and_join();
  let img_requests: Vec<_> = captured.iter().filter(|r| r.path == "/img.png").collect();
