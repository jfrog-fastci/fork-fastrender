...
fn spawn_upload_server_with_accept_txt() -> (
  String,
...
<html>
  ...
  <body>
    <form action="/upload" method="post" enctype="multipart/form-data">
      <input id="f" type="file" accept=".txt" name="f">
...
#[test]
fn file_input_accept_filters_file_picker_choose() {
  let _browser_integration_lock = crate::browser_integration::stage_listener_test_lock();
  let _lock = super::stage_listener_test_lock();

  let (page_url, rx_request, server_join) = spawn_upload_server_with_accept_txt();

  let dir = tempdir().expect("temp dir");
  let png_path = dir.path().join("upload.png");
  std::fs::write(&png_path, b"png-bytes").expect("write upload.png");

  let handle = spawn_ui_worker("fastr-ui-worker-file-input-accept-filters-file-picker")
  ...
  let (input_node_id, accept) = match opened {
    WorkerToUi::FilePickerOpened {
      input_node_id,
      accept,
      ..
    } => (input_node_id, accept),
    _ => unreachable!(),
  };
  assert_eq!(
    accept.as_deref(),
    Some(".txt"),
    "expected FilePickerOpened to report accept=.txt"
  );
  ...
  // Submit the form. The file selection should have been rejected (empty filename).
  ui_tx
    .send(UiToWorker::PointerDown {
      tab_id,
      pos_css: (10.0, 70.0),
  ...
  assert!(
    body_str.contains("filename=\"\""),
    "expected empty filename in multipart body after rejected selection; body={body_str}"
  );
  assert!(
    !body_str.contains("filename=\"upload.png\""),
    "expected rejected .png file to not be submitted; body={body_str}"
  );
