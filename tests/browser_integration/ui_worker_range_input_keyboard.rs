...
#[test]
fn range_input_keyboard_page_up_down_step_and_repaints() -> Result<()> {
  let _browser_integration_lock = crate::browser_integration::stage_listener_test_lock();
  let _lock = super::stage_listener_test_lock();
  let tab_id = TabId(1);
  let viewport_css = (200, 140);
  ...
  let html = r#"<!doctype html>
    <html>
      ...
      <body>
        <label id="lbl" for="r">focus</label>
        <input id="r" type="range" min="0" max="20" step="1" value="0">
        <div id="box"></div>
      </body>
    </html>
  "#;
  ...
  // PageUp increases by a larger step delta.
  let frame_after_page_up = extract_frame(controller.handle_message(UiToWorker::KeyAction {
    tab_id,
    key: KeyAction::PageUp,
  })?)
  .expect("expected FrameReady after PageUp");
  assert_eq!(rgba_at_css(&frame_after_page_up, 10, 90), [0, 255, 0, 255]);

  // PageDown decreases by the same larger delta.
  let frame_after_page_down = extract_frame(controller.handle_message(UiToWorker::KeyAction {
    tab_id,
    key: KeyAction::PageDown,
  })?)
  .expect("expected FrameReady after PageDown");
  assert_eq!(rgba_at_css(&frame_after_page_down, 10, 90), [255, 0, 0, 255]);
