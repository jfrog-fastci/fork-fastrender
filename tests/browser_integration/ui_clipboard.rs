...
#[test]
fn ui_select_all_sets_selection_attributes_and_repaints() {
  let _lock = super::stage_listener_test_lock();
...
<html>
  <head>
    ...
    <style>
      ...
      /* Probe element whose background reflects whether SelectAll wrote selection attributes. */
      #probe {
        position: absolute;
        left: 10px;
        top: 50px;
      ...
      #t[data-fastr-selection-start="0"][data-fastr-selection-end="5"] + #probe {
...
"#,
  ...
  let handle = spawn_ui_worker("fastr-ui-worker-select-all-repaint").expect("spawn ui worker");
  let (ui_tx, ui_rx, join) = handle.split();
  ...
  // Initial paint: probe should be green (no selection).
  let _frame0 =
  ...
  ui_tx
    ...
    .expect("pointer up");
  let _ = next_frame_ready_with_probe_color(
    &ui_rx,
    tab_id,
    (20, 60),
    [0, 255, 0, 255],
  ...
  // SelectAll should set `data-fastr-selection-*` attrs and trigger a repaint.
  ui_tx
    .send(UiToWorker::SelectAll { tab_id })
    .expect("select all");
  let _frame_selected = next_frame_ready_with_probe_color(
    &ui_rx,
    tab_id,
    (20, 60),
    [255, 0, 255, 255],
  ...
  // ArrowLeft should clear the selection, removing the attributes and repainting.
  ui_tx
    .send(UiToWorker::KeyAction {
      tab_id,
