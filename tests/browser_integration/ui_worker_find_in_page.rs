...
fn matchy_page(match_count: usize) -> String {
  // Use many small matches separated by whitespace so layout remains cheap (wrap opportunities)
  // while still producing a true match count well above the cap.
  let text = "a ".repeat(match_count);
  format!(
    r#"<!doctype html>
<html>
  <head>
    ...
    <style>
      ...
      /* Keep line widths bounded so layout doesn't create a single giant unbreakable run. */
      #content {{ width: 200px; word-break: break-word; }}
    </style>
  </head>
  <body>
...
#[test]
fn ui_worker_find_in_page_match_count_is_capped_at_10k() {
  let _lock = super::stage_listener_test_lock();

  let true_matches = MAX_FIND_MATCHES + 123;
  let site = support::TempSite::new();
  let url = site.write("matchy.html", &matchy_page(true_matches));
  ...
  // Ensure layout is prepared before issuing find-in-page queries.
  let _ = h.wait_for_frame(tab_id, support::DEFAULT_TIMEOUT);
  ...
  let (query, case_sensitive, match_count, active) = last_find_result(&events);
  assert_eq!(query, "a");
  assert!(case_sensitive);

  assert!(
    match_count <= MAX_FIND_MATCHES,
    "find-in-page match_count must be capped at {MAX_FIND_MATCHES} (true_matches={true_matches}, got={match_count})"
  );
  assert_eq!(
    match_count, MAX_FIND_MATCHES,
    "expected match_count to saturate at the cap when true match count exceeds it"
  );
  assert_eq!(
    active,
    Some(0),
    "expected first match to be active when matches exist"
  );
}
