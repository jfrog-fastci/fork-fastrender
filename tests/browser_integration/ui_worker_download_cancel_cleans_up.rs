...
fn ui_worker_download_cancel_cleans_up() {
  ...
  with_runtime_toggles(
    ...
    || {
      ...
      .unwrap_or_else(|| {
        panic!("timed out waiting for DownloadProgress after clicking download link")
      });
      ...
      ui_tx
        .send(UiToWorker::CancelDownload { download_id })
        .unwrap();
      ...
      .unwrap_or_else(|| {
        panic!("timed out waiting for DownloadFinished for download {download_id:?}")
      });
