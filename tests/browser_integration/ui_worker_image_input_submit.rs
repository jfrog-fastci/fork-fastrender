...
#[test]
fn click_unnamed_image_input_submits_form_with_x_y_coords() {
  let _browser_integration_lock = crate::browser_integration::stage_listener_test_lock();
  let _lock = super::stage_listener_test_lock();
...
<html>
  ...
  <body>
    <form action="result.html">
      <input id="q" name="q" value="a b">
      <input id="img" type="image" src="{image_src}">
    </form>
  </body>
</html>
"#
...
"#,
  ...
  let handle =
    spawn_ui_worker("fastr-ui-worker-image-input-submit-unnamed").expect("spawn ui worker");
  let (ui_tx, ui_rx, join) = handle.split();
  let tab_id = TabId::new();
  ...
  .unwrap_or_else(|| {
    let msgs = support::drain_for(&ui_rx, Duration::from_millis(200));
    panic!(
      "timed out waiting for NavigationStarted(result.html) for unnamed image input; saw:\n{}",
      support::format_messages(&msgs)
    );
  });
  ...
  let x = x_val.unwrap_or_else(|| {
    panic!("expected submitted URL to include integer x parameter; got {submitted_url}")
  });
  let y = y_val.unwrap_or_else(|| {
    panic!("expected submitted URL to include integer y parameter; got {submitted_url}")
  });
  assert!(
    x > 0 && y > 0,
    "expected x/y from a pointer click to be > 0; got x={x} y={y} url={submitted_url}"
  );
...
#[test]
fn click_unnamed_image_input_submits_form_with_xy_coords() {
  let _browser_integration_lock = crate::browser_integration::stage_listener_test_lock();
  let _lock = super::stage_listener_test_lock();
...
#[test]
fn enter_on_focused_unnamed_image_input_submits_form_with_zero_xy_coords() {
  let _browser_integration_lock = crate::browser_integration::stage_listener_test_lock();
  let _lock = super::stage_listener_test_lock();
...
"#,
  ...
  let handle = spawn_ui_worker("fastr-ui-worker-image-input-submit-unnamed-enter")
  ...
  assert_eq!(
    x_val,
    Some(0),
    "expected Enter activation to submit x=0; got url={submitted_url}"
  );
  assert_eq!(
    y_val,
    Some(0),
    "expected Enter activation to submit y=0; got url={submitted_url}"
  );
