...
<html>
  <head>
    ...
    <style>
      ...
      input { position: absolute; left: 0; top: 0; width: 64px; height: 64px; }
    </style>
  </head>
  <body>
    <input id="img" type="image" src="a.svg">
  </body>
</html>
"#,
  );

  let expected_image_url = url::Url::parse(&index_url)
    .expect("parse base URL")
    .join("a.svg")
    .expect("join relative image URL")
    .to_string();

  (site, index_url, expected_image_url)
}

#[test]
fn context_menu_request_resolves_input_type_image_url() {
  let _browser_integration_lock = crate::browser_integration::stage_listener_test_lock();
  let _lock = super::stage_listener_test_lock();
  let (_site, index_url, expected_image_url) = fixture();

  let worker = spawn_ui_worker("fastr-ui-worker-context-menu-input-type-image")
