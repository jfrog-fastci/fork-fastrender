...
#[test]
fn submit_prevent_default_blocks_enter_form_submission_navigation_without_ids() {
  let _browser_integration_lock = crate::browser_integration::stage_listener_test_lock();
  let _lock = super::stage_listener_test_lock();
...
<html>
  <head>
    <style>
      html, body { margin: 0; padding: 0; }
      input[name="q"] { position: absolute; left: 0; top: 60px; width: 120px; height: 24px; }
      input[type="submit"] { position: absolute; left: 0; top: 0; width: 120px; height: 40px; }
    </style>
  </head>
  <body>
    <form action="result.html">
...
"#,
  ...
  let handle = spawn_ui_worker("fastr-ui-worker-form-submit-enter-prevent-default-no-ids")
    .expect("spawn ui worker");
  let (ui_tx, ui_rx, join) = handle.split();
  let tab_id = TabId::new();
  ...
  // Focus the input (positioned at y=60px) and press Enter to submit the form. The `submit`
  // listener prevents default, so no navigation should occur.
  ui_tx
    .send(UiToWorker::PointerDown {
      tab_id,
      pos_css: (10.0, 70.0),
