...
fn diff_dir() -> PathBuf {
  PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/content_visibility_writing_mode_diffs")
}
...
#[test]
fn content_visibility_writing_mode_regression() {
  ...
  let html_path = fixtures_dir().join("content_visibility_writing_mode/index.html");
  let html =
  ...
  let golden_name = "content_visibility_writing_mode";
