...
#[test]
fn compat_render_should_lift_iframe_data_live_path_and_id() {
  let fixture_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
    .join("tests/pages/fixtures/dom_compat_lazy_iframe_live_id");
  let html_path = fixture_dir.join("index.html");
  let html = fs::read_to_string(&html_path).expect("read fixture HTML");
  ...
  // With compatibility mode, `data-live-path` + `data-live-id` should be promoted into a concrete
  // iframe `src` URL and rendered.
  let (r, g, b, _) = get_pixel(&compat_pixmap, 32, 16);
  assert!(
    is_red(r, g, b),
    "compat render should lift iframe data-live-path + data-live-id into src (red); got rgb({r},{g},{b})"
  );
}
