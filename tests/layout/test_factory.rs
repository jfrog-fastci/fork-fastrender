...
fn test_block_containing_flex() {
  ...
  let fragment = block_fc
    .layout(&block_parent, &standard_constraints())
    .unwrap();
...
fn test_flex_containing_block() {
  ...
  let fragment = flex_fc
    .layout(&flex_parent, &standard_constraints())
    .unwrap();
...
fn test_grid_containing_flex() {
  ...
  let fragment = grid_fc
    .layout(&grid_parent, &standard_constraints())
    .unwrap();
...
fn test_deeply_nested_fcs() {
  ...
  let fragment = block_fc
    .layout(&outer_block, &standard_constraints())
    .unwrap();
...
fn test_all_fcs_handle_wide_constraints() {
  ...
  for &fc_type in factory.supported_types() {
    ...
    assert!(
      result.is_ok(),
      "FC {:?} failed with wide constraints",
      fc_type
    );
  }
}
...
fn test_all_fcs_handle_narrow_constraints() {
  ...
  for &fc_type in factory.supported_types() {
    ...
    assert!(
      result.is_ok(),
      "FC {:?} failed with narrow constraints",
      fc_type
    );
  }
}
...
fn test_all_fcs_handle_indefinite_constraints() {
  ...
  for &fc_type in factory.supported_types() {
    ...
    assert!(
      result.is_ok(),
      "FC {:?} failed with indefinite constraints",
      fc_type
    );
  }
}
