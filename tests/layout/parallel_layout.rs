...
fn build_block_stack_with_fixed_cb_ancestor() -> (BoxTree, usize) {
  let mut root_style = ComputedStyle::default();
  root_style.display = Display::Block;

  let mut container_style = ComputedStyle::default();
  container_style.display = Display::Block;
  // A non-empty transform list establishes a fixed containing block, making fixed-positioned
  // descendants resolve against this element instead of the viewport.
  container_style
    .transform
    .push(Transform::TranslateX(Length::px(0.0)));
...
fn parallel_block_children_preserve_external_fixed_containing_blocks() {
  ...
  // Under an ancestor fixed containing block, fixed fragments are stored in the local coordinate
  // space of their parent fragments, so their Y origin becomes negative when the in-flow ancestor
  // is positioned below the containing block's origin.
  assert!(
    serial_fixed.bounds.y < 0.0,
