...
#[test]
fn collapsed_column_removal_adjusts_colspans_and_offsets_collapsed_border_model() {
  ensure_rayon_threads();
  let html = r#"
    <html>
      <head>
  ...
  assert!(
    gap.abs() < 0.1,
    "collapsed column should be removed from offsets in collapsed border model (expected C x={expected_c_x}, got {}, gap={gap})",
    c.rect.x()
  );
  ...
  assert!(
    span_gap.abs() < 0.1,
    "colspan should shrink to visible columns in collapsed border model (expected S width={expected_span_width}, got {}, gap={span_gap})",
    s.rect.width()
  );
}

#[test]
fn collapsed_column_removal_adjusts_colspans_and_offsets_collapsed_border_model_rtl() {
  ensure_rayon_threads();
  let html = r#"
    <html>
      <head>
  ...
  assert!(
    gap.abs() < 0.1,
    "collapsed column should be removed from offsets in RTL collapsed border model (gap={gap})"
  );
  ...
  assert!(
    span_gap.abs() < 0.1,
    "colspan should shrink to visible columns in RTL collapsed border model (expected S width={expected_span_width}, got {}, gap={span_gap})",
    s.rect.width()
  );
  assert!(
    (s.rect.x() - c.rect.x()).abs() < 0.1,
    "expected spanning cell to start at leftmost visible column in RTL collapsed border model (S.x={} C.x={})",
    s.rect.x(),
    c.rect.x()
  );
}
...
#[test]
fn row_group_visibility_collapse_removes_rows_from_layout_collapsed_border_model() {
  ensure_rayon_threads();
  let html = r#"
    <html>
      <head>
  ...
  assert!(
    gap.abs() < 0.1,
    "rows adjacent after collapsing middle row-group in collapsed border model (gap={gap})"
  );
}
...
#[test]
fn column_group_visibility_collapse_removes_columns_from_layout_collapsed_border_model() {
  ensure_rayon_threads();
  let html = r#"
    <html>
      <head>
  ...
  assert!(
    gap.abs() < 0.1,
    "cells adjacent after collapsing column-group in collapsed border model (gap={gap})"
  );
  assert!(
    (c.rect.width() - 50.0).abs() < 0.1,
    "collapsed column-group should not affect subsequent column width (got {})",
...
#[test]
fn column_group_visibility_collapse_removes_columns_from_layout_collapsed_border_model_rtl() {
  ensure_rayon_threads();
  let html = r#"
    <html>
      <head>
  ...
  assert!(
    gap.abs() < 0.1,
    "cells adjacent after collapsing column-group in RTL collapsed border model (gap={gap})"
  );
  assert!(
    (a.rect.width() - 30.0).abs() < 0.1,
    "expected first source column width 30px (got {})",
...
#[test]
fn column_group_span_visibility_collapse_removes_columns_from_layout_collapsed_border_model() {
  ensure_rayon_threads();
  let html = r#"
    <html>
      <head>
  ...
  assert!(
    gap.abs() < 0.1,
    "cells adjacent after collapsing column-group span in collapsed border model (gap={gap})"
  );
  assert!(
    (a.rect.width() - 30.0).abs() < 0.1,
    "expected first source column width 30px (got {})",
...
#[test]
fn column_group_span_visibility_collapse_removes_columns_from_layout_collapsed_border_model_rtl() {
  ensure_rayon_threads();
  let html = r#"
    <html>
      <head>
  ...
  assert!(
    gap.abs() < 0.1,
    "cells adjacent after collapsing column-group span in RTL collapsed border model (gap={gap})"
  );
  assert!(
    (a.rect.width() - 30.0).abs() < 0.1,
    "expected first source column width 30px (got {})",
