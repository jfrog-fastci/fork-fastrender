...
#[test]
fn webkit_column_break_before_forces_new_manual_column() {
  let mut renderer = FastRender::builder()
    .columns(2, 20.0, 200.0)
    .build()
    .unwrap();
  ...
  let html = r#"
    <html>
      <head>
        <style>
          body { margin: 0; }
          #first { height: 60px; background: rgb(180, 80, 40); }
          /* Legacy WebKit alias used by many sites. */
          #second { height: 40px; background: rgb(40, 120, 180); -webkit-column-break-before: always; }
        </style>
      </head>
      <body>
        <div id="first"></div>
  ...
  let first_color = Rgba::rgb(180, 80, 40);
  let second_color = Rgba::rgb(40, 120, 180);

  assert_eq!(
    fragments.additional_fragments.len(),
    1,
    "-webkit-column-break-before: always should create a second column fragment"
  );
