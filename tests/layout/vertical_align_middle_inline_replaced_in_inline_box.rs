...
#[test]
fn vertical_align_middle_affects_inline_replaced_metrics_inside_inline_box() {
  ...
  // The image is nested inside an inline box (<a>). Its `vertical-align` must still participate
  // in the line box baseline calculation; otherwise, `vertical-align: middle` is ignored and the
  // line height stays inflated by the strut's descent (the classic "inline image gap").
  let html = format!(
    r##"
    <html>
      <head>
      ...
      <body style="margin:0; font-family:sans-serif; font-size:20px; line-height:20px">
        <div><a href="#"><img src="{data_png}" style="width:80px; height:80px"></a>BASE</div>
        <div><a href="#"><img class="v-mid" src="{data_png}" style="width:80px; height:80px"></a>MID</div>
      </body>
    </html>
  "##,
  );
  ...
  assert!(
    ...
    "expected vertical-align: middle to reduce the line baseline position even when the replaced element is inside an inline box (base={base_baseline}, mid={mid_baseline})"
  );
