...
fn float_shrink_to_fit_sums_multiple_floats() {
  ...
  let inner_a = BoxNode::new_block(Arc::new(inner_style_a), FormattingContextType::Block, vec![]);

  let mut inner_style_b = ComputedStyle::default();
  inner_style_b.float = Float::Left;
  inner_style_b.width = Some(Length::px(80.0));
  let inner_b = BoxNode::new_block(Arc::new(inner_style_b), FormattingContextType::Block, vec![]);

  let mut outer_style = ComputedStyle::default();
  outer_style.float = Float::Right;
  let outer =
    BoxNode::new_block(Arc::new(outer_style), FormattingContextType::Block, vec![inner_a, inner_b]);
...
fn float_shrink_to_fit_sums_inline_level_floated_children() {
  ...
  inner_style_a.width = Some(Length::px(100.0));
  let inner_a =
    BoxNode::new_inline_block(Arc::new(inner_style_a), FormattingContextType::Block, vec![]);
  ...
  inner_style_b.width = Some(Length::px(80.0));
  let inner_b =
    BoxNode::new_inline_block(Arc::new(inner_style_b), FormattingContextType::Block, vec![]);
