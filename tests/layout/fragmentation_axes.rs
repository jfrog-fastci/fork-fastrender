...
fn fragmentation_axes_override_vertical_lr_fragments_horizontally() {
  ...
  let child2 = FragmentNode::new_block_with_id(
    Rect::from_xywh(150.0, 0.0, 150.0, 100.0),
    2,
    vec![],
  );
  let root = FragmentNode::new_block(Rect::from_xywh(0.0, 0.0, 300.0, 100.0), vec![child1, child2]);
  let options = FragmentationOptions::new(150.0);

  let default = fragment_tree(&root, &options).expect("fragment_tree");
  assert_eq!(default.len(), 1, "default (Y-axis) fragmentation should fit in one fragmentainer");

  let vertical = fragment_tree_for_writing_mode(&root, &options, WritingMode::VerticalLr, Direction::Ltr)
...
fn fragmentation_axes_override_vertical_rl_respects_reversed_block_progression() {
  ...
  let vertical = fragment_tree_for_writing_mode(&root, &options, WritingMode::VerticalRl, Direction::Ltr)
    .expect("fragment_tree_for_writing_mode");
