...
fn layout_child_y_column_reverse(justify_content: JustifyContent) -> f32 {
  let mut container_style = ComputedStyle::default();
  container_style.display = Display::Flex;
...
fn layout_child_x_rtl_row_reverse(justify_content: JustifyContent) -> f32 {
  let mut container_style = ComputedStyle::default();
  container_style.display = Display::Flex;
  container_style.direction = Direction::Rtl;
  container_style.flex_direction = FlexDirection::RowReverse;
...
fn layout_child_y_vertical_lr_row_reverse(justify_content: JustifyContent) -> f32 {
  let mut container_style = ComputedStyle::default();
  container_style.display = Display::Flex;
...
fn layout_child_x_vertical_lr_column_reverse(justify_content: JustifyContent) -> f32 {
  let mut container_style = ComputedStyle::default();
  container_style.display = Display::Flex;
  container_style.writing_mode = WritingMode::VerticalLr;
  container_style.flex_direction = FlexDirection::ColumnReverse;
...
#[test]
fn justify_content_start_end_in_column_reverse_are_distinct_from_flex_start_end() {
  let flex_start_y = layout_child_y_column_reverse(JustifyContent::FlexStart);
  let start_y = layout_child_y_column_reverse(JustifyContent::Start);
  assert!(
    (flex_start_y - 90.0).abs() < 1e-3,
    "justify-content:flex-start in column-reverse should align to the main-start edge (bottom), got {flex_start_y}"
  );
  assert!(
    (start_y - 0.0).abs() < 1e-3,
    "justify-content:start in column-reverse should align to the container start edge (top), got {start_y}"
  );

  let flex_end_y = layout_child_y_column_reverse(JustifyContent::FlexEnd);
  let end_y = layout_child_y_column_reverse(JustifyContent::End);
  assert!(
    (flex_end_y - 0.0).abs() < 1e-3,
    "justify-content:flex-end in column-reverse should align to the main-end edge (top), got {flex_end_y}"
  );
  assert!(
    (end_y - 90.0).abs() < 1e-3,
...
#[test]
fn justify_content_start_end_in_rtl_row_reverse_align_to_inline_edges() {
  let start_x = layout_child_x_rtl_row_reverse(JustifyContent::Start);
  assert!(
    (start_x - 90.0).abs() < 1e-3,
    "justify-content:start in rtl row-reverse should align to inline-start edge (right), got {start_x}"
  );

  let end_x = layout_child_x_rtl_row_reverse(JustifyContent::End);
  assert!(
    (end_x - 0.0).abs() < 1e-3,
    "justify-content:end in rtl row-reverse should align to inline-end edge (left), got {end_x}"
  );
}
...
#[test]
fn justify_content_start_end_vertical_lr_row_reverse_are_distinct_from_flex_start_end() {
  let flex_start_y = layout_child_y_vertical_lr_row_reverse(JustifyContent::FlexStart);
  let start_y = layout_child_y_vertical_lr_row_reverse(JustifyContent::Start);
  assert!(
    (flex_start_y - 90.0).abs() < 1e-3,
    "justify-content:flex-start in vertical-lr row-reverse should align to the main-start edge (bottom), got {flex_start_y}"
  );
  assert!(
    (start_y - 0.0).abs() < 1e-3,
    "justify-content:start in vertical-lr row-reverse should align to inline-start edge (top), got {start_y}"
  );

  let flex_end_y = layout_child_y_vertical_lr_row_reverse(JustifyContent::FlexEnd);
  let end_y = layout_child_y_vertical_lr_row_reverse(JustifyContent::End);
  assert!(
    (flex_end_y - 0.0).abs() < 1e-3,
    "justify-content:flex-end in vertical-lr row-reverse should align to the main-end edge (top), got {flex_end_y}"
  );
  assert!(
    (end_y - 90.0).abs() < 1e-3,
    "justify-content:end in vertical-lr row-reverse should align to inline-end edge (bottom), got {end_y}"
  );
}
...
fn justify_content_start_end_vertical_rl_row_reverse_direction_rtl_flip_inline_axis() {
  let flex_start_y = layout_child_y_vertical_writing_mode_rtl_row_reverse(
    WritingMode::VerticalRl,
    JustifyContent::FlexStart,
  );
  let start_y = layout_child_y_vertical_writing_mode_rtl_row_reverse(
    WritingMode::VerticalRl,
  ...
  let flex_end_y = layout_child_y_vertical_writing_mode_rtl_row_reverse(
    WritingMode::VerticalRl,
    JustifyContent::FlexEnd,
  );
  let end_y = layout_child_y_vertical_writing_mode_rtl_row_reverse(
    WritingMode::VerticalRl,
    JustifyContent::End,
  );
  assert!(
...
fn justify_content_start_end_vertical_lr_column_reverse_are_distinct_from_flex_start_end() {
  ...
  assert!(
    (flex_start_x - 90.0).abs() < 1e-3,
    "justify-content:flex-start in vertical-lr column-reverse should align to the main-start edge (right), got {flex_start_x}"
  );
  assert!(
    (start_x - 0.0).abs() < 1e-3,
    "justify-content:start in vertical-lr column-reverse should align to block-start edge (left), got {start_x}"
  );

  let flex_end_x = layout_child_x_vertical_lr_column_reverse(JustifyContent::FlexEnd);
  let end_x = layout_child_x_vertical_lr_column_reverse(JustifyContent::End);
  assert!(
    (flex_end_x - 0.0).abs() < 1e-3,
    "justify-content:flex-end in vertical-lr column-reverse should align to the main-end edge (left), got {flex_end_x}"
  );
  assert!(
    (end_x - 90.0).abs() < 1e-3,
    "justify-content:end in vertical-lr column-reverse should align to block-end edge (right), got {end_x}"
  );
}
