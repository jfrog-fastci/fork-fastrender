...
#[test]
fn load_does_not_panic_on_avif_with_trailing_bytes() {
  #[derive(Clone)]
  ...
  let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| cache.load(url)));
  let loaded = result.expect("image load should not panic on avif decode assertions");
  let image = loaded.expect("load should succeed after trimming trailing bytes");
