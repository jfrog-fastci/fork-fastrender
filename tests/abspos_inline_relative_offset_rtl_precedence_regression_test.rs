...
#[test]
fn relative_inline_offset_rtl_prefers_right_over_left() {
  let html = r#"<!doctype html>
<style>
  html, body { margin: 0; padding: 0; background: #fff; }
  .wrap { padding: 40px; font-size: 0; line-height: 0; direction: ltr; }
  .cb { display: inline; position: relative; direction: rtl; left: 10px; right: 30px; top: 5px; border: 10px solid #000; padding: 0; background: #fff; vertical-align: top; }
...
"#;
  ...
  // The border box starts at the normal flow origin (40,40) shifted by the relative offset.
  // With `direction: rtl` and both `left` and `right` specified, CSS 2.1 uses `right`, so the
  // inline box shifts left by 30px and down by 5px.
  assert_eq!(black_bbox, (10, 45, 29, 64));
