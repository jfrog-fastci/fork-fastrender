...
#[test]
fn accessibility_layout_table_suppresses_table_descendant_roles() {
  let html = r##"
    <html>
      <body>
  ...
  assert!(
    ...
    "layout table with role=presentation should be omitted from the accessibility tree"
  );
...
fn accessibility_table_role_grid_maps_cells_to_gridcell() {
  let html = r##"
    <html>
      <body>
        <table id="g" role="grid">
          <tr id="gr"><td id="gc">Cell</td></tr>
        </table>
      </body>
    </html>
  "##;
  ...
  let subset = snapshot_subset(&tree, &["g", "gc"]);
  assert_eq!(
    subset,
    json!({
      "g": {
...
#[test]
fn accessibility_table_role_presentation_disallowed_falls_back_to_table_semantics() {
  let html = r##"
    <html>
      <body>
        <table id="t" role="presentation" aria-label="Data">
          <tr id="r"><td id="c">Cell</td></tr>
        </table>
      </body>
    </html>
  "##;
  ...
  let subset = snapshot_subset(&tree, &["t", "r", "c"]);
  assert_eq!(
    subset,
    json!({
      "t": {
...
#[test]
fn accessibility_table_role_presentation_grid_disallowed_falls_back_to_grid() {
  let html = r##"
    <html>
      <body>
        <table id="g" role="presentation grid" aria-label="Data">
          <tr id="r"><td id="c">Cell</td></tr>
        </table>
      </body>
    </html>
  ...
  let subset = snapshot_subset(&tree, &["g", "r", "c"]);
  assert_eq!(
    subset,
    json!({
      "g": {
