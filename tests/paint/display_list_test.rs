...
fn background_image_set_chooses_best_density_for_display_list() {
  ...
  let high =
    "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='4' height='4'></svg>";
...
fn background_attachment_fixed_anchors_to_viewport_in_display_list() {
  ...
  let second =
    FragmentNode::new_block_styled(Rect::from_xywh(1.0, 0.0, 1.0, 1.0), vec![], style.clone());
...
fn background_attachment_local_clips_to_padding_box_in_display_list() {
  ...
  // Border box stays unpainted due to padding-box clipping for local backgrounds.
  assert_eq!(pixel(&pixmap, 1, 1), (255, 255, 255, 255));
  // Padding box paints the background image.
...
fn background_attachment_local_anchors_to_padding_box() {
  ...
  let fragment =
    FragmentNode::new_block_styled(Rect::from_xywh(0.0, 0.0, 6.0, 6.0), vec![], Arc::new(style));
  let list = fastrender::paint::display_list_builder::DisplayListBuilder::new()
...
fn fragment_box_shadow_emits_items() {
  ...
  assert!(shadows
    .iter()
    .any(|s| !s.inset && s.offset == Point::new(2.0, 3.0)));
...
fn builder_skip_ink_carves_segments() {
  ...
  base_style.text_underline_position = TextUnderlinePosition::FromFont;
  base_style.font_size = 20.0;

  let text = "g g";
  let font_ctx = FontContext::new();
  let shaper = ShapingPipeline::new();
  let runs = match shaper.shape(text, &base_style, &font_ctx) {
    Ok(r) if !r.is_empty() => r,
    _ => return,
  };

  let run = runs.first().expect("shaped run");
  let Ok(metrics) = run.font.metrics() else {
    return;
  };
  ...
  let center = (min_top + max_bottom) * 0.5;
  let direction = if base >= 0.0 { 1.0 } else { -1.0 };
  let offset = (baseline_y - base - center) / direction;

  let mut style = base_style;
  style.text_underline_offset = TextUnderlineOffset::Length(Length::px(offset));
  let fragment = FragmentNode::new_text_shaped(
  ...
  let segments = underline
    .segments
    .as_ref()
    .expect("segments for skip-ink auto");
...
fn builder_skip_ink_none_draws_full_line() {
  ...
  assert!(
    underline.segments.is_none(),
    "skip-ink:none should not carve segments"
  );
}
