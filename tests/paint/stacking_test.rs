...
use fastrender::StackingContextReason;
use std::sync::Arc;
...
fn block_fragment_with_children(
  x: f32,
  y: f32,
  w: f32,
  h: f32,
...
fn test_paint_order_iterator() {
  ...
  neg_child
    .fragments
    .push(block_fragment(10.0, 10.0, 20.0, 20.0));
  ...
  pos_child
    .fragments
    .push(block_fragment(30.0, 30.0, 20.0, 20.0));
...
fn test_fragment_count() {
  ...
  sc.layer5_inlines
    .push(text_fragment(30.0, 0.0, 10.0, 10.0, "hi"));
...
fn test_total_fragment_count() {
  ...
  child
    .layer3_blocks
    .push(block_fragment(20.0, 0.0, 10.0, 10.0));
...
fn test_multiple_triggers_precedence() {
  ...
  assert!(
    reason == Some(StackingContextReason::FixedPositioning)
