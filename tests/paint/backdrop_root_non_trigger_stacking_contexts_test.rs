...
#[test]
fn backdrop_filter_crosses_will_change_container_type_stacking_context() {
  // `will-change` should proactively create stacking contexts for properties that would do so when
  // set to non-initial values (including `container-type`).
  let html = r#"<!doctype html>
    <style>
      body { margin: 0; background: rgb(255 0 0); }
      #sc { position: relative; will-change: container-type; width: 60px; height: 60px; }
      #overlay { position: absolute; left: 0; top: 0; width: 40px; height: 40px; backdrop-filter: invert(1); }
    </style>
    <div id="sc"><div id="overlay"></div></div>
  "#;
