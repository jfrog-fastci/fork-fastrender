...
const HTML_PATH: &str = "tests/fixtures/html/svg_filter_displacement_map_semantics.html";
const GOLDEN_PATH: &str = "tests/fixtures/golden/svg_filter_displacement_map_semantics.png";
const DIFF_DIR: &str = "target/svg_filter_displacement_map_semantics_diffs";
const VIEWPORT: (u32, u32) = (256, 128);
...
fn render_fixture() -> Vec<u8> {
  ...
  let pixmap = renderer
    .render_html_with_options(
      &fs::read_to_string(HTML_PATH).expect("read displacement map fixture"),
      options,
    )
    .expect("render displacement map html");
  encode_image(&pixmap, OutputFormat::Png).expect("encode png")
}

fn should_update_golden() -> bool {
  std::env::var("UPDATE_SVG_FILTER_DISPLACEMENT_MAP_SEMANTICS_GOLDEN").is_ok()
}

#[test]
fn svg_filter_displacement_map_semantics_match_golden() {
  let compare_config =
