...
const HTML_PATH: &str = "tests/fixtures/html/svg_filter_filterres_offset_clip.html";
const GOLDEN_PATH: &str = "tests/fixtures/golden/svg_filter_filterres_offset_clip.png";
const DIFF_DIR: &str = "target/svg_filter_filterres_offset_clip_diffs";
...
fn render_fixture() -> Vec<u8> {
  ...
  let pixmap = renderer
    .render_html_with_options(
      &fs::read_to_string(HTML_PATH).expect("read filterRes fixture"),
      options,
    )
    .expect("render filterRes html");
  encode_image(&pixmap, OutputFormat::Png).expect("encode png")
}

fn should_update_golden() -> bool {
  std::env::var("UPDATE_SVG_FILTER_FILTERRES_OFFSET_CLIP_GOLDEN").is_ok()
}

#[test]
fn svg_filter_filterres_offset_clip_matches_golden() {
  let compare_config =
