...
const HTML_PATH: &str = "tests/fixtures/html/svg_filter_color_interpolation_filters.html";
const GOLDEN_PATH: &str = "tests/fixtures/golden/svg_filter_color_interpolation_filters.png";
const DIFF_DIR: &str = "target/svg_filter_color_interpolation_diffs";
const VIEWPORT: (u32, u32) = (64, 32);
...
fn render_fixture() -> Vec<u8> {
  ...
  let pixmap = renderer
    .render_html_with_options(
      &fs::read_to_string(HTML_PATH).expect("read color interpolation fixture"),
      options,
    )
    .expect("render color interpolation html");
  encode_image(&pixmap, OutputFormat::Png).expect("encode png")
}

fn should_update_golden() -> bool {
  std::env::var("UPDATE_SVG_FILTER_COLOR_INTERPOLATION_GOLDEN").is_ok()
}

#[test]
fn svg_filter_color_interpolation_filters_match_golden() {
  let compare_config =
