...
) -> Vec<fastrender::paint::display_list::FontVariation> {
  let mut variations: Vec<fastrender::paint::display_list::FontVariation> = run
    .variations
    .iter()
    .map(|v| fastrender::paint::display_list::FontVariation::new(v.tag, v.value))
    .collect();
