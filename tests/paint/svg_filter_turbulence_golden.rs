...
const HTML_PATH: &str = "tests/fixtures/html/svg_filter_turbulence.html";
const GOLDEN_PATH: &str = "tests/fixtures/golden/svg_filter_turbulence.png";
const DIFF_DIR: &str = "target/svg_filter_turbulence_diffs";
const VIEWPORT: (u32, u32) = (228, 228);
...
fn render_fixture() -> Vec<u8> {
  ...
  let pixmap = renderer
    .render_html_with_options(
      &fs::read_to_string(HTML_PATH).expect("read turbulence fixture"),
      options,
    )
    .expect("render turbulence html");
  encode_image(&pixmap, OutputFormat::Png).expect("encode png")
}

fn should_update_golden() -> bool {
  std::env::var("UPDATE_SVG_FILTER_TURBULENCE_GOLDEN").is_ok()
}

#[test]
fn svg_filter_turbulence_matches_golden() {
  let compare_config =
