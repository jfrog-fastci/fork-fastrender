...
#[test]
fn transitions_outline_shorthand_expansion_dedupes_by_expanded_longhand_name() {
  let html = r#"
    <style>
      @starting-style { #box { outline: 0px solid rgb(255, 0, 0); } }
      #box {
        width: 100px;
        height: 100px;
        outline: 10px solid rgb(0, 0, 255);
        transition-property: outline, outline-width;
        transition-duration: 1000ms, 2000ms;
        transition-timing-function: linear;
      }
    </style>
  ...
  let viewport = mid.viewport_size();
  // 500ms in:
  // - outline (1000ms) would yield 5px,
  // - outline-width (2000ms) should win after expansion, yielding 2.5px.
  animation::apply_transitions(&mut mid, 500.0, viewport);
  assert!((fragment_outline_width(&mid, box_id) - 2.5).abs() < 1e-3);
}
...
#[test]
fn transition_property_deduplicates_and_last_entry_wins() {
  let html = r#"
    <style>
      @starting-style { #box { opacity: 0; transform: translateX(0px); } }
      #box {
  ...
  // `opacity` appears twice (`opacity` + `all`), so the last entry should win. That means we should
  // use the 2000ms duration (second duration), not the 1000ms duration.
  let mut mid = fragment_tree.clone();
  let viewport = mid.viewport_size();
  animation::apply_transitions(&mut mid, 1000.0, viewport);
  assert!((fragment_opacity(&mid, box_id) - 0.5).abs() < 1e-3);
