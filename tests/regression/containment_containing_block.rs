...
fn diff_dir() -> PathBuf {
  PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/containment_containing_block_diffs")
}
...
#[test]
fn containment_containing_block_regression() {
  ...
  let html_path = fixtures_dir().join("containment_containing_block/index.html");
  let html =
  ...
  let golden_name = "containment_containing_block";
