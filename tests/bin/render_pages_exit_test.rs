...
fn render_pages_normalizes_pages_filter_host_case() {
  ...
  assert!(temp
    .path()
    .join("fetches/renders/example.com.png")
    .is_file());
}
...
fn render_pages_filters_multiple_pages() {
  ...
  fs::write(
    html_dir.join("bar.html"),
    "<!doctype html><title>Bar</title>",
  )
  .expect("write bar html");
  ...
  assert!(
    status.success(),
    "expected render_pages to succeed for matching filters"
  );
...
fn render_pages_accepts_full_url_with_query() {
  ...
  fs::write(
    html_dir.join(format!("{}.html", stem)),
    "<!doctype html><title>Foo</title>",
  )
  .expect("write html");
  ...
  assert!(
    status.success(),
    "expected render_pages to succeed for full URL filter"
  );
  assert!(temp
    .path()
...
fn render_pages_normalizes_positional_filters() {
  ...
  assert!(
    status.success(),
    "expected render_pages to succeed for positional filter"
  );
  assert!(temp
    .path()
    .join("fetches/renders/example.com.png")
...
fn render_pages_combines_multiple_pages_options() {
  ...
  .expect("write foo html");
  fs::write(
    html_dir.join("baz.html"),
    "<!doctype html><title>Baz</title>",
  )
  .expect("write baz html");
  ...
  assert!(
    status.success(),
    "expected render_pages to succeed for combined filters"
  );
  let renders = temp.path().join("fetches/renders");
  assert!(renders.join("foo.png").is_file());
...
fn render_pages_combines_pages_options_and_positionals() {
  ...
  assert!(renders.join("baz.png").is_file());
  assert!(
    renders.join("_summary.log").is_file(),
    "expected summary log to be written"
  );
}
...
fn render_pages_combines_pages_and_positional_with_full_urls() {
  ...
  assert!(
    status.success(),
    "expected render_pages to succeed for mixed filters"
  );
  let renders = temp.path().join("fetches/renders");
...
fn render_pages_ignores_non_matching_filters_if_some_match() {
  ...
  assert!(
    status.success(),
    "expected render_pages to succeed with partial matches"
  );
  let renders = temp.path().join("fetches/renders");
  assert!(renders.join("foo.png").is_file());
