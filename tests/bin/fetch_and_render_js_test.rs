...
#[test]
fn js_flag_supports_document_write_from_promise_microtask_during_parsing() {
  let tmp = tempfile::TempDir::new().expect("tempdir");
  let html_path = tmp.path().join("page.html");
  fs::write(
    &html_path,
...
<script>
  ...
  assert_red(
    no_js_pixel,
    "baseline run should not execute document.write from a Promise microtask",
  );
  assert_green(
    js_pixel,
    "JS run should allow document.write() from a Promise microtask during parsing",
  );
}
