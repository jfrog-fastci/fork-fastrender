...
#[test]
fn transform_stream_async_transform_rejection_symbol_errors_readable_side() -> Result<(), VmError> {
...
globalThis.__writeRejectedIsSymbol = false;
...
(async () => {
  const ts = new TransformStream({
    transform() {
      return Promise.reject(Symbol('boom'));
    }
  });
  ...
  try {
    ...
    globalThis.__writeRejectedIsSymbol = (typeof e === 'symbol');
  }
...
"#,
  ...
  assert_eq!(realm.exec_script("globalThis.__writeRejected")?, Value::Bool(true));
  assert_eq!(
    realm.exec_script("globalThis.__writeRejectedIsSymbol")?,
    Value::Bool(true)
  );
