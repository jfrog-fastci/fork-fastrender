...
#[test]
fn transform_stream_abort_symbol_errors_readable_without_throwing() -> Result<(), VmError> {
...
globalThis.__abortSettled = false;
globalThis.__abortRejected = false;
...
(async () => {
  ...
  const abortPromise = writer.abort(Symbol('boom'));

  try {
    await abortPromise;
    globalThis.__abortSettled = true;
  } catch (e) {
    globalThis.__abortSettled = true;
    globalThis.__abortRejected = true;
  }
...
"#,
  ...
  assert_eq!(realm.exec_script("globalThis.__abortSettled")?, Value::Bool(true));
  assert_eq!(realm.exec_script("globalThis.__abortRejected")?, Value::Bool(false));
