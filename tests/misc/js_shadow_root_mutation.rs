...
use fastrender::js::{EventLoop, JsExecutionOptions, VirtualClock, WindowHost, WindowHostState};
...
fn host_state_with_empty_body_webidl() -> Result<(WindowHostState, EventLoop<WindowHostState>)> {
  let renderer_dom = parse_html("<!doctype html><body></body>")?;
  let dom = Dom2Document::from_renderer_dom(&renderer_dom);
  debug_assert!(dom.body().is_some());
  let clock = Arc::new(VirtualClock::new());
  let event_loop = EventLoop::<WindowHostState>::with_clock(clock.clone());
...
#[test]
fn shadow_root_inserted_as_fragment_open_webidl() -> Result<()> {
  let (mut host, mut event_loop) = host_state_with_empty_body_webidl()?;
...
#[test]
fn shadow_root_inserted_as_fragment_closed_webidl() -> Result<()> {
  let (mut host, mut event_loop) = host_state_with_empty_body_webidl()?;
  ...
  let value = host.exec_script_in_event_loop(
    ...
    (function () {
      ...
      // The ShadowRoot must remain attached after fragment-style insertion.
      try {
        host.attachShadow({ mode: 'open' });
