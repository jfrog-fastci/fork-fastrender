...
#[test]
fn readable_stream_controller_error_symbol_errors_stream_without_throwing() -> Result<(), VmError> {
  let mut realm = WindowRealm::new(WindowRealmConfig::new("https://example.invalid/"))?;

  realm.exec_script(
    r#"
globalThis.__ctor_threw = false;
globalThis.__after_error = null;
  ...
  rs = new ReadableStream({
    start(controller) {
      try {
        ...
        globalThis.__after_error = true;
      } catch (e) {
        globalThis.__after_error = false;
      }
    }
  });
} catch (e) {
...
"#,
  ...
  assert_eq!(realm.exec_script("globalThis.__after_error")?, Value::Bool(true));
