...
#[test]
fn mask_border_shorthand_outset_without_width_triggers_backdrop_root() {
  let html_without_backdrop_root = r#"<!doctype html>
    <style>
      html, body { margin: 0; padding: 0; background: rgb(255 0 0); }
      #parent {
  ...
  let html_with_backdrop_root = r#"<!doctype html>
    <style>
      ...
      #parent {
        ...
        height: 20px;
        margin: 20px;
        /* The `mask-border` shorthand allows specifying outset without width (`/ / <outset>`). */
        mask-border: url(#missing) 30 / / 0;
      }
      #overlay {
        width: 40px;
        height: 40px;
...
#[test]
fn invalid_mask_border_slice_fill_position_does_not_clear_backdrop_root() {
  let html_without_backdrop_root = r#"<!doctype html>
    <style>
      html, body { margin: 0; padding: 0; background: rgb(255 0 0); }
      #parent {
  ...
  let html_with_backdrop_root = r#"<!doctype html>
    <style>
      ...
      #parent {
        ...
        margin: 20px;
        mask-border: url(#missing) 30;
        /* `fill` must follow slice numeric values; invalid placement should not clear the prior
           mask-border value (and thus must not remove the Backdrop Root boundary). */
        mask-border: fill 30;
      }
      #overlay {
...
#[test]
fn invalid_mask_border_mode_does_not_clear_backdrop_root() {
  let html_without_backdrop_root = r#"<!doctype html>
    <style>
      html, body { margin: 0; padding: 0; background: rgb(255 0 0); }
      #parent {
...
#[test]
fn invalid_mask_border_outset_percentage_does_not_clear_backdrop_root() {
  let html_without_backdrop_root = r#"<!doctype html>
    <style>
      html, body { margin: 0; padding: 0; background: rgb(255 0 0); }
      #parent {
  ...
  let html_with_backdrop_root = r#"<!doctype html>
    <style>
      ...
      #parent {
        ...
        margin: 20px;
        mask-border: url(#missing) 30;
        /* `mask-border-outset` does not allow percentages. Invalid shorthand values must not clear
           the prior mask-border value (and thus must not remove the Backdrop Root boundary). */
        mask-border: 30 / 10px / 10%;
      }
      #overlay {
        width: 40px;
        height: 40px;
