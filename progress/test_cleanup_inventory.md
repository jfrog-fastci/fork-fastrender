...
| `tests/misc/` | grab-bag integration tests (legacy bucket) | `tests/integration.rs::misc` + migrate unit tests into `src/**` | Some internal/unit tests have been migrated out already (e.g. composed DOM snapshotting + exportparts algorithm tests are now unit tests in `src/dom.rs`; old files are stubs). VM JS hook payload regressions now live as unit tests under `src/js/vmjs/window_timers.rs` + `src/js/vmjs/module_loader.rs`. |
